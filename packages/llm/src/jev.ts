...
export function jevResponseSchema(request: JevRequest) {
  return JevResponseSchema.superRefine((response, ctx) => {
    ...
    if (
      ...
      issue("Jev question IDs do not match");
    for (const [id, question] of Object.entries(request.questions)) {
      const answer = response.answers[id];
      if (!answer || answer.type !== question.type) {
        issue(`Jev answer type mismatch: ${id}`);
        continue;
      }
      ...
      if (
        JSON.stringify(Object.keys(answer.probabilities).sort()) !== JSON.stringify(options.sort())
      )
        issue(`Jev option mismatch: ${id}`);
      ...
      if (
        ...
        issue(`Jev probabilities must sum to one: ${id}`);
      if (
        answer.type === "choice" &&
        (!options.includes(answer.choice) ||
          answer.probabilities[answer.choice]! <
            Math.max(...Object.values(answer.probabilities)) - 1e-6)
      )
        issue(`Invalid Jev choice: ${id}`);
      if (answer.type === "score" && answer.score > options.length - 1)
        issue(`Invalid Jev score: ${id}`);
    }
  });
}

export type AskJevRunner = (
  input: string,
...
export const runAskJev: AskJevRunner = (input, { signal, timeoutMs }) =>
  new Promise((resolve, reject) => {
    ...
    if (Buffer.byteLength(input) > MAX_BYTES) {
      reject(new Error("Jev request exceeds 1 MiB"));
      return;
    }
    ...
    const child = spawn(
      process.env.ASK_JEV_BIN?.trim() || "ask-jev",
    ...
    child.on("error", () =>
      finish(new Error("Could not launch ask-jev; install the CLI or set ASK_JEV_BIN")),
    );
    ...
    child.stderr.on("data", (chunk: Buffer) => {
      ...
      if (stderrSize > MAX_BYTES) finish(new Error("Jev stderr exceeds 1 MiB"));
    });
    ...
    child.on("close", (code) => {
      if (code !== 0)
        finish(
          new Error(
            `ask-jev failed (exit ${code}); check CLI credentials, service access, and its private request log`,
          ),
        );
      else
        finish(
          stdinFailed
            ? new Error("Could not send the complete request to ask-jev (exit 0)")
            : undefined,
        );
    });
...
export class AskJevProvider implements DecisionProvider {
  ...
  async decide<T>(request: DecisionRequest<T>): Promise<DecisionResult<T>> {
    if (request.kind !== "jev" || !request.preparedPrompt)
      throw new Error("Jev requires a typed Jev request");
    let usage: UsageV2 = unknownUsage(),
      model = request.model;
    try {
      const raw = await this.run(request.preparedPrompt.input, {
        signal: request.signal,
        timeoutMs: Math.min(request.timeoutMs ?? 20_000, 20_000),
      });
      ...
      if (measured.success && measured.data) {
        usage = {
          ...usage,
          inputTokens: measured.data.input_tokens,
          outputTokens: measured.data.output_tokens,
          totalTokens: measured.data.input_tokens + measured.data.output_tokens,
        };
      }
