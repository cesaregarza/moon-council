...
export function supportsExplicitPromptCaching(model: string): boolean {
  ...
  return Boolean(
    match && (Number(match[1]) > 5 || (Number(match[1]) === 5 && Number(match[2] ?? 0) >= 6)),
  );
}
...
function cachedInput(prompt: PreparedPrompt, formatInstructions = ""): ResponseInput {
  ...
  const input: ResponseInput = [
    message("developer", behavior + (formatInstructions ? `\n\n${formatInstructions}` : ""), true),
  ];
  ...
  if (prompt.publicInput) {
    ...
    try {
      ...
      if (
        ...
        ["agent_v3_1", "agent_v3_2"].includes(parsed.PUBLIC_GAME_STATE?.protocolVersion) &&
        Array.isArray(parsed.PUBLIC_GAME_STATE?.evidence)
      )
        state = parsed.PUBLIC_GAME_STATE;
    } catch {
      /* Legacy free-text public context remains intact. */
    }
    ...
    if (state && cachePublic) {
      ...
      input.push(
        message(
          "user",
          JSON.stringify({
            DURABLE_PUBLIC_EVIDENCE: records.filter((r) => DURABLE_EVENTS.has(r.type)),
          }),
          true,
        ),
      );
      input.push(
        message(
          ...
          JSON.stringify({
            PUBLIC_GAME_STATE: {
              ...current,
              evidence: records.filter((r) => !DURABLE_EVENTS.has(r.type)),
            },
          }),
          true,
        ),
  ...
  if (prompt.sharedInput)
    input.push(
      message(
        "user",
        prompt.sharedInput,
        !prompt.publicInput && prompt.cache?.boundary !== "instructions",
      ),
    );
...
export function openAIRequest<T>(
  ...
  return {
    ...
    ...(explicit
      ...
      : {
          instructions:
            prompt.instructions +
            (request.apiResponseFormat ? `\n\n${request.apiResponseFormat.instructions}` : ""),
          input: [prompt.publicInput, prompt.privateInput, prompt.sharedInput, prompt.input]
            .filter(Boolean)
            .join("\n"),
    ...
    text: {
      format: {
        type: "json_schema",
        name: request.apiResponseFormat?.name ?? request.schemaName,
        strict: true,
        schema: providerJsonSchema(request.apiResponseFormat?.schema ?? request.schema) as Record<
          string,
          unknown
        >,
      },
...
export function cacheComparisonGroup<T>(
  request: DecisionRequest<T>,
