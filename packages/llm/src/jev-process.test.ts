...
async function wrapper(body: string) {
  const directory = await mkdtemp(join(tmpdir(), "jev-process-"));
...
describe.skipIf(process.platform === "win32")("Jev subprocess lifecycle", () => {
  ...
  it("retains a useful exit code when the subprocess closes stdin early", async () => {
    ...
    await expect(runAskJev("x".repeat(1024 * 1024), { timeoutMs: 5000 })).rejects.toThrow(
      "exit 37",
    );
  });
  it("kills a wrapper and its grandchild on timeout", async () => {
    const directory = await wrapper(
      'const {spawn}=require("node:child_process");const {writeFileSync}=require("node:fs");const child=spawn(process.execPath,["-e","setInterval(()=>{},1000)"],{stdio:"inherit"});writeFileSync(__dirname+"/pid",String(child.pid));setInterval(()=>{},1000);',
    );
    ...
    for (let n = 0; n < 30 && live; n++) {
      try {
        ...
        live = stat.split(") ")[1]![0] !== "Z";
      } catch (error) {
        // A process can disappear before open (ENOENT) or between open/read (ESRCH).
        if (["ENOENT", "ESRCH"].includes((error as NodeJS.ErrnoException).code ?? "")) live = false;
        else throw error;
      }
    ...
    try {
      ...
      if (live) process.kill(pid, "SIGKILL");
    }
  });
});
