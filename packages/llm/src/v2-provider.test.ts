...
import { DecisionReportV2Schema, providerJsonSchema, reportSchema, type PlayerContextV2, type UsageV2 } from "@werewolf/contracts";
...
function request<T>(schema: z.ZodType<T>, overrides: Partial<DecisionRequest<T>> = {}): DecisionRequest<T> {
  return {
    kind: "decision_v2",
    model: "gpt-test",
...
function context(overrides: Partial<PlayerContextV2> = {}): PlayerContextV2 {
  return {
    ...
    phase: "day_vote",
    day: 1,
    self: { id: "p1", name: "Player 1", role: structuredClone(STARTER_ROLES.find((role) => role.id === "villager")!) },
    players: [
...
describe("V2 provider contracts", () => {
  ...
  it("serializes each prepared prompt independently without conversation state", async () => {
    ...
    await provider.decide(request(answerSchema, { preparedPrompt: { instructions: "frozen-a", input: "packet-a" } }));
    await provider.decide(request(answerSchema, { preparedPrompt: { instructions: "frozen-b", input: "packet-b" } }));
  ...
  it("emits a strict JSON-compatible report schema", () => {
    for(const kind of ["discussion","night_action","team_point","vote","pass"] as const) for(const commitOnly of [false,true]) assertStrictJsonSchema(z.toJSONSchema(reportSchema(kind,commitOnly),{target:"draft-7"}));
  ...
  it("keeps the provider schema stable when delivered citation IDs change",async()=>{
    const first=context({sources:[{id:"known-source",type:"speech.public",day:1,scope:"public",data:{text:"a claim"}}]});
    const second=context({sources:[{id:"different-source",type:"speech.public",day:1,scope:"public",data:{text:"another claim"}}]});
    const firstSchema=reportSchema("vote",true,first),secondSchema=reportSchema("vote",true,second);
    ...
    const fake=await new FakeDecisionProvider().decide(request(firstSchema,{contextV2:first,proposalKind:"vote",commitOnly:true}));
    // Domain validation, rather than a repeated enum in the provider schema, rejects this later.
    expect(firstSchema.safeParse({...fake.data,observations:["not-delivered"]}).success).toBe(true);
    // Stable identifier syntax prevents a model from placing explanatory prose in a citation slot.
    expect(firstSchema.safeParse({...fake.data,observations:["This is an inference, not a source ID."]}).success).toBe(false);
  });
  ...
  it("reports unknown usage once when OpenAI times out", async () => {
    ...
    await expect(provider.decide(request(answerSchema, { timeoutMs: 5, onUsage: (usage) => usages.push(usage) }))).rejects.toThrow(
      "OpenAI decision timed out after 5ms",
    );
    expect(usages).toEqual([
      { inputTokens: null, outputTokens: null, totalTokens: null, cachedInputTokens: null, cacheWriteInputTokens: null, reasoningTokens: null },
    ]);
  });
  ...
  it("honors external OpenAI abort and reports unknown usage", async () => {
    ...
    const usages: UsageV2[] = [];
    const pending = provider.decide(request(answerSchema, { signal: controller.signal, onUsage: (usage) => usages.push(usage) }));
    controller.abort();
    ...
    const provider = new CodexLoginProvider({
      ...
      createClient: () => ({
        startThread: (options) => {
          ...
          return { run: async () => ({ finalResponse: JSON.stringify({ answer: "ok" }), usage: null }) };
        },
      }),
    });

    await provider.decide(request(answerSchema, { reasoningEffort: "high", onUsage: (usage, metadata) => {
      usages.push(usage);
  ...
  it("uses separate Codex threads for concurrent isolated player decisions",async()=>{
    ...
    const provider=new CodexLoginProvider({
      createClient:()=>({startThread:(options)=>{
        started+=1;workingDirectories.push(options?.workingDirectory??"");
        return {run:async()=>{active+=1;maxActive=Math.max(maxActive,active);await new Promise(resolve=>setTimeout(resolve,5));active-=1;return {finalResponse:'{"answer":"ok"}',usage:null};}};
      }}),
    });
    await Promise.all([
      provider.decide(request(answerSchema,{playerId:"p1",preparedPrompt:{instructions:"shared stable prefix",input:"private packet one"}})),
      provider.decide(request(answerSchema,{playerId:"p2",preparedPrompt:{instructions:"shared stable prefix",input:"private packet two"}})),
    ]);
    expect(started).toBe(2);expect(maxActive).toBe(2);expect(new Set(workingDirectories).size).toBe(2);
  });
  ...
  it("reuses one isolated Codex thread within an episode and appends only the changing suffix",async()=>{
    ...
    const provider=new CodexLoginProvider({
      createClient:()=>({startThread:()=>{
        started+=1;
        return {run:async(input)=>{inputs.push(input);return {finalResponse:'{"answer":"ok"}',usage:null};}};
      }}),
    });
    const preparedPrompt={instructions:"game prefix",sharedInput:"stable decision context",input:"suffix one"};
    await provider.decide(request(answerSchema,{sessionKey:"game:decision:0",preparedPrompt}));
    await provider.decide(request(answerSchema,{sessionKey:"game:decision:0",preparedPrompt:{...preparedPrompt,input:"suffix two"}}));
  ...
  it("makes an informationless fake vote a uniform proposal rather than a failure fallback", async () => {
    ...
    expect(result.data.proposal).toEqual({ kind: "vote", targets: { mode: "uniform", playerIds: ["p2", "p3"] } });
