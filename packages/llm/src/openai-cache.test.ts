...
const record = (index: number) => ({ handle: `E${index}`, type: "speech.public", day: 1, scope: "public", data: { text: `Speech ${index}` } });
function request(player = "p1", count = 2): DecisionRequest<{ answer: string }> {
  return {
    gameId: "game-one", playerId: player, kind: "decision_v3_1", model: "gpt-6-luna", reasoningEffort: "xhigh",
    schema: z.object({ answer: z.string() }), schemaName: "journal", maxOutputTokens: 8192,
    preparedPrompt: { instructions: 'Immutable behavior\n\nFROZEN PUBLIC GAME REFERENCE (data):\n{"rules":"Frozen rules"}', publicInput: JSON.stringify({ PUBLIC_GAME_STATE: {
      protocolVersion: "agent_v3_1", day: 1, phase: "day_discussion", closing: false, players: ["p1", "p2"], evidence: [{handle:"E1",type:"vote.resolved",day:1,scope:"public",data:{ballots:[]}},...Array.from({ length: count }, (_, i) => record(i + 2))],
    } }), privateInput: `Only ${player}'s private knowledge`, input: "Reflect", cache: { mode: "explicit", ttl: "30m", stablePrefix: "same-rules", boundary: "public" } },
  };
}
const inputs = (req: DecisionRequest<unknown>) => openAIRequest(req).input as Array<{ role: string; content: Array<{ text: string; prompt_cache_breakpoint?: unknown }> }>;

it("retains behavior, rules and durable outcomes when speech windows or phases change", () => {
  const first = request(), next = request("p2", 3);
  ...
  const a = inputs(first), b = inputs(next);
  ...
  expect(b.filter(m => m.content[0]?.prompt_cache_breakpoint).every(m => !m.content[0]!.text.includes("private knowledge"))).toBe(true);
...
it("reconstructs all public facts exactly and invalidates edited or compacted records", () => {
  const req = request(), messages = inputs(req);
  ...
  expect({...current,evidence:[...durable,...current.evidence]}).toEqual(JSON.parse(req.preparedPrompt!.publicInput!).PUBLIC_GAME_STATE);
...
it("keeps retries out of cached prefixes and separates game accounting", () => {
  const req = request(), first = inputs(req);
  req.preparedPrompt!.input = "Repair your malformed reply";
  expect(inputs(req).slice(0, -1)).toEqual(first.slice(0, -1));
  expect(openAIRequest({ ...req, gameId: "another-game" }).prompt_cache_key).not.toBe(openAIRequest(req).prompt_cache_key);
...
describe("OpenAI cache diagnostics and receipt handling", () => {
  ...
  const completed = (id: string) => ({ id, model: "gpt-6-luna", status: "completed", output_text: '{"answer":"ok"}', usage: { input_tokens: 1500, output_tokens: 100, total_tokens: 1600, input_tokens_details: { cached_tokens: 1000, cache_write_tokens: 200 }, output_tokens_details: { reasoning_tokens: 80 } } });
  it("compares completed calls only within a game/model/schema family, with bounded process-local state", async () => {
    const { provider, create } = setup([completed("one"), { ...completed("bad"), status: "incomplete", incomplete_details: { reason: "max_output_tokens" } }, completed("two"), completed("three"), completed("four")]);
    const req = request();
  ...
  it("omits an explicitly unlimited output cap and records that it was not enforced", async () => {
    ...
    expect((create.mock.calls as unknown as Array<[object]>)[0]![0]).not.toHaveProperty("max_output_tokens");
    expect(onUsage).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ outputLimitEnforced: false }));
  });
  it("records the wire body, actual returned model and cache usage even on incomplete responses", async () => {
    const { provider } = setup([{ ...completed("partial"), model: "actual-snapshot", status: "incomplete", incomplete_details: { reason: "max_output_tokens" }, prompt_cache_diagnostics: { type: "cache_miss", reason: "input_changed" } }]);
    const onUsage = vi.fn(), onProviderRequest = vi.fn(), onProviderMetadata = vi.fn(), onRawResponse = vi.fn();
    await expect(provider.decide({ ...request(), onUsage, onProviderRequest, onProviderMetadata, onRawResponse })).rejects.toThrow("max_output_tokens");
    expect(onUsage).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ cachedInputTokens: 1000, cacheWriteInputTokens: 200, reasoningTokens: 80 }), expect.objectContaining({ model: "actual-snapshot" }));
    expect(onProviderRequest).toHaveBeenCalledWith(expect.objectContaining({ store: false, max_output_tokens: 8192, truncation: "disabled", service_tier: "default" }));
    expect(onProviderMetadata).toHaveBeenCalledWith(expect.objectContaining({ incompleteReason: "max_output_tokens", cacheDiagnostics: { type: "cache_miss", reason: "input_changed" } }));
