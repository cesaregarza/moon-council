 ...
 */
export const STANDARD_ROLE_IDS_V3 = [
  "werewolf",
  "werewolf",
  "seer",
  "bodyguard",
...
export const rosterRoleVersion = (id: string): number =>
  id === "werewolf" || id === "doctor" ? 2 : 1;
...
export const ModelSettingsSchema = z.object({
  ...
  provider: z.enum(["fake", "codex", "codex_direct", "openai"]),
});
...
export const GameConfigV2Schema = z
  .object({
    ...
    protocolVersion: z
      .enum(["agent_v2", "agent_v2_1", "agent_v3", "agent_v3_1", "agent_v3_2"])
      .default("agent_v2"),
    rules: z
      .object({
        packExecution: z.literal("any_unblocked").default("any_unblocked"),
        revealBallots: z.literal(true).default(true),
        firstCycle: z.enum(["night_first", "day_first"]).default("night_first"),
      })
      .default({ packExecution: "any_unblocked", revealBallots: true, firstCycle: "night_first" }),
  ...
  .superRefine((value, ctx) => {
    if (
      ...
      ctx.addIssue({
        code: "custom",
        message: "Jev decisions require the V3.1 or V3.2 handle protocol",
      });
    if (
      value.decisionEngine.mode === "jev" &&
      value.decisionEngine.workflow !== "legacy_v1" &&
      value.discussion.speakerSelection !== "listener_auction"
    )
      ctx.addIssue({
        code: "custom",
        message: "The journal Jev workflow requires listener auctions",
      });
    if (
      value.decisionEngine.mode === "jev" &&
      value.decisionEngine.workflow === "journal_v4" &&
    ...
    if (roster) {
      ...
      if (value.seats.length !== 8 || JSON.stringify(actual) !== JSON.stringify(expected))
        ctx.addIssue({
          code: "custom",
          message: `${value.preset} requires its frozen eight-seat starter roster; use custom-v2 for edited roles`,
        });
    }
    if (value.seats.length !== value.roleDeck.length)
      ctx.addIssue({ code: "custom", message: "role count must equal seat count" });
    if (new Set(value.seats.map((s) => s.id)).size !== value.seats.length)
      ctx.addIssue({ code: "custom", message: "seat IDs must be unique" });
    for (const faction of ["werewolf", "village"])
      if (!value.roleDeck.some((r) => r.alignment === faction))
        ctx.addIssue({ code: "custom", message: `missing ${faction} faction` });
    for (const seat of value.seats)
      if (!value.modelSettings[seat.id])
        ctx.addIssue({ code: "custom", message: `missing resolved model for ${seat.id}` });
    for (const role of value.roleDeck)
      for (const action of role.actions) {
        const filters = [
          ...(action.target.allowedAlignments ?? []),
          ...(action.target.deniedAlignments ?? []),
        ];
        if (
          filters.length &&
          (!role.knowledge.includes("alignment_team") ||
            filters.some((alignment) => alignment !== role.alignment))
        )
          ctx.addIssue({
            code: "custom",
            message:
              "Alignment-filtered legal targets require team knowledge and may only restrict the actor's own alignment; hidden alignment filters would leak roles.",
          });
      }
  });
...
const NewGameSafetySchema = SafetyLimitsV2Schema.extend({
  maxOutputTokens: z.number().int().min(100).max(32_768).default(8_192),
});
...
export const CreateGameV2RequestSchema = CreateGameRequestSchema.extend({
  ...
  decisionEngine: DecisionEngineSchema.extend({
    workflow: z.enum(["legacy_v1", "journal_v2", "journal_v3", "journal_v4"]).default("journal_v4"),
  }).optional(),
});
export type CreateGameV2Request = z.infer<typeof CreateGameV2RequestSchema>;
export const ExperimentSpecV2Schema = ExperimentSpecSchema.extend({
  schemaVersion: z.literal("experiment_v2"),
  baseConfig: GameConfigV2Schema,
});
...
export const ExperimentSummaryV2Schema = ExperimentSummarySchema.extend({
  schemaVersion: z.literal("experiment_summary_v2"),
  completed: z.number().int(),
  interrupted: z.number().int(),
  failed: z.number().int(),
  budgetTruncated: z.number().int(),
  validOutcomeDenominator: z.number().int(),
  cachedInputTokens: z.number().int(),
  reasoningTokens: z.number().int(),
  unknownUsageAttempts: z.number().int(),
  totalAttempts: z.number().int(),
  estimatedCostIsLowerBound: z.boolean(),
  factionWinRates: z.record(z.string(), z.number()),
  roleWinRates: z.record(z.string(), z.number()),
});
export const StoredExperimentSummarySchema = z.union([
  ExperimentSummaryV2Schema,
  ExperimentSummarySchema,
]);
...
export const BeliefV2Schema = z.strictObject({
  playerId: Id,
  probability: z.number().min(0).max(1),
  basis: z.enum(["prior", "inference", "authorized_fact"]),
  note: Brief,
  sources: Refs,
});
...
export const DecisionBriefTextSchema = z.strictObject({
  action: z.string().trim().min(1).max(4000),
  attention: z.string().trim().min(1).max(3000),
});
export const DecisionBriefSchema = DecisionBriefTextSchema.extend({
  playerId: Id,
  evidenceRevision: z.string().length(64),
});
export const PrivateJournalV2Schema = z.strictObject({
  schemaVersion: z.literal("journal_v2"),
  version: z.number().int().nonnegative(),
  /** Free-form journal; legacy fields remain readable for archived games. */
  text: z.string().optional(),
  decisionBrief: DecisionBriefSchema.optional(),
  attentionNotes: z
    .array(z.strictObject({ playerId: Id, note: Brief, sources: Refs }))
    .max(16)
    .optional(),
  beliefs: z.array(BeliefV2Schema).max(16),
  hypotheses: z.array(HypothesisV2Schema).max(6),
  strategy: z.string().max(600),
  goals: z.array(Brief).max(4),
  unresolvedQuestions: z.array(Brief).max(4),
  deceptionPlan: z.string().max(400).nullable(),
});
export type PrivateJournalV2 = z.infer<typeof PrivateJournalV2Schema>;
export const emptyJournalV2 = (): PrivateJournalV2 => ({
...
export const JournalOperationSchema = z.union([
  ...
  z.strictObject({
    op: z.literal("set_strategy"),
    strategy: z.string().max(600),
    goals: z.array(Brief).max(4),
  }),
  z.strictObject({
    op: z.literal("set_attention"),
    notes: z.array(z.strictObject({ playerId: Id, note: Brief, sources: Refs })).max(16),
  }),
...
export const SpeechActV2Schema = z.strictObject({
  kind: z.enum(["accusation", "challenge", "role_claim", "result_claim", "reply"]),
  targetId: Id.nullable(),
  claim: Brief,
  sourceId: SourceId.nullable(),
});
export const PublicSpeechV2Schema = z.strictObject({
  text: z.string().min(1).max(1_200),
  acts: z.array(SpeechActV2Schema).max(4),
  respondsTo: Refs,
});
export const TargetChoiceV2Schema = z.strictObject({
  mode: z.enum(["direct", "uniform"]),
  playerIds: z.array(Id).min(1).max(16),
});
export const ActionProposalV2Schema = z.discriminatedUnion("kind", [
  ...
  z.strictObject({
    ...
    speech: PublicSpeechV2Schema.nullable(),
    ready: z.boolean(),
    interests: z.array(z.enum(["accused_me", "claims", "mentions_me"])).max(3),
    silenceCase: z.strictObject({ speechAlternative: Brief, advantage: Brief }).nullable(),
  }),
  z.strictObject({ kind: z.literal("pass"), reason: Brief }),
]);
export type ActionProposalV2 = z.infer<typeof ActionProposalV2Schema>;
export const ListenerPreferenceV1Schema = z.strictObject({
  playerId: Id,
...
const AgentBeliefSuggestionV1Schema = z.strictObject({
  playerId: Id,
  probability: z.number().min(0).max(1),
  note: Brief,
  evidence: EvidenceHandlesV3Schema,
});
const AgentHypothesisSuggestionV1Schema = z.strictObject({
  statement: Brief,
  confidence: z.number().min(0).max(1),
  evidence: EvidenceHandlesV3Schema,
});
export const MemorySuggestionsV3Schema = z.strictObject({
  attentionUpdate: z
    .array(z.strictObject({ playerId: Id, note: Brief, evidence: EvidenceHandlesV3Schema }))
    .max(16)
    .nullable()
    .optional(),
...
export const DiscussionPlanV3Schema = z.strictObject({
  kind: z.enum(["accusation", "challenge", "role_claim", "result_claim", "reply"]),
  targetId: Id.nullable(),
  respondsTo: EvidenceHandlesV3Schema,
  point: Brief,
});
export const DiscussionBidV3Schema = z.strictObject({
  urge: z.number().min(0).max(1),
  ready: z.boolean(),
  plan: DiscussionPlanV3Schema.nullable(),
  listen: z.record(z.string(), z.number().min(0).max(1)),
  memory: MemorySuggestionsV3Schema,
  rationale: z.string().max(300),
});
export const ListenerBidV3Schema = z.strictObject({
  ready: z.boolean(),
  listen: z.record(z.string(), z.number().min(0).max(1)),
  memory: MemorySuggestionsV3Schema,
  rationale: z.string().max(300),
});
const SpeechActSuggestionV3Schema = z.strictObject({
  kind: z.enum(["accusation", "challenge", "role_claim", "result_claim", "reply"]),
  targetId: Id.nullable(),
  claim: Brief,
  evidence: EvidenceHandleV3Schema.nullable(),
});
export const SpeechSubmissionV3Schema = z.strictObject({
  text: z.string().min(1).max(1_200).nullable(),
  acts: z.array(SpeechActSuggestionV3Schema).max(4),
  respondsTo: EvidenceHandlesV3Schema,
  rationale: z.string().max(300),
  memory: MemorySuggestionsV3Schema,
});
export const TargetChoiceSubmissionV3Schema = z.strictObject({
  mode: z.enum(["direct", "uniform", "abstain"]),
  choiceHandles: z.array(z.string().min(1).max(8)).max(16),
  rationale: z.string().max(300),
  evidence: EvidenceHandlesV3Schema,
  memory: MemorySuggestionsV3Schema,
...
export type V3TaskKind =
  | "journal_update"
  | "discussion_score"
  | "discussion_free_speech"
  | "discussion_bid"
  | "discussion_listen"
  | "discussion_speech"
  | "closing_response"
...
export const DecisionReportV2Schema = z.strictObject({
  ...
  alternatives: z
    .array(z.strictObject({ id: Id, description: Brief, advantage: Brief, drawback: Brief }))
    .min(1)
    .max(3),
  selectedAlternativeId: Id,
  proposal: ActionProposalV2Schema,
  confidence: z.number().min(0).max(1),
  summary: Brief,
  journalPatch: z.array(JournalOperationSchema).max(6),
  control: z.strictObject({
    kind: z.enum(["commit", "continue"]),
    question: Brief.nullable(),
    reason: z
      .enum(["compare_alternative", "resolve_conflict", "plan_response", "coordination_tradeoff"])
      .nullable(),
  }),
});
export type DecisionReportV2 = z.infer<typeof DecisionReportV2Schema>;
export type DecisionReportWithSpeakerIntentV1 = DecisionReportV2 & {
  speakerIntent: SpeakerIntentV1;
};
/** Preserve shared definitions so per-packet citation enums are not repeated. */
export const providerJsonSchema = (schema: z.ZodType) =>
...
export function reportSchema(
...
): z.ZodType<DecisionReportV2> {
  const proposal = ActionProposalV2Schema.options.find(
  ...
  return packet?.rules.speakerSelection === "listener_auction"
    ? (discussion.extend({ speakerIntent: SpeakerIntentV1Schema }) as z.ZodType<DecisionReportV2>)
    : discussion;
}
...
export interface PlayerContextV2 {
  ...
  self: {
    ...
    name: string;
    personality?: string;
    role: z.infer<typeof RoleDefinitionSchema>;
  };
...
export interface DecisionOpportunityV1 {
  ...
  packet: PlayerContextV2;
  status: "open" | "pending" | "committed" | "paused" | "superseded";
  best: DecisionReportV2 | null;
  recovery: number;
  createdAt: string;
  taskType?: V3TaskKind;
  ...
  journalCompaction?: {
    ...
    sourceReport: DecisionReportV2;
    sourceSubmission: unknown;
    sourceAttemptId: string;
    result?: PrivateJournalV2;
    attemptId?: string;
  };
...
export interface PrivateDeliberationTurnV1 {
  decisionId: string;
  playerId: string;
  turnIndex: number;
  recovery: number;
...
export const unknownUsage = (): UsageV2 => ({
  inputTokens: null,
  outputTokens: null,
  totalTokens: null,
