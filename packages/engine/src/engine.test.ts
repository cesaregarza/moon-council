...
const role = (id: string) => structuredClone(STARTER_ROLES.find((candidate) => candidate.id === id)!);
...
function controlledState(roleIds = ["werewolf", "seer", "doctor", "roleblocker", "mayor", "villager"]) {
...
describe("deterministic moderator engine", () => {
  it("assigns roles and shuffles repeatably from the seed", () => {
    ...
    expect(first.players.map((player) => player.role.id)).toEqual(second.players.map((player) => player.role.id));
  ...
  it("rejects illegal self targets and exhausted charges", () => {
    ...
    expect(validateNightAction(state, { actorId: "p1", actionId: "pack_kill", targetIds: ["p1"] })).toContain(
      "self-targeting is not allowed",
    );
    const oneShot = state.players[1]!.role.actions[0]!;
    oneShot.charges = 1;
    state.actionUses["p2:divine_alignment"] = 1;
    expect(validateNightAction(state, { actorId: "p2", actionId: "divine_alignment", targetIds: ["p3"] })).toContain(
      "action has no charges remaining",
    );
  });
  ...
  it("resolves an admitted final charge without rejecting it as already spent", () => {
    ...
    state.actionUses["p2:divine_alignment"] = 1;
    state.pendingNightActions = [{ actorId: "p2", actionId: "divine_alignment", targetIds: ["p1"] }];
  ...
  it("resolves protection before elimination and delivers private inspection", () => {
    ...
    expect(events.find((item) => item.type === "inspection.delivered")?.audienceIds).toEqual(["p2"]);
  });
  ...
  it("requires every living werewolf to submit the same target", () => {
    ...
    expect(resolveNight(state).find((item) => item.type === "player.eliminated")?.payload.playerId).toBe("p3");
    ...
    expect(missingWolf.find((item) => item.type === "night.resolved")?.payload.skippedUnanimousGroups).toEqual([
      "werewolves",
    ]);
  });
  ...
  it("applies weighted votes and treats a top tie as no elimination", () => {
    ...
    expect(resolveVote(state)).toMatchObject({ targetId: "p1", tied: false, tally: { p1: 3, p3: 1 } });
    state.votes = [
...
describe("information projections", () => {
  it("keeps moderator, other-player, and team secrets out of a player view", () => {
    ...
    const events = [
      ...
      event({ id: "moderator", sequence: 2, visibility: "moderator", payload: { secretRole: "werewolf" } }),
      event({ id: "p2-secret", sequence: 3, visibility: "player", audienceIds: ["p2"], payload: { result: "werewolf" } }),
      event({ id: "p1-secret", sequence: 4, visibility: "player", audienceIds: ["p1"], payload: { result: "village" } }),
      event({ id: "wolf-chat", sequence: 5, visibility: "team", audienceIds: ["p1"], payload: { text: "pack only" } }),
    ];
    const view = projectPlayer(
      state,
      events,
      "p2",
      { beliefs: [], goals: [], strategy: "mine", unresolvedQuestions: [] },
    );
  ...
  it("shows werewolves only pointing gestures from their private team channel", () => {
    ...
    const view = projectPlayer(
      ...
      events,
      "p1",
      { beliefs: [], goals: [], strategy: "private reasoning summary", unresolvedQuestions: [] },
    );
