...
export function archivedCacheCases(dbPath: string, gameId: string, attemptIds: string[]) {
  if (!attemptIds.length || attemptIds.length > 12)
    throw new Error("Select 1–12 explicit attempt IDs");
  const db = new Database(dbPath, { readonly: true, fileMustExist: true });
  try {
    return attemptIds.map((id) => {
      const row = db
        .prepare("SELECT value_json FROM provider_attempts WHERE game_id=? AND id=?")
        .get(gameId, id) as { value_json: string } | undefined;
      if (!row) throw new Error(`Attempt not found: ${id}`);
      const attempt = JSON.parse(row.value_json) as ProviderAttemptRecord;
      if (attempt.provider !== "codex" || attempt.status !== "valid")
        throw new Error("Replay accepts validated recorded Codex attempts only");
      const saved = db
        .prepare("SELECT value_json FROM agent_records WHERE game_id=? AND record_key=?")
        .get(gameId, `decision:${attempt.decisionId}`) as { value_json: string } | undefined;
      ...
      if (JSON.stringify(providerJsonSchema(schema)) !== JSON.stringify(attempt.request.schema))
        throw new Error(
          "Archived schema differs from current schema; refusing to silently rewrite the test",
        );
      const request: DecisionRequest<V31Submission> = {
        ...
        contextV2: op.packet,
        apiResponseFormat: v31ApiResponseFormat(op.packet, task),
        schema,
        schemaName: task.type,
        preparedPrompt: attempt.request as PreparedPrompt,
      ...
      return {
        ...
        validate(value: V31Submission) {
          ...
          errors.push(
            ...validatePreparedReport(report, op.packet, 1200, task.type !== "journal_update")
              .errors,
          );
          return errors;
        },
...
export function prefixComparison(
  ...
  while (
    sameMessages < before.length &&
    sameMessages < messages.length &&
    requestFingerprint(before[sameMessages]) === requestFingerprint(messages[sameMessages])
  )
    sameMessages++;
  return {
    equalLeadingMessages: sameMessages,
    equalLeadingBytes: sameMessages
      ? Buffer.byteLength(JSON.stringify(messages.slice(0, sameMessages)))
      : 0,
    schemaEqual: previous
      ? requestFingerprint(previous.text) === requestFingerprint(current.text)
      : null,
    reasoningEqual: previous
      ? requestFingerprint(previous.reasoning) === requestFingerprint(current.reasoning)
      : null,
  };
}

export function cacheReceiptSummary(
  rows: Array<{
    usage?: UsageV2;
    latencyMs: number;
    original?: { usage: UsageV2; latencyMs: number | null };
  }>,
) {
  const sum = (key: keyof UsageV2) => rows.reduce((n, r) => n + (r.usage?.[key] ?? 0), 0);
  const input = sum("inputTokens"),
    cached = sum("cachedInputTokens"),
    written = sum("cacheWriteInputTokens");
  const complete = rows.every(
    (r) =>
      r.usage?.inputTokens != null &&
      r.usage?.cachedInputTokens != null &&
      r.usage?.cacheWriteInputTokens != null,
  );
  return {
    calls: rows.length,
    inputTokens: input,
    ...
    inputRateEquivalentTokens: complete
      ? input - cached - written + cached * 0.1 + written * 1.25
      : null,
    latencyMs: rows.reduce((n, r) => n + r.latencyMs, 0),
    originalInputTokens: rows.reduce((n, r) => n + (r.original?.usage.inputTokens ?? 0), 0),
    originalLatencyMs: rows.reduce((n, r) => n + (r.original?.latencyMs ?? 0), 0),
  };
}
