...
export function recoverStaleWork(repository: LabRepository, now = Date.now()): number {
  ...
  return store.atomic(() => {
    ...
    for (const job of jobs) {
      const games =
        job.kind === "game"
          ? [repository.getGame(job.target_id)].filter((g) => g !== undefined)
          : repository.listGames(100, job.target_id);
      for (const game of games) {
        if (
          game.config.schemaVersion !== "game_config_v2" ||
          !["running", "stepping", "queued"].includes(game.status)
        )
          continue;
        ...
        if (events.length) {
          ...
          repository.appendEvent(game.id, {
            ...
            day: state.day,
            visibility: "public",
            payload: { reason: "runner recovery requires explicit resume", unresolved: true },
          });
        }
        repository.updateGame(game.id, {
          status: "paused",
          error:
            "runner interrupted; persisted reports retained and uncertain requests require recovery",
        });
      }
      if (job.kind === "experiment")
        repository.updateExperiment(job.target_id, {
          status: "paused",
          error: "runner interrupted; stable run indices retained",
        });
