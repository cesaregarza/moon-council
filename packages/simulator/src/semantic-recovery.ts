...
export function acknowledgeSemanticAnomaly(
  ...
  if (!note.trim() || note.length > 1_000) conflict("An operator note of 1–1000 characters is required");
  ...
  return store.atomic(() => {
    ...
    if (op.status === "committed" && op.jevState?.semanticAcknowledgment) {
      return { decisionId, status: "committed", gameStatus: game.status, alreadyAcknowledged: true };
    }
    if (game.status !== "paused" || op.status !== "paused") conflict("The game and decision must be paused");
    if (game.config.schemaVersion !== "game_config_v2") conflict("V2 game required");
    const final = op.jevState?.semanticFinal;
    if (!op.jevState?.semanticRejected || !final) conflict("No final semantically rejected Jev choice is recorded");
    ...
    const stale = state.phase !== "day_vote"
    ...
    if (stale) conflict("The recorded ballot is stale; acknowledgment cannot bypass changed evidence or legal state");
    if (state.votes.some(vote => vote.voterId === op.playerId)) conflict("The player already cast a ballot");
    ...
    const errors = validateReport(final.report, op.packet, game.config.deliberation.maxJournalTokens);
    ...
    if (targetId && !state.players.some(player => player.id === targetId && player.alive && player.id !== op.playerId)) {
    ...
    repository.appendEvent(gameId, {
      ...
      payload: { playerId: op.playerId, decisionId, attemptId: final.attemptId,
        operator: true, note: note.trim(), issues: op.jevState.semanticIssues, proposal: op.best.proposal },
    });
    repository.appendEvent(gameId, {
      type: "decision.reported", phase: state.phase, day: state.day,
      visibility: "player", audienceIds: [op.playerId],
      payload: { playerId: op.playerId, decisionId, report: op.best, submission: op.bestSubmission,
        taskType: op.taskType, attemptId: final.attemptId, continuation: "operator_acknowledged_anomaly" },
    });
    ...
    const journal = applyJournalV2(op.packet.journal, final.report, game.config.deliberation.maxJournalTokens);
    const committed = store.commit(op, journal, [{
      type: "vote.cast", phase: state.phase, day: state.day,
      visibility: "player", audienceIds: [op.playerId],
