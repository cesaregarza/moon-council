...
import {
  ...
  type DecisionReportWithSpeakerIntentV1,
  type GameEventV1,
...
export function isLedgerProtocol(state: GameState): boolean {
  return (
    state.config.schemaVersion === "game_config_v2" && state.config.protocolVersion === "agent_v3_2"
  );
}
function citableFields(state: GameState): Record<string, string[]> {
  const protocol =
    state.config.schemaVersion === "game_config_v2" ? state.config.protocolVersion : "";
  if (protocol !== "agent_v3_2") return publicFields;
  return Object.fromEntries(
    Object.entries(publicFields).filter(([type]) => type !== "phase.changed"),
  );
}
export function authorizedSources(
  ...
  for (const event of events) {
    ...
    if (
      ...
      event.audienceIds.includes(playerId) &&
      event.type === "inspection.delivered"
    ) {
      privateIndex += 1;
      sourcePrivateIndex = privateIndex;
      scope = "player";
      data = Object.fromEntries(
        ["actorId", "targetId", "targetName", "result"].map((k) => [k, event.payload[k]]),
      );
    }
    if (
      event.visibility === "team" &&
      self.role.knowledge.includes("team_channel") &&
      event.audienceIds.includes(playerId) &&
      event.type === "team.point"
    ) {
      privateIndex += 1;
      sourcePrivateIndex = privateIndex;
      ...
      if (event.day === state.day) {
        scope = "team";
        data = { playerId: event.payload.playerId, targetId: event.payload.targetId };
...
export function buildContextV2(
  ...
  events: GameEventV1[],
  playerId: string,
  journal: PrivateJournalV2,
  opportunityKey: string,
  kind: ActionProposalV2["kind"],
  docket: string[] = [],
  closing = false,
  requestSizer?: (packet: PlayerContextV2) => number,
  requiredSources: string[] = [],
): PlayerContextV2 {
  if (state.config.schemaVersion !== "game_config_v2")
    throw new Error("V2 context requires V2 game");
  const config = state.config;
  const self = state.players.find((p) => p.id === playerId)!;
  const salt = `${config.seed}:presentation:${playerId}:${state.day}:${state.phase}:${opportunityKey}`;
  const knownAllies = self.role.knowledge.includes("alignment_team")
    ? state.players
        .filter((p) => p.id !== playerId && p.role.alignment === self.role.alignment)
        .map((p) => ({ id: p.id, name: p.name }))
    : [];
  const legalActions = self.role.actions
    .filter((a) =>
      kind === "team_point"
        ? a.teamAggregation !== "none" && a.effect === "eliminate"
        : a.teamAggregation === "none",
    )
    .map((action) => ({
      actionId: action.id,
        ...
        state.players
          .filter(
            (p) =>
              validateNightAction(
                state,
                { actorId: playerId, actionId: action.id, targetIds: [p.id] },
                { forTeamPoint: kind === "team_point" },
              ).length === 0,
          )
          .map((p) => p.id),
        `${salt}:targets:${action.id}`,
      ),
    }));
  const roleCounts: Record<string, number> = {};
  for (const role of [...config.roleDeck].sort((a, b) => a.name.localeCompare(b.name)))
  ...
  const packet: PlayerContextV2 = {
    ...
    players: shuffled(
      state.players.map((p) => ({
        ...
        name: p.name,
        alive: p.alive,
        ...(p.revealedRole ? { revealedRole: p.revealedRole } : {}),
      })),
      `${salt}:roster`,
    ),
    ...
    rules: {
      ...
      roleCounts,
      firstCycle: config.rules.firstCycle,
      deathRevealsRole: config.revealRolesOnDeath,
      tieEliminatesNobody: true,
      sealedBallots: true,
      factionObjective: self.role.alignment,
      wolfVictory: "living parity",
      villageVictory: "all wolves eliminated",
      doctor:
        "Self-protection allowed; cannot repeat the preceding night's selected target, even if blocked.",
      pack: "Point only. All living wolves must agree within 3 times living wolves committed pointing opportunities. Any unblocked consenting wolf executes the attack; protection still applies.",
      claimsAreUnverified: true,
      ...(config.decisionEngine.mode === "jev" && config.decisionEngine.workflow !== "legacy_v1"
        ? {
            jevWorkflow: config.decisionEngine.workflow,
            maxJournalTokens: config.deliberation.maxJournalTokens,
            voting:
              "Highest nonzero tally eliminates; one vote can suffice when others abstain. A tie eliminates nobody.",
          }
        : {}),
      speakerSelection: closing ? "frozen_closing" : config.discussion.speakerSelection,
      speakerBias: config.discussion.speakerBias,
      speakerPriorityFormula:
        "(bias + urge) * normalized aggregate willingness to listen; maximum wins; response rights restrict the candidate set before scoring",
      listenerIntent:
        "For listener auctions, privately rate willingness to listen to every other living player. This is scheduling preference, not an alignment fact.",
    },
    sources: [],
    legalActions,
    legalTargets:
      kind === "vote"
        ? shuffled(
            state.players.filter((p) => p.alive && p.id !== playerId).map((p) => p.id),
            `${salt}:votes`,
          )
        : (legalActions[0]?.targets ?? []),
    journal,
    responseDocket: docket,
    closing,
  };
  const facts: ContextSourceV2[] = [
    {
      ...
      scope: "player",
      privateIndex: 1,
      data: { role: self.role.name, alignment: self.role.alignment },
    },
    { id: "facts:rules", type: "published.rules", day: 0, scope: "public", data: { roleCounts } },
    {
      ...
      day: state.day,
      scope: "public",
      data: { living: packet.players.filter((p) => p.alive).map((p) => p.id) },
    },
  ];
  packet.rules.roles = [
    ...new Map(config.roleDeck.map((r) => [`${r.id}:${r.version}`, r])).values(),
  ]
  ...
  if (config.protocolVersion === "agent_v3_1") {
    ...
    const publicBudget = Math.max(
      600,
      Math.min(2_000, Math.floor(config.deliberation.maxContextTokens * 0.125)),
    );
    ...
    let used = estimatedTokens(
      publicSources
        .filter((source) => structural.has(source.id))
        .map((source) => ({ type: source.type, day: source.day, data: source.data })),
    );
    ...
    for (const source of [...publicSources].reverse()) {
      ...
      shared.add(source.id);
      used += cost;
    }
    all = all.map((source) =>
      shared.has(source.id) ? { ...source, cacheLayer: "public" as const } : source,
    );
  }
  if (config.protocolVersion === "agent_v3_2") {
    ...
    const publicBudget = Math.max(
      1_000,
      Math.min(4_000, Math.floor(config.deliberation.maxContextTokens * 0.375)),
    );
    const digestChars = config.deliberation.digestChars;
    const publicSources = all.filter((source) => source.scope === "public");
    const cost = (source: ContextSourceV2, tier: DeliveryTier) =>
      estimatedTokens({
        type: source.type,
        day: source.day,
        data: tierPayload(source, tier, digestChars, () => "E1"),
      });
    ...
    const floor = publicSources
      .filter(pinned)
      .reduce((sum, source) => sum + cost(source, "full"), 0);
    ...
    const total = () =>
      demotable.reduce(
        (sum, source) => sum + cost(source, tierForDay.get(source.day) ?? "stub"),
        floor,
      );
    ...
    for (const tier of ["digest", "full"] as const)
      for (const day of [...days].reverse()) {
        ...
        if (total() > publicBudget) {
          tierForDay.set(day, previous);
          break;
        }
      }
    applyTiers = () => {
  ...
  const citations = new Set([
    ...journal.beliefs.flatMap((b) => b.sources),
    ...journal.hypotheses.flatMap((h) => h.sources),
    ...(journal.attentionNotes ?? []).flatMap((n) => n.sources),
    ...docket,
    ...requiredSources,
  ]);
  ...
  const recentOwnCommitments = new Set(
    all
      .filter((s) => s.type === "speech.public" && s.data.playerId === playerId)
      .slice(-4)
      .map((s) => s.id),
  );
  const essential = new Set(
    all
      .filter(
        (s) =>
          s.scope !== "public" ||
          ["player.eliminated", "role.revealed", "vote.resolved"].includes(s.type) ||
          recentOwnCommitments.has(s.id) ||
          citations.has(s.id),
      )
      .map((s) => s.id),
  );
  ...
  const materialize = () => {
    packet.sources = [...facts, ...all.filter((s) => selected.has(s.id))];
  };
  materialize();
  // Measure the exact prompt AND source-dependent schema before adding optional history.
  const requestSize = () =>
    requestSizer?.(packet) ?? decisionRequestV2(packet, kind, config.deliberation.mode).tokens;
  ...
  if (essentialSize > config.deliberation.maxContextTokens)
    throw new ContextLimitError(
      config.protocolVersion === "agent_v3_2"
...
export function journalLimitMessage(
  base: PrivateJournalV2,
  maxTokens: number,
  updatedTokens?: number,
): string {
  if (base.text !== undefined)
    return `journal_limit: updated notebook uses ${updatedTokens ?? "too many"} estimated tokens; maximum ${maxTokens}. Summarize the prose faithfully, preserving current reasoning, uncertainty, listening notes, commitments and deception. Size is UTF-8 prose bytes / 3, rounded up.`;
...
export function applyJournalV2(
  base: PrivateJournalV2,
  ...
  for (const op of report.journalPatch) {
    switch (op.op) {
      case "set_decision_brief":
        next.decisionBrief = structuredClone(op.value);
        break;
      case "write_text": {
        const text =
          op.mode === "append"
            ? [journalText(next), op.text].filter(Boolean).join("\n\n")
            : op.text;
        ...
        next.deceptionPlan = null;
        delete next.attentionNotes;
        break;
      }
      case "upsert_belief":
        next.beliefs = [...next.beliefs.filter((b) => b.playerId !== op.value.playerId), op.value];
        break;
      case "upsert_hypothesis":
        next.hypotheses = [...next.hypotheses.filter((h) => h.id !== op.value.id), op.value];
        break;
      case "remove_hypothesis":
        next.hypotheses = next.hypotheses.filter((h) => h.id !== op.id);
        break;
      case "set_strategy":
        next.strategy = op.strategy;
        next.goals = op.goals;
        break;
      case "set_attention":
        next.attentionNotes = op.notes;
        break;
      case "set_questions":
        next.unresolvedQuestions = op.questions;
        break;
      case "set_deception":
        next.deceptionPlan = op.plan;
        break;
    }
  }
  next.version += 1;
  const validated = PrivateJournalV2Schema.parse(next);
  if (journalTokens(validated) > maxTokens)
    throw new Error(journalLimitMessage(validated, maxTokens, journalTokens(validated)));
  return validated;
}
export function validateReport(
  ...
  const cited = [
    ...report.observations,
    ...report.inferences.flatMap((i) => i.sources),
    ...report.journalPatch.flatMap((op) =>
      op.op === "upsert_belief" || op.op === "upsert_hypothesis"
        ? op.value.sources
        : op.op === "set_attention"
          ? op.notes.flatMap((n) => n.sources)
          : [],
    ),
  ];
  const proposal = report.proposal;
  if (proposal.kind === "discussion" && proposal.speech)
    cited.push(
      ...proposal.speech.respondsTo,
      ...proposal.speech.acts.flatMap((a) => (a.sourceId ? [a.sourceId] : [])),
    );
  ...
  for (const op of report.journalPatch)
    if (
      op.op === "set_decision_brief" &&
      (op.value.playerId !== packet.self.id ||
        op.value.evidenceRevision !== packet.rules.journalRevision)
    )
      errors.push("Decision brief must match the acting player and current authorized evidence");
  for (const op of report.journalPatch)
    if (
      op.op === "set_attention" &&
      op.notes.some((n) => !packet.players.some((p) => p.id === n.playerId))
    )
      errors.push("attention note names an unknown player");
  for (const op of report.journalPatch)
    if (op.op === "upsert_belief") {
      if (!packet.players.some((p) => p.id === op.value.playerId))
        errors.push("belief names an unknown player");
      if (
        op.value.basis === "authorized_fact" &&
        !op.value.sources.some((id) =>
          packet.sources.some(
            (s) =>
              s.id === id &&
              ((s.type === "inspection.delivered" && s.data.targetId === op.value.playerId) ||
                (s.type === "authorized.self" &&
                  (op.value.playerId === packet.self.id ||
                    packet.knownAllies.some((ally) => ally.id === op.value.playerId)))),
          ),
        )
      )
        errors.push(
          "authorized_fact beliefs require an applicable private result or own-role/team source; public claims are not certified facts",
        );
    }
  if (!report.alternatives.some((a) => a.id === report.selectedAlternativeId))
    errors.push("selectedAlternativeId must match an alternative");
  if (
    proposal.kind === "night_action" ||
    proposal.kind === "team_point" ||
    proposal.kind === "vote"
  ) {
    const choice = proposal.targets;
    const action =
      proposal.kind === "night_action"
        ? packet.legalActions.find((a) => a.actionId === proposal.actionId)
        : packet.legalActions[0];
    ...
    if (
      choice &&
      (choice.playerIds.some((id) => !allowed.includes(id)) ||
        new Set(choice.playerIds).size !== choice.playerIds.length)
    )
      errors.push("target set must contain unique legal choices");
    if (choice?.mode === "direct" && choice.playerIds.length !== 1)
      errors.push("direct proposal must name exactly one target");
  }
  if (proposal.kind === "discussion") {
    if (
      proposal.speech?.acts.some(
        (a) => a.targetId && !packet.players.some((p) => p.id === a.targetId),
      )
    )
      errors.push("speech act has unknown target");
    if (
      packet.closing &&
      proposal.speech?.acts.some((a) => a.kind === "accusation" || a.kind === "challenge")
    )
      errors.push("closing replies cannot open new formal accusations or challenges");
    if (packet.closing && proposal.speech) {
      if (!proposal.speech.respondsTo.some((id) => packet.responseDocket.includes(id)))
        errors.push("closing speech must reply to the frozen docket");
      if (proposal.speech.respondsTo.some((id) => !packet.responseDocket.includes(id)))
        errors.push("closing speech can only reply to the frozen docket");
      if (proposal.speech.acts.some((a) => a.kind !== "reply"))
        errors.push("closing speech acts must be replies");
    }
    if (
      packet.rules.speakerSelection !== "bid_only" &&
      packet.responseDocket.length &&
      !proposal.speech &&
      !proposal.silenceCase
    )
      errors.push("when accused, a pass must compare a feasible speech strategy in silenceCase");
    if (packet.rules.speakerSelection === "listener_auction") {
      ...
      const expected = packet.players
        .filter((p) => p.alive && p.id !== packet.self.id)
        .map((p) => p.id)
        .sort();
      ...
      if (!intent) errors.push("listener-auction discussion reports require speakerIntent");
        ...
        if (
          ...
          errors.push("willingnessToListen must rate every other living player exactly once");
        if (!intent.wantsToSpeak && intent.urge !== 0)
          errors.push("a player who does not want to speak must bid zero urge");
        if (intent.wantsToSpeak !== Boolean(proposal.speech))
          errors.push("wantsToSpeak must match whether a speech proposal is supplied");
      }
    }
  }
  try {
    applyJournalV2(packet.journal, report, maxJournalTokens);
  } catch (error) {
