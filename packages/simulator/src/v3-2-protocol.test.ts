...
const role = (id: string) =>
  id === "doctor"
    ...
    : structuredClone(STARTER_ROLES.find((c) => c.id === id)!);
  ...
  for (let day = 1; day <= days; day += 1) {
    for (let index = 0; index < perDay; index += 1) {
      ...
      events.push({
        ...
        audienceIds: [],
        payload: {
          playerId: `p${(index % 8) + 1}`,
          playerName: `Player ${(index % 8) + 1}`,
          text: `Day ${day} statement ${index}. `.repeat(12),
          acts: [],
          respondsTo: [],
          closing: false,
        },
...
const sizer = (candidate: PlayerContextV2) =>
  decisionRequestV32(candidate, task, true, null, null).tokens;
const build = (
  events: GameEventV1[],
  playerId: string,
...
describe("V3.2 delivery ladder", () => {
  ...
  it("keeps the public layer byte-identical across seats even when the ledger is demoted", () => {
    const events = longGame();
    const shared = state();
    const p1 = decisionRequestV32(
      build(events, "p1", emptyJournalV2(), shared),
      task,
      true,
      null,
      null,
    );
    const p2 = decisionRequestV32(
      build(events, "p2", emptyJournalV2(), shared),
      task,
      true,
      null,
      null,
  ...
  it("drops phase transitions from the citable ledger, since they cannot support a claim", () => {
    const phase: GameEventV1 = {
      ...
      visibility: "public",
      audienceIds: [],
      payload: { from: "night_resolution", to: "day_discussion" },
      createdAt: "2026-01-01T00:00:00.000Z",
    };
  ...
  it("names the exact seats to rate, the field that halted the V3.1 pilot", () => {
    ...
    const request = decisionRequestV32(
      build(events, "p1", emptyJournalV2(), shared),
      task,
      true,
      null,
  ...
  it("keeps a closed night's team pointing citable, with its content withheld", () => {
    ...
    const wolves = shared.players
      .filter((player) => player.role.alignment === "werewolf")
    ...
    const point: GameEventV1 = {
      ...
      visibility: "team",
      audienceIds: wolves,
      payload: { playerId: wolves[0], targetId: "p4", round: 0, blind: true },
      createdAt: "2026-01-01T00:00:00.000Z",
    };
    const journal: PrivateJournalV2 = {
      ...emptyJournalV2(),
      hypotheses: [
        { id: "h1", statement: "The pack opened on Dax", confidence: 0.8, sources: ["point-1"] },
      ],
    };
    ...
    const cited = JSON.parse(request.prompt.privateInput).AUTHORIZED_PRIVATE_STATE.journal
      .hypotheses[0].sources;
