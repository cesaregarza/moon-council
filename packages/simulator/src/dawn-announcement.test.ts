...
function elimination(day: number, phase: string, playerId: string): GameEventV1 {
  return {
    ...
    id: `${phase}-${day}-${playerId}`,
    gameId: "game",
    sequence: day * 10,
    type: "player.eliminated",
    phase: phase as GameEventV1["phase"],
    day,
    visibility: "public",
    audienceIds: [],
    payload: {
      ...
      playerName: playerId,
      roleName: "Villager",
      cause: phase === "day_vote" ? "vote" : "night",
    },
...
describe("dawn announcement", () => {
  ...
  it("finds a night-first kill under the same day", () => {
    const events = [elimination(4, "night_resolution", "p3")];
    expect(overnightEliminations(events, 4, "night_first").map((e) => e.payload.playerId)).toEqual([
      "p3",
    ]);
  ...
  it("reports a quiet night as empty", () => {
    expect(
      overnightEliminations([elimination(2, "night_resolution", "p7")], 5, "day_first"),
    ).toEqual([]);
  });
});
