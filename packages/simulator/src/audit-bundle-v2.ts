...
const BundleSchema = z.object({
  ...
  events:z.array(z.object({id:z.string(),sequence:z.number().optional(),day:z.number(),phase:z.string(),type:z.string(),payload:z.record(z.string(),z.unknown())})),
});

/** Read an exported artifact only. Never query the provider or modify game state. */
export function inspectResearchBundle(value:unknown,options:{matches?:string[];day?:number;limit?:number;offset?:number;eventTypes?:string[];player?:string}={}) {
  const bundle=BundleSchema.parse(value);
  const {matches=[],day,limit=30,offset=0,eventTypes=[],player}=options;
  if(eventTypes.some(term=>!term.trim()) || player==="" || matches.some(term=>!term.trim()) || !Number.isInteger(limit) || limit<1 || limit>200 || !Number.isInteger(offset) || offset<0 || day!==undefined && (!Number.isInteger(day)||day<0)) throw new Error("Invalid inspection filters");
  ...
  const records=bundle.events.flatMap(event=>{
    ...
    if(!matches.length && !eventTypes.length || matches.length && !matches.some(term=>search.includes(term.toLocaleLowerCase("en-US"))))return [];
    return [{eventId:event.id,sequence:event.sequence,day:event.day,phase:event.phase,type:event.type,playerId:p.playerId,decisionId:p.decisionId,content}];
  });
...
export function journalSnapshotMarkdown(value:unknown,player?:string):string {
  ...
  const players=z.array(z.object({id:z.string(),name:z.string(),role:z.object({name:z.string()}).optional()})).parse(roster??[]);
  const latest=new Map<string,{event:typeof events[number];journal:z.infer<typeof PrivateJournalV2Schema>;order:number}>();
  ...
  const entries=[...latest].sort(([a],[b])=>a.localeCompare(b)).map(([id,{event,journal}])=>{
