      ...
      answers: {
        target: {
          ...
          probabilities: Object.fromEntries(
            Object.keys(options).map((key) => [key, Number(key === handle)]),
          ),
        },
      },
    });
...
function recorder(onCall?: () => void) {
  ...
  return {
    requests,
    provider: {
      decide: async <T>(request: DecisionRequest<T>) => {
        requests.push(request);
        onCall?.();
...
describe("v4 review regressions", () => {
  it.each(["jev", "llm"] as const)(
    "keeps the rules cache stable across seats, speech and private results (%s)",
    (mode) => {
      const repo = repository();
      const config = actorConfig();
      config.decisionEngine.mode = mode;
      const game = repo.createGame(config),
        runner = new V2GameOrchestrator(repo, new FakeDecisionProvider());
      runner.initialize(game.id);
      const frames = [];
      for (const event of [undefined, "speech.public", "inspection.delivered"]) {
        if (event)
          repo.appendEvent(game.id, {
            type: event,
            phase: "day_discussion",
            day: 1,
            visibility: event === "speech.public" ? "public" : "player",
            audienceIds: ["p1"],
            payload: {
              playerId: "p2",
              text: "A fresh accusation",
              actorId: "p1",
              targetId: "p2",
              result: "werewolf",
            },
        ...
        for (const playerId of ["p1", "p2"]) {
          const events = repo.listEvents(game.id),
            state = reduceGame(game.id, events);
          ...
          const prepared = decisionRequestV31(
            packet,
            {
              type: "journal_update",
          ...
          const wire = openAIRequest({
            kind: "decision_v3_1",
            model: "gpt-6-luna",
            gameId: game.id,
            schemaName: "journal_update",
          ...
          frames.push({
            rules: JSON.stringify(Array.isArray(wire.input) ? wire.input.slice(0, 2) : wire.input),
            key: wire.prompt_cache_key,
            revision: packet.rules.journalRevision,
          });
        }
      }
  ...
  it("rejects undersized actor journals and reserves prose space even for worst-case escaping", () => {
    for (const limit of [200, 1200, 8500, 15999]) {
      const config = actorConfig();
      config.deliberation.maxJournalTokens = limit;
      expect(() => GameConfigV2Schema.parse(config)).toThrow(
        "maximum current brief and prose headroom",
      );
    ...
    const journal = {
      ...
      decisionBrief: {
        ...
        action: "\u0000".repeat(4000),
        attention: "\u0000".repeat(3000),
      },
    };
    expect(journalTokens(journal)).toBeLessThan(16000);
  });
  it.each(["inspection.delivered", "vote.resolved"])(
    "reflects on %s alone and carries the new evidence",
    async (type) => {
      const repo = repository(),
        llm = recorder(),
        { game, runner } = staged(repo, llm.provider);
      await runner.runGameStep(game.id);
      expect(llm.requests).toHaveLength(8);
      llm.requests.length = 0;
      const event = repo.appendEvent(game.id, {
        type,
        phase: "day_vote",
        day: 2,
        visibility: type === "inspection.delivered" ? "player" : "public",
        audienceIds: ["p1"],
        payload:
          type === "inspection.delivered"
            ? { actorId: "p1", targetId: "p2", result: "werewolf" }
            : { ballots: [{ voterId: "p1", targetId: null }], targetId: null, tally: {}, tied: [] },
      });
      await runner.runGameStep(game.id);
      ...
      expect(
        repo
          .listEvents(game.id)
          .filter((e) => e.type === "journal.refreshed")
          .every(
            (e) => typeof e.payload.reviewedThroughSequence === "number" && !e.payload.sourceIds,
          ),
      ).toBe(true);
    },
  );
  it("pauses during reflection without falling into a missing-brief error and resumes the pending work", async () => {
    ...
    const llm = recorder(() => {
      if (!paused) {
        paused = true;
        repo.updateGame(gameId, { status: "paused" });
      }
    });
    const { game, runner } = staged(repo, llm.provider);
    gameId = game.id;
    await runner.runGameStep(game.id);
  ...
  it("keeps sealed votes and another player's private inspection out of every v4 peer context", () => {
    const repo = repository(),
      fixture = createActorHoldout(repo, "uncertain-last-wolf");
    ...
    repo.appendEvent(game.id, {
      ...
      payload: { vote: { voterId: actor.id, targetId: fixture.target.id } },
    });
    const events = repo.listEvents(game.id),
      state = reduceGame(game.id, events);
    for (const player of state.players.filter((player) => player.id !== actor.id)) {
      ...
      expect(
        peer.sources.some(
          (source) => source.type === "inspection.delivered" || source.type === "vote.cast",
        ),
      ).toBe(false);
  ...
  it("keeps a wolf's deception distinct from private pack knowledge", () => {
    const repo = repository(),
      fixture = createActorHoldout(repo, "cleared-target-preference"),
      events = repo.listEvents(fixture.game.id),
      state = reduceGame(fixture.game.id, events);
    ...
    packet.journal.decisionBrief = {
      playerId: wolf.id,
      evidenceRevision: String(packet.rules.journalRevision),
      action:
        "I will falsely claim Seer to frame a villager. I know my pack ally is a wolf; my public story is a bluff.",
      attention: "Hear the real Seer.",
    };
    expect(
      actorBriefing({ packet, playerId: wolf.id, taskType: "vote_choice" }).currentReasoning,
    ).toContain("bluff");
  ...
  it("records exactly one reconsideration for the incident shape with doubting public speeches", async () => {
    const repo = repository(),
      fixture = createActorHoldout(repo, "doubt-dominated"),
      options = holdoutVote(repo, fixture);
    const expected = String.fromCharCode(
      97 + fixture.packet.legalTargets.indexOf(fixture.target.id),
    );
    ...
    expect(reduceGame(fixture.game.id, repo.listEvents(fixture.game.id)).votes).toContainEqual({
      voterId: fixture.actor.id,
      targetId: fixture.target.id,
    });
  });
  it("explains a pending reconsideration at maxCalls=1 and resumes exactly that review", async () => {
    const repo = repository(),
      fixture = createActorHoldout(repo, "uncertain-last-wolf", "journal_v4", 1),
      options = holdoutVote(repo, fixture);
    const expected = String.fromCharCode(
        97 + fixture.packet.legalTargets.indexOf(fixture.target.id),
      ),
      jev = votes(["abstain", expected]);
  ...
  it("acknowledges and commits only the final recorded ballot, stays paused, and is idempotent", async () => {
    const repo = repository(),
      ...
      jev = votes(["abstain"]);
    await expect(
      new DecisionExecutorV2(repo, new FakeDecisionProvider(), jev.provider).execute(options),
    ).rejects.toThrow("one reconsideration");
    repo.updateGame(fixture.game.id, { status: "paused" });
    expect(
      acknowledgeSemanticAnomaly(
        repo,
        fixture.game.id,
        options.opportunity.id,
        "Accept this recorded anomaly for the experiment",
      ),
    ).toMatchObject({ status: "committed", gameStatus: "paused", alreadyAcknowledged: false });
    expect(
      acknowledgeSemanticAnomaly(repo, fixture.game.id, options.opportunity.id, "Repeat"),
    ...
    expect(reduceGame(fixture.game.id, events).votes).toContainEqual({
      voterId: fixture.actor.id,
      targetId: null,
    });
    expect(events.filter((e) => e.type === "decision.semantic_acknowledged")).toHaveLength(1);
    expect(
      JSON.stringify(observerPayload(repo, repo.getGame(fixture.game.id)!, { kind: "public" })),
    ).not.toContain("Accept this recorded anomaly");
  });
  it("refuses acknowledgment after relevant evidence changes", async () => {
    ...
    repo.updateGame(fixture.game.id, { status: "paused" });
    repo.appendEvent(fixture.game.id, {
      type: "speech.public",
      day: 2,
      phase: "day_vote",
      visibility: "public",
      payload: { playerId: "p2", text: "New evidence", acts: [], respondsTo: [] },
    });
    expect(() =>
      acknowledgeSemanticAnomaly(repo, fixture.game.id, options.opportunity.id, "Reviewed"),
    ).toThrow("stale");
  ...
  it("rolls back the semantic assessment together with a failed checkpoint write", async () => {
    ...
    repo.connection.sqlite.exec(
      "CREATE TRIGGER fail_semantic BEFORE INSERT ON events WHEN NEW.type = 'decision.jev_stage' BEGIN SELECT RAISE(ABORT, 'injected checkpoint failure'); END",
    );
    await expect(
      new DecisionExecutorV2(repo, new FakeDecisionProvider(), jev.provider).execute(options),
    ).rejects.toThrow();
    const op = fixture.store.get<DecisionOpportunityV1>(
      fixture.game.id,
      `decision:${options.opportunity.id}`,
    )!;
    expect(
      repo.listEvents(fixture.game.id).filter((e) => e.type === "decision.semantic_assessed"),
    ).toHaveLength(0);
