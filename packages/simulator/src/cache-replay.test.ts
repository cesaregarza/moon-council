...
function fixture() {
  const directory = mkdtempSync(join(tmpdir(), "cache-replay-test-"));
  directories.push(directory);
  const path = join(directory, "test.db"),
    db = new Database(path);
  db.exec(
    "CREATE TABLE provider_attempts(game_id TEXT,id TEXT,value_json TEXT); CREATE TABLE agent_records(game_id TEXT,record_key TEXT,value_json TEXT)",
  );
  const roleIds = [
    "werewolf",
    "werewolf",
  ...
  const config = GameConfigV2Schema.parse({
    ...
    modelSettings: Object.fromEntries(
      seats.map((s) => [s.id, { provider: "codex", model: "gpt-6-luna" }]),
    ),
  ...
  const state = createGameState("game", config),
    packet = buildContextV2(state, [], "p0", emptyJournalV2(), "fixture", "discussion");
  const prepared = decisionRequestV31(
    packet,
    { type: "journal_update", sourceIds: [], revision: "one" },
    true,
    null,
    null,
  );
  const attempt = {
    ...
    request: { ...prepared.prompt, schema: prepared.jsonSchema },
  };
  db.prepare("INSERT INTO provider_attempts VALUES(?,?,?)").run(
    "game",
    "attempt",
    JSON.stringify(attempt),
  );
  db.prepare("INSERT INTO agent_records VALUES(?,?,?)").run(
    "game",
    "decision:decision",
    JSON.stringify({ id: "decision", packet }),
  );
  db.close();
  return { path, attempt };
}
...
describe("recorded cache replay", () => {
  it("keeps the exact source prompt/model/effort, uses the stable API envelope, and leaves the source unchanged", () => {
    ...
    const count = before.prepare("SELECT count(*) AS n FROM provider_attempts").get();
    before.close();
    ...
    expect(entry!.request).toMatchObject({
      ...
      apiResponseFormat: { name: "moon_council_reflection_speech_v1" },
    });
    const after = new Database(path, { readonly: true });
    expect(after.prepare("SELECT count(*) AS n FROM provider_attempts").get()).toEqual(count);
    after.close();
  ...
  it("rejects archive/schema drift instead of silently changing the replay", () => {
    ...
    db.prepare("UPDATE provider_attempts SET value_json=?").run(
      JSON.stringify({
        ...attempt,
        request: { ...attempt.request, schema: { type: "object", properties: {} } },
      }),
    );
    db.close();
  ...
  it("distinguishes shared messages from incompatible schema settings", () => {
    const a = {
      input: [{ role: "developer", content: "same" }],
      text: { name: "journal" },
      reasoning: { effort: "xhigh" },
    };
    expect(prefixComparison(a, { ...a, text: { name: "speech" } })).toMatchObject({
      equalLeadingMessages: 1,
      schemaEqual: false,
      reasoningEqual: true,
    });
  });
  it("accounts for read/write/input categories once, and does not turn missing usage into zero cost", () => {
    ...
    expect(cacheReceiptSummary([{ usage, latencyMs: 10 }])).toMatchObject({
      cacheReadFraction: 0.5,
      inputRateEquivalentTokens: 600,
    });
