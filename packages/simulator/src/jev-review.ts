      ...
      .map(([key, answer]) => {
        ...
        return [
          key,
          {
            ...
            top: Object.entries(probabilities)
              .sort((a, b) => b[1] - a[1])
              .slice(0, 3),
          },
        ];
...
export function reviewJevDecisions(
  value: unknown,
  options: {
    day?: number;
    player?: string;
  ...
  if (options.day !== undefined && (!Number.isInteger(options.day) || options.day < 1))
    throw new Error("Invalid review day");
  if (options.player === "" || options.decision === "" || options.task === "")
    throw new Error("Empty review filter");
  const attempts = [...new Map(bundle.attempts.map((a) => [a.id, a])).values()];
  const records = bundle.decisions
    .flatMap(({ opportunity: op }) => {
      if (
        (options.day !== undefined && options.day !== op.day) ||
        (options.player && options.player !== op.playerId) ||
        (options.decision && options.decision !== op.id) ||
        (options.task && options.task !== op.taskType)
      )
        return [];
      ...
      const reasoning = calls
        .filter((a) => a.promptVersion === "jev_reasoning_v1" && a.status === "valid")
        .map((a) => ({ attemptId: a.id, response: jsonObject(a.response) }));
      const sequence =
        bundle.events.find((e) => e.type === "decision.opened" && e.payload.decisionId === op.id)
          ?.sequence ?? null;
      return [
        {
          sequence,
          ...
          name: op.packet.self.name,
          task: op.taskType,
          workflow:
            first.promptVersion === "jev_actions_v4"
              ? "journal_v4"
              : first.promptVersion === "jev_actions_v3"
                ? "journal_v3"
                : first.promptVersion === "jev_actions_v2"
                  ? "journal_v2"
                  : "legacy_v1",
          reasoningAvailable: state.reasoningAvailable,
          ...
          reasoning,
          semanticAssessments: bundle.events
            .filter(
              (e) => e.type === "decision.semantic_assessed" && e.payload.decisionId === op.id,
            )
            .map((e) => e.payload),
          submission: op.bestSubmission,
          invalidAttempts: calls
            .filter((a) => a.status === "invalid")
            .map((a) => ({ attemptId: a.id, provider: a.provider, error: a.error })),
          ...(options.evidence
            ? {
                evidence: {
                  attemptId: first.id,
                  questions,
                  ...(state.currentReasoning !== undefined
                    ? {
                  ...
                  ...(state.journal !== undefined
                    ? { journal: state.journal, situation: state.situation, facts: state.facts }
                    : {}),
                  public: state.public,
                  private: state.private,
                  task: state.task,
                  plans: questions.plan?.criteria ?? null,
                },
              }
            : {}),
        },
  ...
  return {
    ...
    journalDecisions: records.filter((r) => r.workflow !== "legacy_v1").length,
    reasoned: records.filter((r) => r.reasoned).length,
    directWithGate: records.filter((r) => !r.reasoned && r.reasoningAvailable === true).length,
    directWithoutGate: records.filter((r) => !r.reasoned && r.reasoningAvailable === false).length,
    records,
  };
}

/** Export one recorded CLI input unchanged; never call a model or load credentials. */
export function extractJevInput(
  value: unknown,
  options: { decision: string; attempt?: string },
): string {
  if (!options.decision.trim() || (options.attempt !== undefined && !options.attempt.trim()))
    throw new Error("Empty Jev input selector");
  const bundle = BundleSchema.pick({ attempts: true }).parse(value);
  const calls = [...new Map(bundle.attempts.map((a) => [a.id, a])).values()].filter(
    (a) =>
      a.provider === "jev" &&
      a.decisionId === options.decision &&
      (options.attempt === undefined ? a.status === "valid" : a.id === options.attempt),
  );
  if (calls.length !== 1)
    throw new Error(
      `Expected one Jev input, found ${calls.length}; use --attempt to select an exact recorded attempt`,
    );
