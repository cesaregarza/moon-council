...
export const HOLDOUT_LABELS = [
  "uncertain-last-wolf",
  "cleared-target-preference",
  "no-preference",
  "doubt-dominated",
] as const;
...
export function createActorHoldout(
  repository: LabRepository,
  label: HoldoutLabel,
  workflow: "journal_v3" | "journal_v4" = "journal_v4",
  maxCalls = 3,
) {
  ...
  if (endgame)
    inputs.push({
      ...
      visibility: "public",
      payload: { playerId: wolves[1]!.id, playerName: wolves[1]!.name, cause: "vote" },
    });
  ...
  inputs.push({
    ...
    payload: {
      ...
      result: inspected.role.alignment,
    },
  });
  const doubts = [
  ...
  for (const [index, text] of doubts.entries())
    inputs.push({
      type: "speech.public",
      day: 2,
      ...
      visibility: "public",
      payload: {
        playerId: initial.players.filter(
          (player) => player.id !== actor.id && player.id !== wolves[1]!.id,
        )[index]!.id,
        text,
        acts: [],
        respondsTo: [],
  ...
  inputs.push({
    ...
    phase: "day_vote",
    visibility: "public",
    payload: { from: "day_discussion", to: "day_vote" },
  });
  ...
  packet.journal = {
    ...packet.journal,
    text: `Day 1: I kept my identity private. Day 2: ${action}\nListening: ${attention}`,
    decisionBrief: { playerId: actor.id, evidenceRevision: revision, action, attention },
  };
  ...
  return {
    ...
    inputs,
    rubric: endgame
      ? "Vote for the privately verified last wolf despite an uncertain or neutral preference."
      : "Do not execute the actor-cleared villager merely because the brief prefers it; no unique alternative is required.",
...
export function handleTarget(packet: PlayerContextV2, handle: string | undefined) {
  return handle === "abstain"
    ? null
    : handle?.length === 1
      ? packet.legalTargets[handle.charCodeAt(0) - 97]
      : undefined;
}
