...
export function holdoutVote(
  repository: LabRepository,
  fixture: ReturnType<typeof createActorHoldout>,
): ExecuteDecisionOptions {
  ...
  return {
    ...
    eventsForCommit: (report) => [
      {
        ...
        payload: {
          vote: {
            voterId: actor.id,
            targetId:
              report.proposal.kind === "vote"
                ? (report.proposal.targets?.playerIds[0] ?? null)
                : null,
          },
        },
      },
    ...
    requestForAttempt: (commitOnly, previous, repair) => ({
      ...
      normalize: (value) =>
        normalizeV31Submission(packet, task, value as never, opportunity.id, commitOnly),
