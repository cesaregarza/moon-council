...
const evaluation = JSON.stringify({
  answers: {
    ...
    target: { type: "choice", choice: "a", confidence: 0.2, probabilities: { a: 0.51, b: 0.49 } },
  },
});
const request = (available: boolean) => ({
  input: JSON.stringify({
    state: {
      reasoningAvailable: available,
      public: { evidence: "Zoë spoke" },
      private: { journal: "private fixture" },
      legalChoices: { a: "Zoë", b: "René" },
    },
    questions: {},
  }),
});
...
const opportunity = {
  ...
  packet: { self: { name: "Zoë" } },
  bestSubmission: { mode: "direct", choiceHandles: ["a"] },
};
const bundle = {
  attempts: [attempt],
  decisions: [{ opportunity }],
  events: [{ sequence: 12, type: "decision.opened", payload: { decisionId: "d" } }],
};

describe("offline Jev review", () => {
  it("extracts prose-first evidence and choices without exposing the journal by default", () => {
    const state = {
      journal: "Hear Zoë defend herself before voting.",
      situation: "Day 2",
      facts: "Verified result",
      task: { type: "vote_choice", rules: "Plurality" },
    };
    ...
    const data = {
      ...
      attempts: [
        {
          ...attempt,
          promptVersion: "jev_actions_v3",
          request: {
            input: JSON.stringify({
              state,
              questions: { target: { type: "choice", criteria: choices } },
            }),
          },
        },
      ],
    ...
    expect(brief).toMatchObject({
      journalDecisions: 1,
      records: [{ workflow: "journal_v3", legalChoices: choices }],
    });
  ...
  it("distinguishes a declined gate from unavailable reasoning and excludes invalid retries", () => {
    ...
    expect(reviewed.records[0]).toMatchObject({
      ...
      invalidAttempts: [{ attemptId: "bad" }],
    });
    expect(
      reviewJevDecisions({ ...bundle, attempts: [{ ...attempt, request: request(false) }] })
        .directWithoutGate,
    ).toBe(1);
  });
  it("preserves explicit advice, final choice and Unicode; evidence requires an explicit option", () => {
    const analysis = {
      ...
      response: JSON.stringify({ rationale: "Compare Zoë and René." }),
    };
    const final = {
      ...attempt,
      id: "c",
      response: JSON.stringify({
        answers: { target: { type: "choice", choice: "b", probabilities: { a: 0.1, b: 0.9 } } },
      }),
    };
    ...
    expect(result).toMatchObject({
      ...
      records: [
        {
          name: "Zoë",
          final: { target: { choice: "b" } },
          reasoning: [{ response: { rationale: "Compare Zoë and René." } }],
        },
      ],
    });
    expect(result.records[0]).not.toHaveProperty("evidence");
    expect(reviewJevDecisions(data, { evidence: true }).records[0]?.evidence?.private).toEqual({
      journal: "private fixture",
    });
  });
  it("filters without changing source IDs and rejects malformed input", () => {
    ...
    expect(
      reviewJevDecisions(bundle, { task: "vote_choice", evidence: true }).records[0]?.evidence,
    ).toMatchObject({ attemptId: "a", questions: {} });
...
describe("exact offline Jev input export", () => {
  it("preserves every byte, including Unicode, whitespace and private context", () => {
    const input =
      ' {"model":"fixture", "state":{"note":"Zoë\\nRené","gameReference":{}},"questions":{}}\n';
    expect(
      extractJevInput({ attempts: [{ ...attempt, request: { input } }] }, { decision: "d" }),
    ).toBe(input);
    expect(extractJevInput({ attempts: [attempt, attempt] }, { decision: "d" })).toBe(
  ...
  it("requires an explicit attempt when selection is ambiguous and can select failed inputs", () => {
    ...
    expect(() => extractJevInput({ attempts: [attempt, second] }, { decision: "d" })).toThrow(
      "found 2",
    );
    expect(
      extractJevInput(
        { attempts: [attempt, { ...second, status: "invalid" }] },
        { decision: "d", attempt: "second" },
      ),
    ).toBe(second.request.input);
    expect(() => extractJevInput({ attempts: [attempt] }, { decision: "missing" })).toThrow(
      "found 0",
    );
    expect(() => extractJevInput({ attempts: [attempt] }, { decision: " " })).toThrow("Empty");
    expect(() =>
      extractJevInput(
        { attempts: [{ ...attempt, request: { input: "broken" } }] },
        { decision: "d" },
      ),
    ).toThrow("JSON object");
  });
