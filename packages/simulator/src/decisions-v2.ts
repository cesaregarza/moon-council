...
import { providerJsonSchema, type DecisionOpportunityV1, type DecisionReportV2, type GameConfigV2 } from "@werewolf/contracts";
...
import { applyJournalV2, ContextLimitError, estimatedTokens, journalLimitMessage, validateReport } from "./context-v2";

export function preparedTokenEstimate(prepared: {tokens:number;jsonSchema:unknown;apiResponseFormat?:ApiResponseFormat}, api = false): number {
  const format=api?prepared.apiResponseFormat:undefined;
  return prepared.tokens+(format?estimatedTokens(providerJsonSchema(format.schema))+estimatedTokens(format.instructions)-estimatedTokens(prepared.jsonSchema):0);
}
...
export function usageTotals(store: DecisionStore, gameId: string) {
  ...
  return { calls: attempts.length, knownTokens: attempts.reduce((n,a) => n + (a.usage.totalTokens ?? 0), 0), unknownAttempts: attempts.filter(a => a.usage.totalTokens === null).length,
    admissionTokens: attempts.reduce((n,a) => n + (a.usage.totalTokens ?? estimatedTokens(a.request) + (a.maxOutputTokens ?? 600)), 0) };
}
export function assertV2Budget(store: DecisionStore, gameId: string, config: GameConfigV2, additionalActiveMs = 0, admitCall = true): void {
  ...
  if (config.maxTotalTokens !== null && usage.admissionTokens >= config.maxTotalTokens) throw new V2BudgetError("total-token admission threshold reached (unknown usage conservatively reserved; in-flight overshoot is possible)");
...
export class DecisionExecutorV2 {
  ...
  constructor(private repository: LabRepository, private provider: DecisionProvider, private jevProvider: DecisionProvider = new AskJevProvider()) { this.store = new DecisionStore(repository); }
  ...
  private async executeEpisode(options: ExecuteDecisionOptions, setSessionKey:(key:string)=>void): Promise<boolean> {
    ...
    let feedback: string | undefined = op.recovery > 0 ? prior.findLast(attempt => attempt.status === "invalid")?.error ?? undefined : undefined;
    if (feedback?.startsWith("journal_limit:")) feedback = journalLimitMessage(op.packet.journal, policy.maxJournalTokens);
    ...
    const commit = () => {
      ...
      const journal = op.journalCompaction?.result ?? applyJournalV2(op.packet.journal, op.best, policy.maxJournalTokens);
    ...
    const firstIndex = usesJev(op, config) ? prior.filter(attempt => attempt.recovery === op.recovery).length : 0;
    for (let index = firstIndex; index < maxAttempts; index += 1) {
      ...
      const apiResponseFormat = settings.provider === "openai" && stage?.provider !== "jev" && "apiResponseFormat" in prepared ? prepared.apiResponseFormat : undefined;
      ...
      const reasoningEffort=compacting && settings.provider === "openai" ? "low" : stage?.provider === "jev" ? "none" : stage ? settings.reasoningEffort : options.reasoningEffort??settings.reasoningEffort;
      ...
      const releaseOutputCap=stage?.provider!=="jev" && settings.provider==="openai" && config.maxTotalTokens===null
        && (compacting || feedback?.startsWith("OpenAI response incomplete: max_output_tokens"));
      const maxOutputTokens = releaseOutputCap ? null
        : stage?.provider !== "jev" && settings.provider === "openai" ? config.safety.maxOutputTokens : Math.min(config.safety.maxOutputTokens,prepared.maxOutputTokens);
      ...
      const timeoutMs = Math.min(releaseOutputCap && !compacting ? policy.episodeTimeoutMs : policy.requestTimeoutMs, policy.episodeTimeoutMs - (Date.now() - start));
      const attempt = this.store.beginAttempt(op, { maxOutputTokens, timeoutMs, model: callModel, provider: stage?.provider === "jev" ? "jev" : settings.provider, reasoningEffort, optional, request: { ...prompt, schema: jsonSchema },promptVersion:prepared.promptVersion,schemaVersion:prepared.schemaVersion });
      ...
      try {
        ...
        const call = callProvider.decide({ gameId: op.gameId, kind: options.requestForAttempt?providerKind:"decision_v2", playerId: op.playerId, model: callModel, reasoningEffort,
          apiResponseFormat, contextV2: op.packet, proposalKind: op.kind, commitOnly, sessionKey, preparedPrompt: prompt, schemaName: prepared.schemaName, schema,
          maxOutputTokens, signal: controller.signal, timeoutMs, onUsage: recordUsage,onRawResponse:recordResponse,
          onProviderRequest: body => { attempt.wireRequest = body; this.store.updateAttempt(attempt); },
          onProviderMetadata: metadata => { attempt.providerMetadata = metadata; this.store.updateAttempt(attempt); } });
        ...
        const submissionErrors="validateSubmission" in basePrepared?basePrepared.validateSubmission?.(submission)??[]:[];
        ...
        const validation=validatePreparedReport(report,op.packet,policy.maxJournalTokens,Boolean(options.requestForAttempt)&&op.taskType!=="journal_update");
        ...
        if (!compacting && op.taskType==="journal_update" && errors.length>0 && errors.every(error=>error.startsWith("journal_limit:"))) {
          const candidate=applyJournalV2(op.packet.journal,report,Infinity);
          op.journalCompaction={candidate,sourceReport:report,sourceSubmission:submission,sourceAttemptId:attempt.id};
          op.status="open";attempt.status="valid";
          this.store.atomic(()=>{
            ...
            this.log(op,"decision.journal_compaction_requested",{attemptId:attempt.id,beforeTokens:journalTokens(candidate),maximumTokens:policy.maxJournalTokens});
          });
          feedback=undefined;optional=false;repairUsed=false;
          continue;
        }
        if (errors.length) {
        ...
        if (checkpoint) {
          ...
          this.store.atomic(() => {
            ...
            if (current?.recovery !== op.recovery || current?.status === "committed") throw new DecisionPausedError("execution episode superseded");
            this.store.updateAttempt(attempt); this.store.save(op);
            this.log(op, "decision.jev_stage", { attemptId: attempt.id, stage: checkpoint.stage, checkpoint });
          });
        ...
        if (compacting) op.journalCompaction={...op.journalCompaction!,result:materializeCompactedJournal(op.journalCompaction!,submission),attemptId:attempt.id};
        op.best = report;
        op.bestSubmission=compacting?op.journalCompaction!.sourceSubmission:options.requestForAttempt?structuredClone(submission):undefined;
  ...
  private allowJevReasoning(op: DecisionOpportunityV1, config: GameConfigV2, remaining: number, mandatory: number, start: number): boolean {
    ...
    const allowance = op.phase.startsWith("night") ? config.deliberation.optionalNightCalls : config.deliberation.optionalDayCalls;
    ...
    return usage.calls + reservedCalls <= config.safety.maxModelCalls
      && (config.maxTotalTokens === null || usage.admissionTokens + reservedCalls * (estimatedTokens(op.packet) + config.safety.maxOutputTokens) < config.maxTotalTokens)
  ...
  private continuationVerdict(op: DecisionOpportunityV1, config: GameConfigV2, index: number, max: number, mandatory: number, start: number): string {
    ...
    const usage = usageTotals(this.store, op.gameId);
    if (usage.calls + mandatory + 1 >= config.safety.maxModelCalls || (config.maxTotalTokens !== null && usage.admissionTokens + (mandatory + 1) * (estimatedTokens(op.packet) + config.safety.maxOutputTokens) >= config.maxTotalTokens)) return "denied_mandatory_reserve";
