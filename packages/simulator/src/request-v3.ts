...
export type V3Submission = DiscussionBidV3 | ListenerBidV3 | SpeechSubmissionV3 | TargetChoiceSubmissionV3;

interface HandleMap { aliases:string[]; toAlias:Map<string,string>; toCanonical:Map<string,string> }
function evidenceMap(packet:PlayerContextV2):HandleMap {
  const aliases=packet.sources.map((_source,index)=>`e${index+1}`);
  return {aliases,toAlias:new Map(packet.sources.map((source,index)=>[source.id,aliases[index]!])),toCanonical:new Map(packet.sources.map((source,index)=>[aliases[index]!,source.id]))};
}
export function canonicalizeV3Plan(packet:PlayerContextV2,plan:DiscussionPlanV3):DiscussionPlanV3 {
...
function memorySchema(packet:PlayerContextV2,evidence:typeof EvidenceHandle):z.ZodType<MemorySuggestionsV3> {
...
export function v3SubmissionSchema(packet:PlayerContextV2,task:V3TaskSpec):z.ZodType<V3Submission> {
  ...
  const otherLiving=packet.players.filter(player=>player.alive&&player.id!==packet.self.id).map(player=>player.id).sort();
  const listen=z.strictObject(Object.fromEntries(otherLiving.map(id=>[id,z.number().min(0).max(1)])));
  if(task.type==="discussion_bid") {
    const target=asEnum(packet.players.filter(player=>player.alive).map(player=>player.id).sort());
    const plan=z.strictObject({kind:z.enum(["accusation","challenge","role_claim","result_claim","reply"]),targetId:target.nullable(),respondsTo:refs,point:Brief});
    return z.strictObject({urge:z.number().min(0).max(1),ready:z.boolean(),plan:plan.nullable(),listen,memory,rationale:Rationale}) as z.ZodType<V3Submission>;
  }
  if(task.type==="discussion_listen") return z.strictObject({ready:z.boolean(),listen,memory,rationale:Rationale}) as z.ZodType<V3Submission>;
  if(task.type==="discussion_speech"||task.type==="closing_response") {
    ...
    const act=z.strictObject({kind:kinds,targetId:target.nullable(),claim:Brief,evidence:evidence.nullable()});
    return z.strictObject({text:z.string().min(1).max(1_200).nullable(),acts:z.array(act).max(4),respondsTo:refs,rationale:Rationale,memory}) as z.ZodType<V3Submission>;
  }
  return z.strictObject({mode:z.enum(["direct","uniform","abstain"]),choiceHandles:z.array(ChoiceHandle).max(16),rationale:Rationale,evidence:refs,memory,reconsiderationQuestion:Brief.nullable()}) as z.ZodType<V3Submission>;
}

function mappedJournal(packet:PlayerContextV2,map:HandleMap) {
  const source=(id:string)=>map.toAlias.get(id)??id;
  return {...packet.journal,beliefs:packet.journal.beliefs.map(belief=>({...belief,sources:belief.sources.map(source)})),hypotheses:packet.journal.hypotheses.map(hypothesis=>({...hypothesis,sources:hypothesis.sources.map(source)}))};
}
function compactEvidence(packet:PlayerContextV2,map:HandleMap) {
  return packet.sources.map(source=>({handle:map.toAlias.get(source.id),type:source.type,day:source.day,scope:source.scope,data:source.data}));
}
function taskPacket(packet:PlayerContextV2,task:V3TaskSpec) {
  const evidence=evidenceMap(packet),choices=choiceMap(packet);
  const presentedTask=task.type==="discussion_speech"
    ? {...task,plan:{...task.plan,respondsTo:task.plan.respondsTo.map(id=>evidence.toAlias.get(id)??id)}}
    : task;
  return {
    view:{phase:packet.phase,day:packet.day,closing:packet.closing,players:packet.players,self:packet.self,knownAllies:packet.knownAllies,factionObjective:packet.rules.factionObjective,journal:mappedJournal(packet,evidence),evidence:compactEvidence(packet,evidence),responseDocket:packet.responseDocket.map(id=>evidence.toAlias.get(id)??id)},
    task:presentedTask,
...
function compactPrevious(report:DecisionReportV2|null) {
  ...
  return {proposal:report.proposal,rationale:report.summary,reconsideration:report.control.question};
}

export function decisionRequestV3(packet:PlayerContextV2,task:V3TaskSpec,commitOnly:boolean,previous:DecisionReportV2|null,repair:string|null) {
  ...
  const input=JSON.stringify({REQUEST:{task:task.type,commitOnly,previous:compactPrevious(previous),repair}});
  const prompt={instructions,sharedInput,input,cache:{mode:"explicit" as const,ttl:"30m" as const,stablePrefix:`werewolf-player-v3:${task.type}`}};
  ...
  return {schema,prompt,jsonSchema,tokens:estimatedTokens({instructions,sharedInput,input})+estimatedTokens(jsonSchema),maxOutputTokens,promptVersion:"player_prompt_v3",schemaVersion:`${task.type}_v3`};
}
...
export function normalizeV3Submission(packet:PlayerContextV2,task:V3TaskSpec,submission:V3Submission,decisionId:string,commitOnly:boolean):DecisionReportV2 {
  if(task.type==="discussion_bid"||task.type==="discussion_listen") {
    const bid=submission as DiscussionBidV3|ListenerBidV3;
    const eligible=task.type==="discussion_bid",plan=eligible?(bid as DiscussionBidV3).plan:null,urge=eligible?(bid as DiscussionBidV3).urge:0;
    ...
    return {...report,speakerIntent:{wantsToSpeak:Boolean(plan),urge,willingnessToListen:Object.entries(bid.listen).map(([playerId,willingness])=>({playerId,willingness}))}} as DecisionReportV2;
  }
  if(task.type==="discussion_speech"||task.type==="closing_response") {
    const speech=submission as SpeechSubmissionV3,handles=evidenceMap(packet),source=(id:string)=>handles.toCanonical.get(id)??id;
    const respondsTo=task.type==="discussion_speech"?task.plan.respondsTo:speech.respondsTo.map(source);
    const publicSpeech=speech.text?{text:speech.text,acts:speech.acts.map(act=>({kind:act.kind,targetId:act.targetId,claim:act.claim,sourceId:act.evidence?source(act.evidence):null})),respondsTo}:null;
  ...
  const choice=submission as TargetChoiceSubmissionV3,choices=choiceMap(packet),source=evidenceMap(packet),ids=choice.choiceHandles.map(handle=>choices.toCanonical.get(handle)!).filter(Boolean);
...
export function validateV3Submission(packet:PlayerContextV2,task:V3TaskSpec,submission:V3Submission):string[] {
  const errors:string[]=[];
  ...
  if(task.type==="discussion_bid"||task.type==="discussion_listen") {
    const bid=submission as DiscussionBidV3|ListenerBidV3;
    const expected=packet.players.filter(player=>player.alive&&player.id!==packet.self.id).map(player=>player.id).sort(),actual=Object.keys(bid.listen).sort();
    if(JSON.stringify(expected)!==JSON.stringify(actual))errors.push("listen must rate every other living player exactly once");
    if(task.type==="discussion_bid"){
    ...
    if(task.type==="discussion_speech"){
      ...
      const matching=speech.acts.some(act=>act.kind===task.plan.kind&&act.targetId===task.plan.targetId);
      if(!matching)errors.push("selected speech must contain an act matching the frozen plan kind and target");
    } else {
      const canonicalReplies=speech.respondsTo.map(handle=>evidenceMap(packet).toCanonical.get(handle)??handle);
      if(speech.acts.some(act=>act.kind!=="reply"))errors.push("closing acts may only reply");
      if(speech.text&&!canonicalReplies.some(id=>packet.responseDocket.includes(id)))errors.push("closing speech must reply to the frozen docket");
      if(canonicalReplies.some(id=>!packet.responseDocket.includes(id)))errors.push("closing reply references must come from the frozen docket");
    }
  } else {
  ...
  if(evidence.some(handle=>!delivered.has(handle)))errors.push("evidence handles must reference delivered evidence");
  return errors;
}
