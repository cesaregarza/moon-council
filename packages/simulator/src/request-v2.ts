...
export const estimatedTokens = (value: unknown): number =>
  Math.ceil(
    Buffer.byteLength(typeof value === "string" ? value : JSON.stringify(value), "utf8") / 3,
  );
...
function mapJournalReferences(
  journal: PrivateJournalV2,
  map: (id: string) => string,
): PrivateJournalV2 {
  return {
    ...structuredClone(journal),
    beliefs: journal.beliefs.map((belief) => ({ ...belief, sources: belief.sources.map(map) })),
    hypotheses: journal.hypotheses.map((hypothesis) => ({
      ...hypothesis,
      sources: hypothesis.sources.map(map),
    })),
  };
}

function mapReportReferences(
  report: DecisionReportV2,
  map: (id: string) => string,
): DecisionReportV2 {
  const next = structuredClone(report);
  next.observations = next.observations.map(map);
  next.inferences = next.inferences.map((inference) => ({
    ...inference,
    sources: inference.sources.map(map),
  }));
  next.journalPatch = next.journalPatch.map((operation) => {
    if (operation.op === "upsert_belief")
      return {
        ...operation,
        value: { ...operation.value, sources: operation.value.sources.map(map) },
      };
    if (operation.op === "upsert_hypothesis")
      return {
        ...operation,
        value: { ...operation.value, sources: operation.value.sources.map(map) },
      };
  ...
  if (next.proposal.kind === "discussion" && next.proposal.speech)
    next.proposal.speech = {
      ...next.proposal.speech,
      respondsTo: next.proposal.speech.respondsTo.map(map),
      acts: next.proposal.speech.acts.map((act) => ({
        ...act,
        sourceId: act.sourceId ? map(act.sourceId) : null,
      })),
    };
  return next;
}

export function canonicalizeReportReferences(
  report: DecisionReportV2,
...
function presentReportReferences(
  report: DecisionReportV2,
  packet: PlayerContextV2,
): DecisionReportV2 {
...
function compactEvidence(
  source: PlayerContextV2["sources"][number],
  maps: CitationMaps,
  ...
  if (source.id === "facts:self" || source.id === "facts:rules" || source.id === "facts:roster") {
    return {
      ...
      scope: source.scope,
      reference:
        "Use the corresponding self, game-reference, or public-player record in this request.",
    };
  }
  ...
  const acts = Array.isArray(source.data.acts)
    ? source.data.acts.map((value) => {
        ...
        return {
          ...
          sourceId:
            typeof act.sourceId === "string"
              ? (maps.toAlias.get(act.sourceId) ?? act.sourceId)
              : act.sourceId,
        };
      })
    : [];
  return {
    ...
    scope: source.scope,
    data: {
      speakerId: source.data.playerId,
      speakerName: source.data.playerName,
      text: source.data.text,
      acts,
      respondsTo: Array.isArray(source.data.respondsTo)
        ? source.data.respondsTo.map((id) =>
            typeof id === "string" ? (maps.toAlias.get(id) ?? id) : id,
          )
        : [],
...
function compactRole(role: PublishedRole): Record<string, unknown> {
  const actions = Array.isArray(role.actions)
    ? role.actions.map((value) => {
        ...
        return {
          ...
          target: {
            min: target.min,
            max: target.max,
            allowSelf: target.allowSelf,
            allowConsecutiveTarget: target.allowConsecutiveTarget,
          },
          teamAggregation: action.teamAggregation,
          charges: action.charges,
        };
      })
    : [];
  const passives =
    role.passives && typeof role.passives === "object"
      ? (role.passives as Record<string, unknown>)
      : {};
  return {
    id: role.id,
    ...
    actions,
    passives: { voteWeight: passives.voteWeight },
...
export function stableGameReference(packet: PlayerContextV2): Record<string, unknown> {
  const {
    roles,
    factionObjective: _factionObjective,
    speakerSelection: _speakerSelection,
    journalRevision: _journalRevision,
    ...publishedRules
  } = packet.rules;
...
export function stableDecisionContext(
  packet: PlayerContextV2,
  kind: ActionProposalV2["kind"],
): Record<string, unknown> {
  const maps = citationMaps(packet);
  const publicEvidence = packet.sources
    .filter((source) => source.scope === "public")
    .map((source) => compactEvidence(source, maps));
  const privateEvidence = packet.sources
    .filter((source) => source.scope !== "public")
    .map((source) => compactEvidence(source, maps));
  return {
    task: {
      ...
      speakerSelection: packet.rules.speakerSelection,
    },
    ...
    privateView: {
      self: {
        id: packet.self.id,
        name: packet.self.name,
        personality: packet.self.personality,
        roleRef: {
          id: packet.self.role.id,
          version: packet.self.role.version,
          name: packet.self.role.name,
          alignment: packet.self.role.alignment,
          description: packet.self.role.description,
          knowledge: packet.self.role.knowledge,
        },
      },
      ...
      legalTargets: packet.legalTargets,
      currentJournal: mapJournalReferences(packet.journal, (id) => maps.toAlias.get(id) ?? id),
      responseDocket: packet.responseDocket.map((id) => maps.toAlias.get(id) ?? id),
    },
  };
}

export function initialCommitOnly(
  packet: PlayerContextV2,
  kind: ActionProposalV2["kind"],
  mode: "single" | "gated",
): boolean {
  const informative = packet.sources.some((s) =>
    ["speech.public", "inspection.delivered", "team.point", "vote.resolved"].includes(s.type),
  );
  return (
    mode === "single" ||
    kind === "pass" ||
    (kind === "discussion" && !packet.responseDocket.length) ||
    !informative
  );
}

export function decisionRequestV2(
  packet: PlayerContextV2,
  kind: ActionProposalV2["kind"],
  mode: "single" | "gated",
  commitOnly = initialCommitOnly(packet, kind, mode),
  previousProposal: DecisionReportV2 | null = null,
  ...
  const sharedInput = JSON.stringify({
    STABLE_DECISION_CONTEXT: stableDecisionContext(packet, kind),
  });
  const input = JSON.stringify({
    EPISODE_SUFFIX: {
      mode,
      commitOnly,
      journalPatchBaseVersion: packet.journal.version,
      previousProposal: previousProposal ? presentReportReferences(previousProposal, packet) : null,
      repair,
    },
  });
  const prompt = {
    ...
    sharedInput,
    input,
    cache: { mode: "explicit" as const, ttl: "30m" as const, stablePrefix: "werewolf-player-v2.2" },
  };
