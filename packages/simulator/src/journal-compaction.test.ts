...
import {
  compactNotebookSchema,
  compactionByteBudget,
...
const candidate: PrivateJournalV2 = {
  ...emptyJournalV2(),
  version: 1,
  beliefs: [
    {
      playerId: "p1",
      probability: 0,
      basis: "authorized_fact",
      note: "I am a villager.",
      sources: ["facts:self"],
    },
  ],
  hypotheses: [
    { id: "h1", statement: "Possibly a wolf; uncertain.", confidence: 0.4, sources: ["event:1"] },
  ],
  attentionNotes: [{ playerId: "p2", note: "They owe an answer.", sources: ["event:1"] }],
  deceptionPlan: null,
};
...
describe("journal compaction", () => {
  it("allows shorter prose and redundant hypothesis removal while protecting beliefs and provenance", () => {
    const result = {
      ...
      hypotheses: [],
      attentionNotes: [{ ...candidate.attentionNotes![0]!, note: "Await reply." }],
    };
    expect(validateJournalCompaction(candidate, result, 1200)).toEqual([]);
    expect(
      validateJournalCompaction(
        candidate,
        { ...result, beliefs: [{ ...result.beliefs[0]!, probability: 1 }] },
        1200,
      ).join(" "),
    ).toContain("belief values");
    expect(
      validateJournalCompaction(candidate, { ...result, attentionNotes: [] }, 1200).join(" "),
    ).toContain("every listening-note");
    expect(
      validateJournalCompaction(
        candidate,
        { ...result, attentionNotes: [{ ...result.attentionNotes[0]!, sources: ["forged"] }] },
        1200,
      ).join(" "),
    ...
    expect(
      validateJournalCompaction(
        candidate,
        { ...result, hypotheses: [{ ...candidate.hypotheses[0]!, id: "invented" }] },
        1200,
      ).join(" "),
    ).toContain("invented");
  ...
  it("explains conservative byte accounting and reuses only structurally faithful repair drafts", () => {
    const report = { journalPatch: [] } as unknown as NonNullable<
      DecisionOpportunityV1["journalCompaction"]
    >["sourceReport"];
    const compaction = {
      candidate,
      sourceReport: report,
    ...
    const prompt = journalCompactionRequest(
      compaction,
      1200,
      8192,
      "shorter",
    ...
    expect(compactionByteBudget(candidate, 1200)).toMatchObject({
      maximumSerializedBytes: 3600,
      targetSerializedBytes: 2700,
    });
    const corrupted = { ...draft, beliefs: [{ ...draft.beliefs[0]!, probability: 1 }] };
    expect(
      JSON.parse(
        journalCompactionRequest(compaction, 1200, 8192, "repair", JSON.stringify(corrupted)).prompt
          .input,
      ).previousSummary,
    ).toBeNull();
  });
  ...
  it("requires existing deception-plan text and prevents inventing a plan when none exists", () => {
    ...
    expect(schema.safeParse({ ...journalTextDraft(planned), deceptionPlan: null }).success).toBe(
      false,
    );
    expect(
      compactNotebookSchema(candidate, 1200).safeParse(journalTextDraft(planned)).success,
    ).toBe(false);
  });
  ...
  it("leaves an achievable soft prose target when protected metadata dominates", () => {
    const source = {
      ...
      beliefs: [
        {
          ...candidate.beliefs[0]!,
          sources: Array.from({ length: 50 }, (_, i) => `event:${String(i).padStart(36, "0")}`),
        },
      ],
    };
    const budget = compactionByteBudget(source, 1200);
    expect(budget.metadataBytesWithAllHypotheses).toBeGreaterThan(2400);
    expect(budget.targetSerializedBytes).toBeGreaterThan(
      budget.metadataBytesWithAllHypotheses + 300,
    );
  ...
  it("enforces short prose in the compaction schema instead of repeatedly asking for a smaller draft", () => {
    const source = {
      ...structuredClone(candidate),
      attentionNotes: Array.from({ length: 7 }, (_, i) => ({
        playerId: `p${i + 1}`,
        note: "long note",
        sources: ["event:one", "event:two"],
      })),
    ...
    const strings = (shape: Record<string, z.ZodType>, nullable = false) =>
      Object.fromEntries(
        Object.entries(shape).map(([id, field]) => [
          id,
          "x".repeat(
            (nullable ? (field as z.ZodNullable<z.ZodString>).unwrap() : (field as z.ZodString))
              .maxLength!,
          ),
        ]),
      );
    const longest = {
      beliefNotes: strings(schema.shape.beliefNotes.shape),
      attentionNotes: strings(schema.shape.attentionNotes.shape),
      hypotheses: strings(schema.shape.hypotheses.shape, true),
      strategy: "x".repeat(schema.shape.strategy.maxLength!),
      goals: Array(4).fill("x".repeat(schema.shape.goals.element.maxLength!)),
      unresolvedQuestions: Array(4).fill(
        "x".repeat(schema.shape.unresolvedQuestions.element.maxLength!),
      ),
      deceptionPlan: null,
    };
    ...
    const compaction = {
      candidate: source,
      sourceReport: {} as NonNullable<DecisionOpportunityV1["journalCompaction"]>["sourceReport"],
      sourceSubmission: null,
      sourceAttemptId: "source",
    };
    ...
    expect(materialized.beliefs[0]).toMatchObject({
      playerId: "p1",
      probability: 0,
      basis: "authorized_fact",
      sources: ["facts:self"],
  ...
  it.each([null, 2_000_000])(
    "checkpoints and commits compaction once, with output budgeting for total limit %s",
    async (maxTotalTokens) => {
      ...
      const store = new DecisionStore(repository),
        task = { type: "journal_update" as const, revision: "initial", sourceIds: [] };
      ...
      const provider: DecisionProvider = {
        decide: async (request) => {
          ...
          expect(request.reasoningEffort).toBe(
            request.schemaName === "journal_compaction" ? "low" : "xhigh",
          );
          expect(request.maxOutputTokens).toBe(
            kinds.length > 1 && maxTotalTokens === null ? null : config.safety.maxOutputTokens,
          );
          expect(request.timeoutMs).toBeLessThanOrEqual(config.deliberation.episodeTimeoutMs);
          if (kinds.length === 2 && maxTotalTokens === null)
            expect(request.timeoutMs).toBeGreaterThan(config.deliberation.requestTimeoutMs);
          ...
          if (kinds.length === 1) {
            request.onUsage?.(
              ...
              { provider: "openai", model: "luna", outputLimitEnforced: true },
            );
            throw new Error("OpenAI response incomplete: max_output_tokens");
          }
          let value: unknown;
          if (request.schemaName === "journal_update")
            value = {
              memory: {
                attentionUpdate: seats.map((s) => ({
                  playerId: s.id,
                  note: "Important observation. ".repeat(10),
                  evidence: [],
                })),
                beliefs: [],
                hypotheses: [],
                strategyUpdate: { strategy: "Long strategy. ".repeat(35), goals: [] },
                questionsUpdate: null,
                deceptionUpdate: null,
              },
              rationale: "Update based on the latest evidence.",
            };
          else {
            ...
            value = journalTextDraft({
              ...source,
              attentionNotes: source.attentionNotes.map(
                (n: { playerId: string; sources: string[] }) => ({ ...n, note: "Observe." }),
              ),
      ...
      const options: ExecuteDecisionOptions = {
        ...
        validateCurrent: () => store.journal(game.id, "p1").version === 0,
        eventsForCommit: () => [
          {
            ...
            visibility: "player",
            audienceIds: ["p1"],
            payload: { playerId: "p1", revision: "initial", sourceIds: [] },
          },
        ],
        requestForAttempt: (commit, previous, repair) => ({
          ...decisionRequestV31(packet, task, commit, previous, repair),
          providerKind: "decision_v3_1",
          schemaName: task.type,
          normalize: (value) => normalizeV31Submission(packet, task, value as never, op.id, true),
      ...
      const compacted = repository
        .listEvents(game.id)
        .filter((e) => e.type === "journal.compacted");
      expect(compacted).toHaveLength(1);
      expect(compacted[0]).toMatchObject({ visibility: "player", audienceIds: ["p1"] });
