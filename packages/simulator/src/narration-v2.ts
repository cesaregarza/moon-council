...
const instructions =
  "You are a neutral Werewolf moderator. Narrate only the supplied engine-produced public disclosure packet. Do not infer, reveal, or invent private information, player context, hidden reasoning, actions, or outcomes. Return only {text} as a concise atmospheric announcement.";
const active = (repository: LabRepository, gameId: string) =>
  ["running", "stepping"].includes(repository.getGame(gameId)?.status ?? "");
...
export class NarrationV2 {
  ...
  acknowledge(gameId: string, decisionId: string, text?: string): string {
    ...
    if (!checkpoint?.best || (text !== undefined && checkpoint.best !== text))
      throw new NarrationPausedError("narration acknowledgement has no matching pending report");
  ...
  async narrate(options: NarrationV2Options): Promise<string> {
    ...
    const log = (type: string, payload: Record<string, unknown>) =>
      this.repository.appendEvent(options.gameId, {
        type,
        phase: options.phase,
        day: options.day,
        visibility: "moderator",
        payload: { playerId: "moderator", decisionId: options.decisionId, ...payload },
      });
    const game = this.repository.getGame(options.gameId);
    if (!game || game.config.schemaVersion !== "game_config_v2")
      throw new Error("V2 narration requires a V2 game");
    ...
    const basePrompt: Prompt = {
      instructions,
      input: JSON.stringify({ disclosurePacket: options.disclosurePacket }),
    };
    const jsonSchema = providerJsonSchema(NarrationSchema);
    let checkpoint = this.checkpoint(options.gameId, options.decisionId);
    if (checkpoint && checkpoint.basePrompt.input !== basePrompt.input)
      throw new NarrationPausedError("narration disclosure packet changed after checkpoint");
    if (checkpoint?.best && ["pending", "committed"].includes(checkpoint.status))
      return checkpoint.best;
    if (checkpoint?.status === "skipped") return options.fallbackText;
    if (!active(this.repository, options.gameId)) {
      if (checkpoint)
        this.store.put(options.gameId, keyOf(options.decisionId), {
          ...checkpoint,
          status: "paused",
        });
    ...
    const recoveryAttempts = () =>
      this.store
        .attempts(options.gameId, options.decisionId)
        .filter((attempt) => attempt.recovery === checkpoint!.recovery);
    if (recoveryAttempts().some((attempt) => ["started", "received"].includes(attempt.status))) {
      ...
      this.store.put(options.gameId, keyOf(options.decisionId), checkpoint);
      throw new NarrationPausedError(
        "uncertain in-flight narration; resume starts a recorded recovery",
      );
    }
    ...
    const reserveTokens =
      options.mandatoryTokenReserve ??
      reserveCalls * (estimatedTokens(basePrompt) + estimatedTokens(jsonSchema) + maxOutputTokens);
    const admitsBudget = () => {
      const usage = usageTotals(this.store, options.gameId);
      try {
        assertV2Budget(this.store, options.gameId, config);
      } catch (error) {
        if (!(error instanceof V2BudgetError)) throw error;
        return false;
      }
      return (
        usage.calls + 1 + reserveCalls < config.safety.maxModelCalls &&
        (config.maxTotalTokens === null ||
          usage.admissionTokens +
            estimatedTokens(basePrompt) +
            estimatedTokens(jsonSchema) +
            maxOutputTokens +
            reserveTokens <
    ...
    if (!admitsBudget()) {
      checkpoint = {
        ...checkpoint,
        status: "skipped",
        error: "optional narration skipped to reserve mandatory decision budget",
      };
      this.store.put(options.gameId, keyOf(options.decisionId), checkpoint);
    ...
    for (let index = priorAttempts.length; index < 2; index += 1) {
        ...
        index === 0
          ...
          : {
              ...
              input: JSON.stringify({
                disclosurePacket: options.disclosurePacket,
                correctionRequired: checkpoint.error,
              }),
            };
      ...
      const attempt = this.store.beginAttempt(opportunity, {
        model: options.model,
      ...
      const abort = () =>
        controller.abort(options.signal?.reason ?? new Error("narration aborted"));
      if (options.signal?.aborted) abort();
      else options.signal?.addEventListener("abort", abort, { once: true });
      const requestTimeout = Math.min(
        timeoutMs,
        Math.max(1, config.deliberation.episodeTimeoutMs - (Date.now() - started)),
      );
      const timer = setTimeout(
        () => controller.abort(new Error("narration timeout")),
        requestTimeout,
      );
      const abortPoll = setInterval(() => {
        if (this.repository.getGame(options.gameId)?.status === "aborted")
      ...
      try {
        ...
        const call = this.provider.decide<Narration>({
          kind: "narration",
          model: options.model,
          playerId: "moderator",
          disclosurePacket: options.disclosurePacket,
        ...
        receivedResult = true;
        if (!usageReceived)
          onUsage(unknownUsage(), {
            provider: attempt.provider,
            model: attempt.model,
            outputLimitEnforced: options.provider !== "codex",
          });
        ...
        this.store.atomic(() => {
          this.store.updateAttempt(attempt);
          this.store.put(options.gameId, keyOf(options.decisionId), checkpoint);
          log("decision.narration_reported", {
            attemptId: attempt.id,
            recovery: checkpoint!.recovery,
            report,
          });
        ...
        if (!active(this.repository, options.gameId))
          throw new NarrationPausedError(
            "narration report is pending after pause; no announcement committed",
          );
        return report.text;
      } catch (error) {
        ...
        attempt.status =
          controller.signal.aborted || !receivedResult || !usageReceived
            ? receivedResult
              ? "invalid"
              : "unknown"
            : "invalid";
        ...
        if (!active(this.repository, options.gameId)) {
          ...
          this.store.put(options.gameId, keyOf(options.decisionId), checkpoint);
          throw new NarrationPausedError("narration paused after failed attempt");
        }
...
export const narrateV2 = (
  repository: LabRepository,
  provider: DecisionProvider,
  options: NarrationV2Options,
) => new NarrationV2(repository, provider).narrate(options);
