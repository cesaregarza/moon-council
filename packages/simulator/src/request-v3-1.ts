...
const FreeformMemorySchema = z.strictObject({
  journalUpdate: z.strictObject({mode:z.enum(["append","replace"]),text:z.string().min(1)}).nullable(),
});
const ActorMemorySchema = FreeformMemorySchema.extend({ decisionBrief: DecisionBriefTextSchema.nullable() });
type V31Memory = MemorySuggestionsV3 | z.infer<typeof FreeformMemorySchema> | z.infer<typeof ActorMemorySchema>;
const proseWorkflow = (packet:PlayerContextV2) => ["journal_v3","journal_v4"].includes(String(packet.rules.jevWorkflow));

type V31Bid = {
  urge: number; ready: boolean; plan: DiscussionPlanV3 | null;
  listen: Record<string,number|null>; memory: V31Memory; rationale: string;
};
...
type V31Speech = {
  ...
  respondsTo:string[];rationale:string;memory:V31Memory;
};
type V31Choice = {
  mode:"direct"|"uniform"|"abstain";choiceHandles:string[];rationale:string;evidence:string[];
  memory:V31Memory;reconsiderationQuestion:string|null;
};
...
function memorySchema(packet:PlayerContextV2, attention=false):z.ZodType<V31Memory> {
  ...
  return z.strictObject({
    ...(attention?{attentionUpdate:z.array(z.strictObject({playerId:player,note:Brief,evidence:refs})).max(16).nullable()}:{}),
...
export function v31SubmissionSchema(packet:PlayerContextV2,task:V3TaskSpec):z.ZodType<V31Submission> {
  ...
  if(task.type==="journal_update")return z.strictObject({memory,rationale:Rationale}) as z.ZodType<V31Submission>;
  if(task.type==="discussion_score")return z.strictObject({urge:z.number().min(0).max(1),ready:z.boolean(),listen,memory,rationale:Rationale}) as z.ZodType<V31Submission>;
  ...
  if(task.type==="discussion_speech"||task.type==="discussion_free_speech"||task.type==="closing_response"){
...
export function v31ApiResponseFormat(packet: PlayerContextV2, task: V3TaskSpec): ApiResponseFormat | undefined {
  if (!["journal_v2","journal_v3","journal_v4"].includes(String(packet.rules.jevWorkflow)) || !["journal_update", "discussion_free_speech", "closing_response"].includes(task.type)) return undefined;
  const player=asEnum(seatIds(packet));
  const speech=z.strictObject({text:z.string().min(1).max(1200).nullable(),acts:z.array(z.strictObject({kind:z.enum(["accusation","challenge","role_claim","result_claim","reply"]),targetId:player.nullable(),claim:Brief,evidence:EvidenceHandle.nullable()})).max(4),respondsTo:z.array(PublicHandle).max(6)});
  const schema=z.strictObject({task:z.enum(["journal_update","discussion_free_speech","closing_response"]),memory:memorySchema(packet,true),speech:speech.nullable(),rationale:Rationale});
  return {
    name:packet.rules.jevWorkflow==="journal_v4"?"moon_council_reflection_speech_v3":packet.rules.jevWorkflow==="journal_v3"?"moon_council_reflection_speech_v2":"moon_council_reflection_speech_v1",schema,
    instructions:(packet.rules.jevWorkflow==="journal_v4"?"For journal_update, always refresh memory.decisionBrief.action and .attention as current prose from your own perspective. For speech tasks, decisionBrief must be null. ":"")+(proseWorkflow(packet)?"For speech tasks, memory.journalUpdate must be null. For journal_update return a prose append/replace update, or null if nothing changed. ":"")+"API response envelope: task must exactly match REQUEST.task.type. For journal_update, speech must be null. For discussion_free_speech or closing_response, put text/acts/respondsTo in speech and leave memory unchanged (all update fields null or empty). Closing responses may only use reply acts. The task-specific game constraints still apply. Evidence is grouped by durability; E numbers preserve public event order, not the order of the message layers.",
    decode(value) {
      const data=schema.parse(value);
      if(data.task!==task.type)throw new Error(`Expected API response task ${task.type}, received ${data.task}`);
      ...
      if("journalUpdate" in data.memory){
        if("decisionBrief" in data.memory&&data.memory.decisionBrief!==null)throw new Error("Decision brief changes belong in journal_update");
        if(data.memory.journalUpdate!==null)throw new Error("Journal changes belong in journal_update");
        return {...data.speech,memory:data.memory,rationale:data.rationale};
      }
      if(data.memory.attentionUpdate!==null)throw new Error("Listening-note replacement belongs in journal_update");
...
function mapJournal(journal:PrivateJournalV2,map:HandleMap,strict:boolean):PrivateJournalV2 {
  ...
  return {...structuredClone(journal),...(journal.attentionNotes?{attentionNotes:journal.attentionNotes.map(note=>({...note,sources:refs(note.sources)}))}:{}),beliefs:journal.beliefs.map(belief=>({...belief,sources:refs(belief.sources)})),hypotheses:journal.hypotheses.map(hypothesis=>({...hypothesis,sources:refs(hypothesis.sources)}))};
}
...
function expandedPublicEvidence(packet:PlayerContextV2,map:HandleMap,digestChars:number) {
  const cited=new Set([...packet.journal.beliefs.flatMap(b=>b.sources),...packet.journal.hypotheses.flatMap(h=>h.sources),...(packet.journal.attentionNotes??[]).flatMap(n=>n.sources),...packet.responseDocket]);
  return packet.sources
...
const FREEFORM_JOURNAL_INSTRUCTIONS=JOURNAL_INSTRUCTIONS.replace("Use references only in evidence and respondsTo fields, never in public text.","Use references in private notes and structured evidence/respondsTo fields, never in public text.").replace(
  JOURNAL_INSTRUCTIONS.slice(JOURNAL_INSTRUCTIONS.indexOf("For journal_update"),JOURNAL_INSTRUCTIONS.indexOf("For discussion_free_speech")),
...
const ACTOR_JOURNAL_INSTRUCTIONS=FREEFORM_JOURNAL_INSTRUCTIONS.replace("You are one isolated Werewolf player.","You take the perspective of the named player in this game; your role and objectives are given in your private state.").replace("Your free-form journal is the primary reasoning Jev receives for speaking urgency, listening interest, votes and night actions.","Your free-form journal is your durable memory. Jev decides for you using your current decision brief and your verified private facts.")+`
...
function layers(packet:PlayerContextV2,task:V3TaskSpec,commitOnly:boolean,previous:DecisionReportV2|null,repair:string|null,protocol:"agent_v3_1"|"agent_v3_2"="agent_v3_1") {
  ...
  const reflectionEvidence=task.type==="journal_update"?packet.sources.filter(source=>task.sourceIds.includes(source.id)&&tierOf(source)!=="full").map(source=>renderSource(source,handles,digestChars,"full")):[];
  if(task.type==="journal_update"&&task.sourceIds.some(id=>!handles.toAlias.has(id)))throw new Error("Journal update is missing newly delivered evidence");
  const privateInput=JSON.stringify({AUTHORIZED_PRIVATE_STATE:{
    ...
    factionObjective:packet.rules.factionObjective,journal:proseWorkflow(packet)?journalText(packet.journal):mapJournal(packet.journal,handles,v32),
    ...(packet.rules.jevWorkflow==="journal_v4"?{previousDecisionBrief:packet.journal.decisionBrief??null}:{}),
    ...
    ...(task.type.endsWith("_choice")?{legalChoices:Object.fromEntries(choices.aliases.map(alias=>{const id=choices.toCanonical.get(alias)!;return [alias,{playerId:id,name:packet.players.find(player=>player.id===id)?.name??id}]})),action:task.type==="vote_choice"?"elimination_vote":packet.legalActions[0]?.actionId??null}:{}),
  }});
  const presentedTask=task.type==="journal_update"?{...task,sourceIds:task.sourceIds.map(id=>handles.toAlias.get(id)).filter(Boolean)}:task.type==="discussion_speech"?{...task,plan:{...task.plan,respondsTo:task.plan.respondsTo.map(id=>handles.toAlias.get(id)??id)}}:task;
  ...
  const listenDirective=v32&&(task.type==="discussion_bid"||task.type==="discussion_score"||task.type==="discussion_listen")
    ? {listen:{
    ...
    : {};
  const journalDirective=task.type==="journal_update"&&proseWorkflow(packet)?{journalBudget:packet.rules.maxJournalTokens}:task.type==="journal_update"?{memoryUpdateLimit:"At most six changes: each belief/hypothesis entry counts as one, and each non-null attentionUpdate, strategyUpdate, questionsUpdate or deceptionUpdate counts as one. Leave unchanged sections null and unchanged beliefs/hypotheses empty."}:{};
  const input=JSON.stringify({REQUEST:{task:presentedTask,commitOnly,previous:previous?{proposal:previous.proposal,rationale:previous.summary,reconsideration:previous.control.question}:null,repair,...listenDirective,...journalDirective}});
...
function decisionRequest(packet:PlayerContextV2,task:V3TaskSpec,commitOnly:boolean,previous:DecisionReportV2|null,repair:string|null,protocol:"agent_v3_1"|"agent_v3_2") {
  ...
  const maxOutputTokens=task.type==="discussion_bid"||task.type==="discussion_score"||task.type==="discussion_listen"?300:task.type==="journal_update"?900:task.type==="discussion_speech"||task.type==="discussion_free_speech"||task.type==="closing_response"?500:250;
  return {schema,prompt,jsonSchema,apiResponseFormat,tokens:estimatedTokens({...built,jsonSchema}),maxOutputTokens,promptVersion:`player_prompt_${version}`,schemaVersion:`${task.type}_${suffix}`};
}
...
function memoryPatch(packet:PlayerContextV2,memory:V31Memory,decisionId:string) {
  if("journalUpdate" in memory) {
    const patch:DecisionReportV2["journalPatch"]=memory.journalUpdate?[{op:"write_text",mode:memory.journalUpdate.mode,text:memory.journalUpdate.mode==="append"?`Day ${packet.day}, ${packet.phase}:\n${memory.journalUpdate.text}`:memory.journalUpdate.text}]:[];
    if("decisionBrief" in memory&&memory.decisionBrief) {
      if(typeof packet.rules.journalRevision!=="string")throw new Error("Missing authorized journal revision");
      patch.push({op:"set_decision_brief",value:{...memory.decisionBrief,playerId:packet.self.id,evidenceRevision:packet.rules.journalRevision}});
    }
    return patch;
  }
  ...
  if(memory.attentionUpdate)patch.push({op:"set_attention",notes:memory.attentionUpdate.map(note=>({playerId:note.playerId,note:note.note,sources:note.evidence.map(source)}))});
...
export function normalizeV31Submission(packet:PlayerContextV2,task:V3TaskSpec,submission:V31Submission,decisionId:string,commitOnly:boolean):DecisionReportV2 {
  if(task.type==="journal_update")return baseReport(submission.rationale,memoryPatch(packet,submission.memory,decisionId),{kind:"pass",reason:"Private journal update; no game action."});
  if(task.type==="discussion_bid"||task.type==="discussion_score"||task.type==="discussion_listen"){
    const bid=submission as V31Bid|V31Listen,eligible=task.type==="discussion_bid",plan=eligible?(bid as V31Bid).plan:null,urge=eligible?(bid as V31Bid).urge:task.type==="discussion_score"?(submission as V31Score).urge:0;
    ...
    return Object.assign(report,{speakerIntent:{wantsToSpeak:task.type==="discussion_score"?urge>0:Boolean(plan),urge,willingnessToListen:Object.entries(bid.listen).filter((entry):entry is [string,number]=>typeof entry[1]==="number").map(([playerId,willingness])=>({playerId,willingness}))}});
  }
  if(task.type==="discussion_speech"||task.type==="discussion_free_speech"||task.type==="closing_response"){
...
export function validateV31Submission(packet:PlayerContextV2,task:V3TaskSpec,submission:V31Submission):string[] {
  const errors:string[]=[],handles=evidenceMapV31(packet),delivered=new Set(handles.aliases),suppliedMemory=submission.memory;
  if("journalUpdate" in suppliedMemory&&suppliedMemory.journalUpdate&&!suppliedMemory.journalUpdate.text.trim())errors.push("Journal prose must not be blank");
  if("journalUpdate" in suppliedMemory&&task.type!=="journal_update"&&suppliedMemory.journalUpdate!==null)errors.push("Journal changes belong in journal_update");
  if(packet.rules.jevWorkflow==="journal_v4") {
    const brief="decisionBrief" in suppliedMemory?suppliedMemory.decisionBrief:null;
    if(task.type==="journal_update"&&!brief)errors.push("Refresh the current action and attention decision briefs");
    if(task.type!=="journal_update"&&brief)errors.push("Decision brief changes belong in journal_update");
  }
  const memory:MemorySuggestionsV3="journalUpdate" in suppliedMemory?{beliefs:[],hypotheses:[],strategyUpdate:null,questionsUpdate:null,deceptionUpdate:null}:suppliedMemory;
  const evidence=[...memory.beliefs.flatMap(item=>item.evidence),...memory.hypotheses.flatMap(item=>item.evidence),...(memory.attentionUpdate??[]).flatMap(item=>item.evidence)];
  if(task.type==="journal_update"){
    const operations=memory.beliefs.length+memory.hypotheses.length+Number(memory.attentionUpdate!=null)+Number(memory.strategyUpdate!=null)+Number(memory.questionsUpdate!=null)+Number(memory.deceptionUpdate!=null);
    if(operations>6)errors.push("A journal update may change at most six entries or sections; prioritize the most useful changes");
    const notes=memory.attentionUpdate??[];
    if(new Set(notes.map(n=>n.playerId)).size!==notes.length)errors.push("attention notes must name each player at most once");
  }else if(task.type==="discussion_bid"||task.type==="discussion_score"||task.type==="discussion_listen"){
  ...
  }else if(task.type==="discussion_speech"||task.type==="discussion_free_speech"||task.type==="closing_response"){
