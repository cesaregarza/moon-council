...
function eventText(event: GameEventV1, players: Player[]): string {
  ...
  switch (event.type) {
    ...
    case "team.consensus_reached":
      return `The pack unanimously points at ${playerName(payload.targetId, players)}.`;
    ...
    case "night.resolved": {
      const eliminated = Array.isArray(payload.eliminatedPlayerIds)
        ...
        : [];
      const protectedIds = Array.isArray(payload.protectedPlayerIds)
        ? payload.protectedPlayerIds.map((id) => playerName(id, players))
        : [];
      const blocked = Array.isArray(payload.blockedActorIds)
        ? payload.blockedActorIds.map((id) => playerName(id, players))
        : [];
      const parts = [
    ...
    case "decision.reported":
      return `${playerName(payload.playerId, players)} recorded a deliberation report.`;
    default:
      return event.type;
  }
}
...
export function GameRoom({ gameId, onBack, onChanged, onClone }: Props) {
  ...
  async function refresh(): Promise<ObserverGamePayload | undefined> {
    ...
    try {
      ...
      if (replaySequence === null)
        setLiveMaxSequence((next.events ?? next.state.events ?? []).at(-1)?.sequence ?? 0);
      setError("");
      return next;
    } catch (reason) {
  ...
  useEffect(() => {
    ...
    if (perspective !== "moderator" && perspective !== "player") {
      setOpportunities([]);
      setDecisionDetail(undefined);
      return;
    }
  ...
  useEffect(() => {
    let cancelled = false;
    setDecisionDetail(undefined);
    if (!selectedDecision || (perspective !== "moderator" && perspective !== "player")) {
      setDecisionDetail(undefined);
      return;
    }
    void api
      .decision(gameId, selectedDecision, {
        view: perspective,
        playerId: selectedPlayer || undefined,
        at: replaySequence,
      })
  ...
  const transcript = events.filter((event) =>
    [
      ...
      "message.public",
      "team.pointed",
      "team.consensus_reached",
      "team.consensus_failed",
  ...
  const selected =
    payload?.state.players.find((player) => player.id === selectedPlayer) ??
    payload?.state.players[0];
  const tokenTotal = payload?.usage?.reduce((sum, row) => sum + (row.totalTokens ?? 0), 0) ?? 0;
  const reportedInputTotal =
    payload?.usage?.reduce((sum, row) => sum + (row.inputTokens ?? 0), 0) ?? 0;
  const cachedInputTotal =
    payload?.usage?.reduce((sum, row) => sum + (row.cachedInputTokens ?? 0), 0) ?? 0;
  const cacheRate =
    reportedInputTotal > 0 ? Math.round((cachedInputTotal / reportedInputTotal) * 100) : null;
  ...
  if (!payload || (payload.view && payload.view !== perspective))
    return (
      <section className="workspace">
        <div className="panel empty-state">{error || "Opening the council chamber…"}</div>
      </section>
    );
  const journals = payload.state.journals ?? {};
  const journal = selected ? journals[selected.id] : undefined;
  const journalRecord =
    journal && typeof journal === "object" ? (journal as Record<string, unknown>) : undefined;
  ...
  return (
    <section className="workspace game-room">
      <header className="game-header panel">
        ...
        <div>
          <div className="eyebrow">
            Day {payload.state.day} · {payload.state.phase.replaceAll("_", " ")}
            {replaySequence !== null ? ` · replay at #${replaySequence}` : ""}
          </div>
          <h2>{payload.game.name}</h2>
        </div>
        <div className="status-cluster">
          <span className={`status-pill ${payload.game.status}`}>{payload.game.status}</span>
          <span className="token-pill">
            {perspective === "public" || perspective === "team"
              ? "Private usage hidden"
              : `${payload.state.modelCalls ?? payload.usage?.length ?? 0} attempts · ${formatCompactCount(tokenTotal)} known tokens${payload.usage?.some((row) => row.totalTokens === null) ? " + unknown usage" : ""}${cacheRate === null ? "" : ` · ${formatCompactCount(cachedInputTotal)} cached (${cacheRate}%)`}`}
          </span>
        </div>
      </header>
      ...
      <div className="control-bar panel">
        ...
        {payload.game.legacyReplayOnly && (
          <span className="replay-only-label">
            Legacy game · run / resume / phase step are replay-only
          </span>
        )}
        <label className="toggle spoiler-toggle">
          <input
            type="checkbox"
            checked={perspective === "moderator"}
            onChange={(event) => {
              setPerspective(event.target.checked ? "moderator" : "public");
              setReplaySequence(null);
            }}
          />
          <span>Spoiler glass</span>
        </label>
        <select
          value={perspective}
          onChange={(event) => {
            const next = event.target.value as GameView;
            setPerspective(next);
            setReplaySequence(null);
            if (next === "player" && !selectedPlayer)
              setSelectedPlayer(payload.state.players[0]?.id ?? "");
            if (next === "team") setTeamId("werewolves");
            else setTeamId("");
          }}
          aria-label="Observer perspective"
        >
          <option value="public">Public view</option>
          <option value="moderator">Moderator view</option>
        ...
        {perspective === "player" && (
          <select
            ...
            aria-label="Player view subject"
          >
        ...
        {perspective === "team" && (
          <input
            ...
            value={teamId}
            placeholder="Team channel (werewolves)"
            onChange={(event) => setTeamId(event.target.value)}
            aria-label="Team channel"
          />
        )}
        <a className="export-link" href={exportUrl("json")}>
          JSON
        </a>
        <a className="export-link" href={exportUrl("jsonl")}>
          JSONL
        </a>
        {onClone && perspective === "moderator" && payload.game.config && (
          <button className="ghost clone-button" onClick={() => onClone(payload.game.config)}>
            Clone setup
          </button>
        )}
      </div>
      ...
      <div className="game-grid observer-grid">
        <div className="panel player-panel">
          <div className="section-heading">
            <div>
              <div className="eyebrow">Living table</div>
              <h3>Players</h3>
            </div>
            <b>{payload.state.players.filter((player) => player.alive).length}</b>
          </div>
          <div className="player-list">
            {payload.state.players.map((player) => (
              <button
                className={`player-card ${player.alive ? "" : "dead"} ${selected?.id === player.id ? "selected" : ""}`}
                key={player.id}
                onClick={() => setSelectedPlayer(player.id)}
              >
                <span className="avatar">{player.name.slice(-1)}</span>
                <span>
                  <strong>{player.name}</strong>
                  <small>
                    {player.role?.name ??
                      player.revealedRole ??
                      (player.alive ? "Role hidden" : "Unknown role")}
                  </small>
                </span>
                <i>{player.alive ? "●" : "×"}</i>
              </button>
          ...
          {perspective === "moderator" && selected && (
            <div className="journal">
              ...
              <h4>Goals</h4>
              <ul>
                {stringList(recordValue(journalRecord, "goals")).map((goal) => (
                  <li key={goal}>{goal}</li>
                ))}
              </ul>
            </div>
        ...
        <div className="panel transcript-panel">
          <div className="section-heading">
            <div>
              <div className="eyebrow">
                {replaySequence === null ? "Live transcript" : "Historical replay"}
              </div>
              <h3>Council record</h3>
            </div>
            <button
              className="ghost"
              onClick={() => setReplaySequence(replaySequence === null ? maxSequence : null)}
            >
              {replaySequence === null ? "Enter replay" : "Return live"}
            </button>
          </div>
          {replaySequence !== null && (
            <div className="replay-control">
              <input
                ...
                min={0}
                max={maxSequence}
                value={Math.min(replaySequence, maxSequence)}
                onChange={(event) => setReplaySequence(Number(event.target.value))}
              />
              <span>
                Visible event #{replaySequence} / {maxSequence}
              </span>
            </div>
          )}
          <div className="transcript">
            {transcript.length === 0 && (
              <div className="empty-state">No events are visible in this perspective.</div>
            )}
            {transcript.map((event) => (
              <article
                className={`event ${event.type.replaceAll(".", "-")} ${event.visibility}`}
                key={event.id}
              >
                <div className="event-meta">
                  <span>{eventActor(event, payload.state.players)}</span>
                  <small>
                    #{event.sequence} · {event.visibility}
                  </small>
                </div>
                <p>{eventText(event, payload.state.players)}</p>
                {event.type === "vote.resolved" && (
                  <details>
                    <summary>Published individual ballots and tally</summary>
                    <pre>
                      {JSON.stringify(
                        { ballots: event.payload.ballots, tally: event.payload.tally },
                        null,
                        2,
                      )}
                    </pre>
                ...
                {event.type === "speech.public" && Array.isArray(event.payload.acts) && (
                  <small>
                    {(event.payload.acts as { kind: string; targetId: string | null }[])
                      .map(
                        (act) =>
                          `${act.kind.replaceAll("_", " ")} ${act.targetId ? playerName(act.targetId, payload.state.players) : ""}`,
                      )
                      .join(" · ")}{" "}
        ...
        <aside className="panel audit-panel">
          <div className="eyebrow">Observer ledger</div>
          ...
          <div className="timeline">
            {events
              ...
              .map((event) => (
                <div key={event.id}>
                  <span className={`timeline-dot ${event.visibility}`} />
                  <p>{event.type}</p>
                  <small>
                    #{event.sequence} · D{event.day}
                  </small>
                </div>
              ))}
          </div>
      ...
      {(perspective === "moderator" || perspective === "player") && (
        <section className="panel">
          <div className="section-heading">
            <div>
              <div className="eyebrow">Historical private state · {selected?.name}</div>
              <h3>{selected?.role?.name ?? "Private journal"}</h3>
            </div>
          </div>
          <details>
            <summary>Beliefs, listening notes, plans, and public deception</summary>
            <pre className="packet-inspector">
              {typeof journalRecord?.text === "string"
                ? journalRecord.text
                : JSON.stringify(journalRecord ?? {}, null, 2)}
            </pre>
          </details>
          <details>
            <summary>Attempt usage (unknown is not zero)</summary>
            <pre className="packet-inspector">{JSON.stringify(payload.usage ?? [], null, 2)}</pre>
          </details>
        </section>
      )}
      {(perspective === "moderator" || perspective === "player") && (
        <section className="decision-layout">
          <div className="panel decision-list">
            <div className="section-heading">
              <div>
                <div className="eyebrow">Decision opportunities</div>
            ...
            {opportunities.length === 0 && (
              <div className="empty-state">
                No decision opportunities for this view or replay position.
              </div>
            )}
            {opportunities.map((item) => (
              <button
                key={item.id}
                className={`decision-row ${item.id === selectedDecision ? "selected" : ""}`}
                onClick={() => setSelectedDecision(item.id)}
              >
                <span>
                  <strong>
                    {playerName(item.playerId, payload.state.players)} ·{" "}
                    {(item.taskType ?? item.kind).replaceAll("_", " ")}
                  </strong>
                  <small>
                    {item.playerId} · D{item.day} · {item.status}
                  </small>
                </span>
                <i>r{item.recovery}</i>
              </button>
          ...
          <div className="panel decision-detail">
            {!decisionDetail ? (
              <div className="empty-state">
                Select a decision to inspect its exact context packet and report.
              </div>
            ) : (
              <>
                <div className="section-heading">
                  <div>
                    <div className="eyebrow">
                      {decisionDetail.opportunity.status} · {decisionDetail.opportunity.id}
                    </div>
                    <h3>
                      {playerName(decisionDetail.opportunity.playerId, payload.state.players)} ·{" "}
                      {(
                        decisionDetail.opportunity.taskType ?? decisionDetail.opportunity.kind
                      ).replaceAll("_", " ")}
                    </h3>
                  </div>
                  <span className="status-pill">{decisionDetail.turns.length} turns</span>
                </div>
                <div className="decision-columns">
                  <div>
                    <h4>Context packet</h4>
                    <p className="muted compact">
                      Exact player context ·{" "}
                      {Array.isArray(recordValue(packet, "sources"))
                        ? `${(recordValue(packet, "sources") as unknown[]).length} sources`
                    ...
                    <details className="packet-inspector">
                  ...
                  <div>
                    <h4>
                      {decisionDetail.opportunity.taskType ? "Application record" : "Best report"}
                    </h4>
                    {best ? (
                      <>
                        ...
                        <div className="decision-facts">
                          <span>
                            Observations · {stringList(recordValue(best, "observations")).length}
                          </span>
                          <span>
                            Proposal ·{" "}
                            {typeof recordValue(best, "proposal") === "object"
                              ? "recorded"
                              : "none"}
                          </span>
                          <span>
                            Journal patch ·{" "}
                            {Array.isArray(recordValue(best, "journalPatch"))
                              ? (recordValue(best, "journalPatch") as unknown[]).length
                              : 0}
                          </span>
                        </div>
                        <div className="alternative-list">
                          {Array.isArray(recordValue(best, "alternatives")) &&
                            (recordValue(best, "alternatives") as Record<string, unknown>[]).map(
                              (alternative) => (
                                <div
                                  key={String(alternative.id)}
                                  className={
                                    alternative.id === recordValue(best, "selectedAlternativeId")
                                      ? "alternative selected"
                                      : "alternative"
                                  }
                                >
                                  <strong>{String(alternative.description)}</strong>
                                  <small>
                                    ＋ {String(alternative.advantage)} · −{" "}
                                    {String(alternative.drawback)}
                                  </small>
                                </div>
                              ),
                            )}
                        </div>
                        {decisionDetail.opportunity.bestSubmission && (
                          <details className="packet-inspector">
                            <summary>Inspect small model submission</summary>
                            <pre>
                              {JSON.stringify(decisionDetail.opportunity.bestSubmission, null, 2)}
                            </pre>
                          </details>
                        )}
                        <details className="packet-inspector">
                          <summary>Inspect application proposal &amp; journal patch</summary>
                          <pre>
                            {JSON.stringify(
                              {
                                observations: recordValue(best, "observations"),
                                proposal: recordValue(best, "proposal"),
                                journalPatch: recordValue(best, "journalPatch"),
                              },
                              null,
                              2,
                            )}
                ...
                <details>
                  <summary>Full report turns, continuation verdicts, and attempt metadata</summary>
                  <pre className="packet-inspector">
                    {JSON.stringify(
                      {
                        turns: decisionDetail.turns,
                        attempts: decisionDetail.attempts,
                        events: decisionDetail.events,
                      },
                      null,
                      2,
                    )}
                ...
                <div className="turn-strip">
                  <h4>Attempts, turns &amp; report events</h4>
                  {decisionDetail.attempts.map((attempt) => (
                    <span key={attempt.id} className="attempt-chip">
                      {attempt.provider ?? "provider"} · {attempt.status}
                    </span>
                  ))}
                  {decisionDetail.turns.map((turn) => (
                    <span key={`${turn.decisionId}:${turn.turnIndex}`} className="attempt-chip">
                      turn {turn.turnIndex + 1} ·{" "}
                      {String(
                        recordValue(turn.report, "control")
                          ? (recordValue(turn.report, "control") as Record<string, unknown>).kind
                          : "report",
                      )}
                    </span>
                  ))}
