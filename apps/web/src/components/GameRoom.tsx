  ...
  return (
    <section className="workspace game-room">
      ...
      {(perspective === "moderator" || perspective === "player") && (
        <section className="panel"><div className="section-heading"><div><div className="eyebrow">Historical private state · {selected?.name}</div><h3>{selected?.role?.name ?? "Private journal"}</h3></div></div><details><summary>Beliefs, listening notes, plans, and public deception</summary><pre className="packet-inspector">{typeof journalRecord?.text === "string" ? journalRecord.text : JSON.stringify(journalRecord ?? {},null,2)}</pre></details><details><summary>Attempt usage (unknown is not zero)</summary><pre className="packet-inspector">{JSON.stringify(payload.usage ?? [],null,2)}</pre></details></section>
      )}
      {(perspective === "moderator" || perspective === "player") && (
