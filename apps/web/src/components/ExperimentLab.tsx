...
export function ExperimentLab({ games, experiments, onCreated }: Props) {
  ...
  async function control(id: string, action: "pause" | "resume") {
    try {
      await api.controlExperiment(id, action);
      onCreated();
    } catch (reason) {
  ...
  async function create() {
    ...
    try {
      ...
      const spec: ExperimentSpecV1 | Record<string, unknown> =
        source.game.config.schemaVersion === "game_config_v2"
          ? {
              schemaVersion: "experiment_v2",
              name: `${source.game.name} study`,
              baseConfig: source.game.config,
              runs,
              concurrency,
              baseSeed: `${source.game.config.seed}-batch`,
    ...
    <section className="workspace experiment-layout">
      <div className="panel experiment-create">
        ...
        <label>
          ...
          <select value={gameId} onChange={(event) => setGameId(event.target.value)}>
            ...
            {games.map((game) => (
              <option key={game.game.id} value={game.game.id}>
                {game.game.name}
              </option>
            ))}
          </select>
        ...
        <details>
          <summary>Per-model cost rates (optional)</summary>
          <label>
            USD per million tokens
            <textarea
              value={rates}
              onChange={(event) => setRates(event.target.value)}
              placeholder={'{"model-id":{"input":0,"output":0}}'}
            />
          </label>
      ...
      <div className="experiment-results">
        {experiments.length === 0 && (
          <div className="empty-state">No experiments yet. Start with a small ten-game batch.</div>
        )}
        {experiments.map((experiment) => (
          <article className="panel experiment-card" key={experiment.id}>
            <div className="card-heading">
              <div>
                <div className="eyebrow">{experiment.status}</div>
                <h3>{experiment.name}</h3>
              </div>
              <strong>
                {experiment.summary?.runsCompleted ?? 0}/{experiment.spec.runs}
              </strong>
            </div>
            {experiment.summary ? (
              <>
                <div className="metric-row">
                  {Object.entries(experiment.summary.winsByAlignment).map(([alignment, count]) => (
                    <div key={alignment}>
                      <small>{alignment} wins</small>
                      <b>{count}</b>
                    </div>
                  ))}
                  <div>
                    <small>vote accuracy</small>
                    <b>{Math.round(experiment.summary.voteAccuracy * 100)}%</b>
                  </div>
                  <div>
                    <small>avg. days</small>
                    <b>{experiment.summary.averageCycles.toFixed(1)}</b>
                  </div>
                  {experiment.summary.completed !== undefined && (
                    <div>
                      <small>completed</small>
                      <b>{experiment.summary.completed}</b>
                    </div>
                  )}
                  {experiment.summary.interrupted !== undefined && (
                    <div>
                      <small>interrupted</small>
                      <b>{experiment.summary.interrupted}</b>
                    </div>
                  )}
                  {experiment.summary.failed !== undefined && (
                    <div>
                      <small>failed</small>
                      <b>{experiment.summary.failed}</b>
                    </div>
                  )}
                  {experiment.summary.budgetTruncated !== undefined && (
                    <div>
                      <small>budget truncated</small>
                      <b>{experiment.summary.budgetTruncated}</b>
                    </div>
                  )}
                </div>
                <div className="bar-track">
                  <span
                    style={{
                      width: `${(experiment.summary.runsCompleted / experiment.spec.runs) * 100}%`,
                    }}
                  />
                </div>
                <p className="muted compact">
                  {formatCompactCount(
                    experiment.summary.inputTokens + experiment.summary.outputTokens,
                  )}{" "}
                  tokens · {experiment.summary.followUps} follow-ups ·{" "}
                  {experiment.summary.modelFailures} failures
                  {experiment.summary.validOutcomeDenominator !== undefined
                    ? ` · ${experiment.summary.validOutcomeDenominator} valid outcomes`
                    : ""}
                </p>
              </>
            ) : (
