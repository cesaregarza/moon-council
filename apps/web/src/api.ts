...
export interface DecisionOpportunity {
  ...
  createdAt: string;
  taskType?: string;
  bestSubmission?: Record<string, unknown> | null;
}
...
export interface ExperimentRecord {
  ...
  spec:
    ...
    | {
        ...
        baseConfig: StoredGameConfig;
        runs: number;
        concurrency: number;
        name: string;
        baseSeed: string;
        pricingPerMillionTokens: Record<string, number>;
      };
  summary?: ExperimentSummaryV1 & {
    completed?: number;
    interrupted?: number;
    failed?: number;
    budgetTruncated?: number;
    validOutcomeDenominator?: number;
  };
  error?: string;
}
...
export const api = {
  ...
  game: (
    id: string,
    options: { view?: GameView; playerId?: string; teamId?: string; at?: number | null } = {},
  ) => {
  ...
  controlExperiment: (id: string, action: "pause" | "resume") =>
    request<{ ok: boolean; status: string }>(`/api/v1/experiments/${id}/control`, {
      method: "POST",
      body: JSON.stringify({ action }),
    }),
  createExperiment: (spec: ExperimentSpecV1 | Record<string, unknown>) =>
    request<ExperimentRecord>("/api/v1/experiments", {
      method: "POST",
      body: JSON.stringify(spec),
    }),
  decisions: (
    id: string,
    options: { view: "moderator" | "player"; playerId?: string; at?: number | null },
  ) => {
  ...
  decision: (
    ...
    return request<DecisionDetail>(
      `/api/v1/games/${id}/decisions/${decisionId}?${query.toString()}`,
    );
  },
};
