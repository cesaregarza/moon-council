...
const role = (id: string, name = id) => ({
  schemaVersion: "role_v1",
  id,
  version: id === "doctor" ? 2 : 1,
  name,
  alignment: id === "werewolf" ? "werewolf" : "village",
  ...
  winCondition: {
    terminal: true,
    predicate: { kind: "alignment_eliminated", alignment: "werewolf" },
  },
});
const roles = [
...
const config = {
  ...
  roleDeck: [roles[1], roles[1], roles[2], roles[3], roles[0], roles[0], roles[0], roles[0]],
  ...
  deliberation: {
    ...
    episodeTimeoutMs: 300000,
    maxContextTokens: 8000,
    maxJournalTokens: 1200,
  ...
  modelSettings: Object.fromEntries(
    players.map((player) => [
      player.id,
      { model: "gpt-5.6-luna", reasoningEffort: "xhigh", provider: "fake" },
    ]),
  ),
};
const publicEvent = (
  sequence: number,
  type = "message.public",
  payload: Record<string, unknown> = { text: "The village watches the moon." },
) => ({
...
const privateEvent = (
  sequence: number,
  visibility: "moderator" | "player" | "team",
  payload: Record<string, unknown>,
  audienceIds: string[] = [],
) => ({ ...publicEvent(sequence, "decision.reported", payload), visibility, audienceIds });

function gamePayload(view: string, at: number | null, legacy = false) {
  const all = [
    publicEvent(0, "game.started", {}),
    publicEvent(1),
    privateEvent(
      2,
      "player",
      {
        ...
        submission: { urge: 0, ready: false, plan: null, rationale: "Wait for another clue" },
        report: {
          observations: ["event-1"],
          alternatives: [
            {
              id: "stay",
              description: "Stay quiet",
              advantage: "less exposure",
              drawback: "less information",
            },
          ],
          selectedAlternativeId: "stay",
          summary: "Wait for another clue",
          proposal: { kind: "pass", reason: "Fixture" },
          journalPatch: [],
    ...
    publicEvent(3, "phase.changed", { from: "day_discussion", to: "day_vote" }),
  ];
  const visible =
    view === "moderator"
      ? all
      : view === "player"
        ? all.filter(
            (event) =>
              ...
              (event.visibility === "player" && event.audienceIds.includes("p1")),
          )
        : view === "team"
          ? all.filter((event) => event.visibility === "public")
          : all.filter((event) => event.visibility === "public");
  const sliced =
    at === null
      ? visible
      : visible.slice(0, Math.max(1, at + 1)).map((event, sequence) => ({ ...event, sequence }));
  const safePlayers =
    view === "moderator" ? players : players.map(({ id, name, alive }) => ({ id, name, alive }));
  return {
    game: {
      id: "g-v2",
      name: legacy ? "Legacy Fixture" : "V3.1 Fixture",
      status: "paused",
      config:
        view === "moderator"
          ? legacy
            ? { ...config, schemaVersion: "game_config_v1" }
            : config
          : {
              schemaVersion: legacy ? "game_config_v1" : "game_config_v2",
              ...(legacy
                ? {}
                : {
                    preset: "standard-8-v2",
        ...
        view === "moderator"
          ? {
              p1: {
                ...
                beliefs: [],
                hypotheses: [],
                strategy: "Keep the table talking.",
                goals: ["Find a contradiction"],
                unresolvedQuestions: [],
                deceptionPlan: null,
              },
            }
    ...
    usage:
      view === "moderator"
        ? [
            {
              inputTokens: 1_946_911,
              outputTokens: 83_182,
              totalTokens: 2_030_093,
...
async function mockApi(page: Page, options: { legacy?: boolean } = {}) {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (url.pathname === "/api/v1/games" && request.method() === "GET")
      return json([
        {
          ...gamePayload(url.searchParams.get("view") ?? "public", null, options.legacy),
          state: {
            ...gamePayload("public", null, options.legacy).state,
            players: players.map(({ id, name, alive }) => ({ id, name, alive })),
          },
        },
      ]);
    if (url.pathname === "/api/v1/games" && request.method() === "POST") {
      creations.push(request.postDataJSON() as Record<string, unknown>);
      return json(gamePayload("moderator", null), 201);
    }
    ...
    if (url.pathname.endsWith("/control")) {
      ...
      return json({ ok: true, status: "stepping" });
    }
    if (url.pathname.endsWith("/decisions/d-1"))
      return json({
        opportunity: {
          id: "d-1",
          gameId: "g-v2",
          ...
          packet: {
            ...
            phase: "day_discussion",
            day: 1,
            sources: [{ id: "event-1", type: "message.public", day: 1, scope: "public", data: {} }],
            legalActions: [],
            players: [],
          },
          status: "pending",
          bestSubmission: { urge: 0, ready: false, plan: null, rationale: "Wait for another clue" },
          best: {
            observations: ["event-1"],
            alternatives: [
              {
        ...
        events: [
          privateEvent(
            2,
            "player",
            { decisionId: "d-1", continuation: "continue", verdict: "accepted" },
            ["p1"],
          ),
        ],
      });
    if (url.pathname.endsWith("/decisions"))
      return json(
        url.searchParams.get("view") === "moderator" || url.searchParams.get("view") === "player"
          ? [
              {
                id: "d-1",
                gameId: "g-v2",
    ...
    if (url.pathname.endsWith("/export"))
      return json({ schemaVersion: "werewolf_research_bundle_v3_1" });
    if (url.pathname === "/api/v1/experiments" && request.method() === "GET")
      return json([experiment]);
    if (url.pathname === "/api/v1/experiments" && request.method() === "POST") {
      experiment = { ...experiment, status: "queued" };
      return json(experiment, 201);
    }
    if (url.pathname.match(/\/games\/g-v2$/))
      return json(
        gamePayload(
          url.searchParams.get("view") ?? "public",
          url.searchParams.has("at") ? Number(url.searchParams.get("at")) : null,
          options.legacy,
        ),
      );
    return json({});
...
test("creates the standard V3.1 setup with editable deliberation, concurrency, and resolved model controls", async ({
  page,
}) => {
  ...
  expect(mock.creations[0]).toMatchObject({
    ...
    discussion: {
      speakerSelection: "listener_auction",
      speakerBias: 0.25,
      maxParallelDecisions: 2,
    },
    deliberation: { mode: "gated", requestTimeoutMs: 90000, maxJournalTokens: 1500 },
  });
});

test("keeps observer views safe while covering both stepping controls, decisions, replay, and exports", async ({
  page,
}) => {
  const mock = await mockApi(page);
  await page.goto("/");
  ...
  await expect(page.getByRole("combobox", { name: "Observer perspective" })).toHaveValue(
    "moderator",
  );
...
test("labels legacy games replay-only and clones them into prefilled setup", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Legacy game · run \/ resume \/ phase step are replay-only/),
  ).toBeVisible();
  await page.getByLabel("Spoiler glass").check();
