...
describe("HTTP control surface", () => {
  ...
  it("freezes Jev selection independently of the player model and rejects invalid thresholds", async () => {
    ...
    const payload = { name: "Jev API", seed: "jev-api", preset: "standard-8-v2", seats: ids.map((_, i) => ({ id: `p${i}`, name: `P${i}` })), roleRefs: ids.map(id => ({ id })), decisionEngine: { mode: "jev", model: "jev-latest", reasoningThreshold: 0.6 } };
    const created = await built.app.inject({ method: "POST", url: "/api/v1/games", payload });
    expect(created.statusCode).toBe(201);
    expect(created.json().game.config.decisionEngine).toEqual({...payload.decisionEngine,workflow:"journal_v4"});
    expect(created.json().game.config.modelSettings.p0.model).toBe("fake-model");
    const invalid = await built.app.inject({ method: "POST", url: "/api/v1/games", payload: { ...payload, decisionEngine: { ...payload.decisionEngine, reasoningThreshold: 1.1 } } });
  ...
  it("freezes API-backed Luna and a reasoning-inclusive output budget for new games", async () => {
    ...
    const ids = ["werewolf", "werewolf", "seer", "doctor", "villager", "villager", "villager", "villager"];
    const response = await built.app.inject({ method: "POST", url: "/api/v1/games", payload: {
      name: "API Luna", seed: "api-luna", preset: "standard-8-v2", seats: ids.map((_, i) => ({id:`p${i}`,name:`P${i}`})), roleRefs: ids.map(id => ({id})), decisionEngine: {mode:"jev"},
    } });
    expect(response.statusCode).toBe(201);
    expect(response.json().game.config).toMatchObject({maxTotalTokens:null,deliberation:{maxContextTokens:32000,maxJournalTokens:16000},safety:{maxOutputTokens:8192},modelSettings:{p0:{provider:"openai",model:"gpt-6-luna",reasoningEffort:"xhigh"}},decisionEngine:{mode:"jev",workflow:"journal_v4"}});
  });

  it("removes a finite token ceiling without rewriting history, and preserves unlimited on replay", async () => {
    const built=await buildApi({databaseUrl:":memory:"});apps.push(built.app);
    const ids=["werewolf","werewolf","seer","doctor","villager","villager","villager","villager"];
    const created=await built.app.inject({method:"POST",url:"/api/v1/games",payload:{name:"Unlimited",seed:"unlimited",maxTotalTokens:1_000,seats:ids.map((_,i)=>({id:`p${i}`,name:`P${i}`})),roleRefs:ids.map(id=>({id}))}});
    expect(created.statusCode).toBe(201);const id=created.json().game.id;
    await built.app.inject({method:"POST",url:`/api/v1/games/${id}/control`,payload:{action:"start"}});
    const state=reduceGame(id,built.repository.listEvents(id));
    built.repository.appendEvent(id,{type:"game.budget_exhausted",phase:state.phase,day:state.day,visibility:"public",payload:{reason:"total-token admission threshold reached"}});
    built.repository.updateGame(id,{status:"budget_exhausted",error:"total-token admission threshold reached"});
    const extended=await built.app.inject({method:"POST",url:`/api/v1/games/${id}/control`,payload:{action:"extend_budget",maxTotalTokens:null,maxWallClockMs:1_800_000}});
    expect(extended.statusCode).toBe(200);
    expect(built.repository.getGame(id)?.config).toMatchObject({maxTotalTokens:null});
    expect(reduceGame(id,built.repository.listEvents(id)).config).toMatchObject({maxTotalTokens:null});
    expect((built.repository.listEvents(id).find(event=>event.type==="game.created")?.payload.config as {maxTotalTokens:number}).maxTotalTokens).toBe(1_000);
    built.repository.appendEvent(id,{type:"game.budget_exhausted",phase:state.phase,day:state.day,visibility:"public",payload:{reason:"active-runtime limit reached"}});
    built.repository.updateGame(id,{status:"budget_exhausted"});
    const reduced=await built.app.inject({method:"POST",url:`/api/v1/games/${id}/control`,payload:{action:"extend_budget",maxTotalTokens:100_000_000,maxWallClockMs:3_600_000}});
