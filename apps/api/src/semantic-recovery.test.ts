...
async function paused(repository: LabRepository) {
  const fixture = createActorHoldout(repository, "uncertain-last-wolf"),
    options = holdoutVote(repository, fixture);
  let calls = 0;
  const provider = new AskJevProvider(async (input) => {
    calls++;
    const request = JSON.parse(input),
      keys = Object.keys(request.questions.target.criteria);
    return JSON.stringify({
      ...
      answers: {
        target: {
          ...
          choice: "abstain",
          confidence: 1,
          probabilities: Object.fromEntries(keys.map((key) => [key, Number(key === "abstain")])),
        },
      },
    });
  });
  await expect(
    new DecisionExecutorV2(repository, new FakeDecisionProvider(), provider).execute(options),
...
describe("operator semantic acknowledgment", () => {
  it("validates the API action, commits the recorded abstention, and requires a separate resume", async () => {
    ...
    cleanups.push(() => built.app.close());
    const fixture = await paused(built.repository),
      url = `/api/v1/games/${fixture.game.id}/control`;
    const missing = await built.app.inject({
      method: "POST",
      url,
      payload: { action: "acknowledge_semantic_anomaly", decisionId: fixture.decisionId },
    });
    ...
    const response = await built.app.inject({
      ...
      payload: {
        ...
        note: "Keep the model's recorded anomaly for this experiment",
      },
    });
    ...
    expect(
      built.repository
        ...
        .map((event) => event.payload.vote),
    ).toEqual([{ voterId: fixture.actor.id, targetId: null }]);
  });
  it("supports the documented pilot flag without credentials or a new provider call", async () => {
    const directory = await mkdtemp(join(tmpdir(), "moon-council-ack-"));
    cleanups.push(() => rm(directory, { recursive: true, force: true }));
    const path = join(directory, "game.db"),
      db = openDatabase(path),
      repository = new LabRepository(db);
    const fixture = await paused(repository);
    db.close();
    ...
    const output = execFileSync(
      ...
      [
        ...
        "--game",
        fixture.game.id,
        "--acknowledge-semantic",
        fixture.decisionId,
        "--operator-note",
        "Reviewed the final model receipt",
      ],
      {
        cwd: root,
        encoding: "utf8",
        timeout: 20000,
        env: { PATH: process.env.PATH, LLM_PROVIDER: "fake" },
      },
    );
    expect(JSON.parse(output)).toMatchObject({ status: "committed", gameStatus: "paused" });
    const reopened = openDatabase(path);
    try {
      expect(new LabRepository(reopened).getGame(fixture.game.id)?.status).toBe("paused");
    } finally {
      reopened.close();
    }
  });
