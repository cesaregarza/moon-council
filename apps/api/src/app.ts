...
const Query = z.object({
  ...
  at: z.coerce.number().int().min(0).optional(),
  view: z.enum(["public", "moderator", "player", "team"]).default("public"),
...
export async function buildApi(
  options: ApiOptions = {},
): Promise<{ app: FastifyInstance; repository: LabRepository; connection: DatabaseConnection }> {
  ...
  app.post("/api/v1/roles", async (request, reply) =>
    reply.status(201).send(repository.createRole(request.body)),
  );
  app.get("/api/v1/games", async () =>
    repository.listGames(100).map((game) => observerPayload(repository, game, { kind: "public" })),
  );
  app.post("/api/v1/games", async (request, reply) => {
    const input = CreateGameV2RequestSchema.parse(request.body);
    if (input.roleRefs.length !== input.seats.length)
      return reply.status(400).send({ error: "role_count_mismatch" });
    const roster = PRESET_ROSTERS[input.preset];
    const refs = roster
      ? roster.map((id) => ({ id, version: rosterRoleVersion(id) }))
      : input.roleRefs;
    if (roster && input.seats.length !== 8)
      return reply.status(400).send({ error: "standard_preset_requires_eight_seats" });
    ...
    const config = GameConfigV2Schema.parse({
      ...
      protocolVersion: input.protocolVersion === "agent_v3_2" ? "agent_v3_2" : "agent_v3_1",
      rules: { packExecution: "any_unblocked", revealBallots: true, firstCycle: "day_first" },
      discussion: { ...input.discussion, speakerSelection: "listener_auction" },
      deliberation: {
        ...input.deliberation,
        bidReasoningEffort: input.deliberation.bidReasoningEffort ?? "medium",
      },
      roleDeck,
      moderatorModel: input.moderatorModel ?? resolveModeratorModel(defaultModel, provider),
      modelSettings: Object.fromEntries(
        input.seats.map((seat) => [
          seat.id,
          { model: seat.model ?? defaultModel, reasoningEffort: effort, provider },
        ]),
      ),
    });
  ...
  app.post("/api/v1/games/:id/control", async (request, reply) => {
    ...
    if (input.action === "acknowledge_semantic_anomaly") {
      ...
      return {
        ok: true,
        ...acknowledgeSemanticAnomaly(repository, id, input.decisionId, input.note),
      };
    }
    ...
    if (input.action === "extend_budget") {
      ...
      if (game.status !== "budget_exhausted" && !contextLimited)
        return reply.status(409).send({ error: "game_has_no_extendable_limit" });
      if (input.maxTotalTokens === undefined || input.maxWallClockMs === undefined)
        return reply.status(400).send({ error: "extended_budgets_required" });
      const nextContextTokens =
        input.maxContextTokens ?? currentConfig.deliberation.maxContextTokens;
      if (
        (input.maxTotalTokens !== null &&
          (currentConfig.maxTotalTokens === null ||
            input.maxTotalTokens < currentConfig.maxTotalTokens)) ||
        input.maxWallClockMs < currentConfig.safety.maxWallClockMs ||
        nextContextTokens < currentConfig.deliberation.maxContextTokens
      )
        return reply.status(400).send({ error: "limit_extension_cannot_reduce_limits" });
      const events = repository.listEvents(id);
      const interrupted = contextLimited
        ? events.findLast(
            (event) =>
      ...
      const config = GameConfigV2Schema.parse({
        ...
        safety: { ...currentConfig.safety, maxWallClockMs: input.maxWallClockMs },
        deliberation: { ...currentConfig.deliberation, maxContextTokens: nextContextTokens },
      });
      store.atomic(() => {
        repository.updateGameConfig(id, config);
        repository.appendEvent(id, {
          type: "game.budget_extended",
          phase: interrupted.phase,
          day: interrupted.day,
          visibility: "moderator",
          payload: {
            previousMaxTotalTokens: currentConfig.maxTotalTokens,
            maxTotalTokens: config.maxTotalTokens,
            previousMaxWallClockMs: currentConfig.safety.maxWallClockMs,
            maxWallClockMs: config.safety.maxWallClockMs,
            previousMaxContextTokens: currentConfig.deliberation.maxContextTokens,
            maxContextTokens: config.deliberation.maxContextTokens,
            resumedFromSequence: interrupted.sequence,
            reason: contextLimited ? "operator context extension" : "operator budget extension",
          },
        });
    ...
    if (["completed", "aborted", "budget_exhausted", "failed"].includes(game.status))
      return reply.status(409).send({ error: "game_is_terminal" });
    ensureInitialized(repository, game);
    const state = reduceGame(id, repository.listEvents(id));
    if (input.action === "speed") {
      if (input.speedMs === undefined) return reply.status(400).send({ error: "speedMs_required" });
      repository.updateGame(id, { speedMs: input.speedMs });
    ...
    store.atomic(() => {
      ...
      repository.appendEvent(id, {
        type,
        phase: status === "aborted" ? "ended" : state.phase,
        day: state.day,
        visibility: "public",
        payload: { reason: `operator ${input.action}` },
      });
  ...
  app.get("/api/v1/games/:id/events", async (request, reply) => {
    ...
    return observerEvents(repository, id, viewer(query), query.at).filter(
      (e) => e.sequence > query.after,
    );
  });
  app.get("/api/v1/games/:id/events/stream", async (request, reply) => {
    ...
    const query = Query.parse(request.query);
    if (!repository.getGame(id)) return reply.status(404).send({ error: "not_found" });
    const perspective = viewer(query);
    ...
    const send = () => {
      for (const event of observerEvents(repository, id, perspective, query.at).filter(
        (e) => e.sequence > last,
      )) {
        reply.raw.write(
  ...
  app.get("/api/v1/games/:id/decisions/:decisionId", async (request, reply) => {
    const { id, decisionId } = Params.extend({ decisionId: z.string() }).parse(request.params);
    const query = Query.parse(request.query);
    if (!repository.getGame(id)) return reply.status(404).send({ error: "not_found" });
  ...
  app.get("/api/v1/games/:id/export", async (request, reply) => {
    ...
    if (!game) return reply.status(404).send({ error: "not_found" });
    const perspective = viewer(query);
    const payload = observerPayload(repository, game, perspective, query.at, {
      includeAttempts: true,
    });
    const decisions = decisionRecords(repository, id, perspective, query.at).map((op) =>
      decisionDetail(repository, id, op.id, perspective, query.at),
    );
    const protocol =
    ...
    const bundle = {
      schemaVersion:
        protocol === "agent_v3_2"
          ? "werewolf_research_bundle_v3_2"
          : protocol === "agent_v3_1"
            ? "werewolf_research_bundle_v3_1"
            : protocol === "agent_v3"
              ? "werewolf_research_bundle_v3"
              : game.config.schemaVersion === "game_config_v2"
                ? "werewolf_research_bundle_v2"
                : "werewolf_research_bundle_v1",
      perspective: query.view,
    ...
    reply.header(
      "content-disposition",
      `attachment; filename="${id}-${query.view}.${query.format}"`,
    );
    if (query.format === "jsonl") {
      ...
      return [
        ...
        ...Object.entries(bundle.journals).map(([playerId, journal]) =>
          JSON.stringify({ kind: "journal", playerId, journal }),
        ),
        ...exportDecisions.map((decision) => JSON.stringify({ kind: "decision", decision })),
        ...bundle.attempts.map((attempt) => JSON.stringify({ kind: "attempt", attempt })),
        ...bundle.usage.map((usage) => JSON.stringify({ kind: "usage", usage })),
      ].join("\n");
    }
    return bundle;
  });
  ...
  app.post("/api/v1/experiments", async (request, reply) => {
    ...
    if (spec.schemaVersion !== "experiment_v2")
      return reply.status(409).send({ error: "legacy_replay_only" });
    const experiment = repository.createExperiment(spec);
    repository.enqueueJob("experiment", experiment.id);
    return reply.status(201).send(experiment);
  });
  app.get("/api/v1/experiments/:id", async (request, reply) => {
    const { id } = Params.parse(request.params);
    const experiment = repository.getExperiment(id);
    if (!experiment) return reply.status(404).send({ error: "not_found" });
    return {
      experiment: { ...experiment, summary: summarizeExperiment(repository, id, experiment.spec) },
      games: repository.listGames(100, id),
    };
  });
  app.post("/api/v1/experiments/:id/control", async (request, reply) => {
    ...
    const experiment = repository.getExperiment(id);
    if (!experiment) return reply.status(404).send({ error: "not_found" });
    if (experiment.spec.schemaVersion !== "experiment_v2")
      return reply.status(409).send({ error: "legacy_replay_only" });
    repository.updateExperiment(id, {
      status: action === "pause" ? "paused" : "queued",
      error: null,
    });
    for (const game of repository.listGames(100, id))
      if (action === "pause" && ["running", "stepping"].includes(game.status))
