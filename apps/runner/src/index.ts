...
async function processNextJob(): Promise<boolean> {
  ...
  try {
    if (job.kind === "experiment") {
      ...
      if (
        experiment?.spec.schemaVersion === "experiment_v2" &&
        Object.values(experiment.spec.baseConfig.modelSettings).some(
          (settings) => settings.provider !== providerKind,
        )
      ) {
    ...
    if (before.config.schemaVersion !== "game_config_v2") {
      repository.finishJob(job.id, "legacy replay-only game cannot resume");
      return true;
    }
    ...
    if (
      Object.values(before.config.modelSettings).some(
        (settings) => settings.provider !== providerKind,
      )
    ) {
      ...
      repository.appendEvent(before.id, {
        type: "game.paused",
        phase: state.phase,
        day: state.day,
        visibility: "public",
    ...
    if (
      wasStepping &&
      !["completed", "budget_exhausted", "aborted", "failed", "paused"].includes(after.status)
    ) {
      ...
      (!after.experimentId || repository.getExperiment(after.experimentId)?.status !== "paused")
    ) {
    ...
    if (job.kind === "game")
      repository.updateGame(job.targetId, { status: "failed", error: message });
...
function shutdown(): void {
  ...
  if (activeExperimentId)
    repository.updateExperiment(activeExperimentId, {
      status: "paused",
      error: "runner shutting down",
    });
  for (const game of repository.listGames(100)) {
    const lease = connection.sqlite
      .prepare("SELECT owner FROM runner_leases WHERE game_id=?")
      .get(game.id) as { owner: string } | undefined;
    if (
      (lease?.owner === owner ||
        Boolean(activeExperimentId && game.experimentId === activeExperimentId)) &&
      ["running", "stepping"].includes(game.status)
    ) {
      ...
      repository.appendEvent(game.id, {
        ...
        day: state.day,
        visibility: "public",
        payload: { reason: "runner shutting down" },
      });
...
console.log(
  `Werewolf runner ready · provider=${providerKind} · model=${defaultModel} · db=${connection.path}`,
);
await run();
connection.close();
