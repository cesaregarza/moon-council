...
class CheckpointTests(unittest.TestCase):
    def setUp(self):
        ...
        ''')
        config = {'rules': {'firstCycle': 'day_first'}, 'decisionEngine': {'mode': 'jev', 'workflow': 'journal_v4'}, 'modelSettings': {'p1': {'provider': 'openai', 'model': 'example', 'reasoningEffort': 'xhigh'}}, 'seed': 'synthetic'}
        self.db.execute('INSERT INTO games VALUES(?,?,?)', ('game', 'paused', json.dumps(config)))
        self.db.execute('INSERT INTO events VALUES(?,?,?,?,?)', ('game', 12, 1, 'night_team', 'game.paused'))
        self.db.execute('INSERT INTO provider_attempts VALUES(?,?,?)', ('game', 'valid', json.dumps({'provider': 'jev', 'usage': {'totalTokens': 123}})))
        self.db.commit()
    ...
    def test_rejects_old_workflow_and_incompatible_phase(self):
        self.db.execute("UPDATE games SET config_json=json_set(config_json,'$.decisionEngine.workflow','journal_v3')")
        self.db.commit()
        with self.assertRaisesRegex(ValueError, 'archived workflows'):
            self.capture()
        self.db.execute("UPDATE games SET config_json=json_set(config_json,'$.decisionEngine.workflow','journal_v4')")
    ...
    def test_rejects_night_calls_and_unresolved_decisions(self):
        self.db.execute("INSERT INTO events VALUES('game',11,1,'night_team','model.attempt_started')")
        self.db.commit()
        with self.assertRaisesRegex(ValueError, 'Night work'):
            self.capture()
        self.db.execute('DELETE FROM events WHERE sequence=11')
        self.db.execute("INSERT INTO agent_records VALUES('game','decision:a','{\"status\":\"pending\"}')")
        self.db.commit()
    ...
    def test_cli_runs_via_shebang_and_rejects_mounted_paths(self):
        ...
        result = subprocess.run([script,'capture','--db',str(self.source),'--game','game','--out',str(self.root/'base'),'--source-revision','a'*40],capture_output=True,text=True,check=True)
        self.assertEqual(json.loads(result.stdout)['phase'],'night_team')
        result = subprocess.run([script,'fork','--base',str(self.root/'base'),'--out','/mnt/c/forbidden'],capture_output=True,text=True)
