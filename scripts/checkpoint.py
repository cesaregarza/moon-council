...
def describe(connection, game_id):
    row = connection.execute('SELECT status,config_json FROM games WHERE id=?', (game_id,)).fetchone()
    if not row:
        raise ValueError('Unknown game')
    config = json.loads(row[1])
    last = connection.execute('SELECT sequence,day,phase FROM events WHERE game_id=? ORDER BY sequence DESC LIMIT 1', (game_id,)).fetchone()
    ...
    if config.get('rules', {}).get('firstCycle') != 'day_first' or engine.get('mode') != 'jev' or engine.get('workflow') != 'journal_v4':
        raise ValueError('Base requires day-first Jev journal_v4; archived workflows cannot be relabeled')
    night_work = connection.execute("SELECT count(*) FROM events WHERE game_id=? AND day=1 AND phase LIKE 'night_%' AND type IN ('model.attempt_started','team.point','team.agreement_frozen','night.action_submitted','night.resolved')", (game_id,)).fetchone()[0]
    pending = connection.execute("SELECT count(*) FROM agent_records WHERE game_id=? AND record_key LIKE 'decision:%' AND json_extract(value_json,'$.status') NOT IN ('committed','superseded')", (game_id,)).fetchone()[0]
    inflight = connection.execute("SELECT count(*) FROM provider_attempts WHERE game_id=? AND status IN ('started','received')", (game_id,)).fetchone()[0]
    if night_work or pending or inflight:
        raise ValueError('Night work or unresolved decisions prevent a reusable Day 1 base')
    counts = connection.execute("SELECT provider,count(*),sum(coalesce(json_extract(value_json,'$.usage.totalTokens'),0)) FROM (SELECT json_extract(value_json,'$.provider') AS provider,value_json FROM provider_attempts WHERE game_id=?) GROUP BY provider ORDER BY provider", (game_id,)).fetchall()
    return {'gameId': game_id, 'status': row[0], 'day': last[1], 'phase': last[2], 'eventSequence': last[0],
            'configSha256': hashlib.sha256(row[1].encode()).hexdigest(), 'decisionEngine': engine,
            'modelSettings': config.get('modelSettings'), 'seed': config.get('seed'),
            'baselineUsage': [{'provider': provider, 'calls': count, 'knownTokens': tokens} for provider, count, tokens in counts]}
...
def capture(db, game_id, output, revision):
    ...
    manifest = {'schema': SCHEMA, 'sourceRevision': revision, 'databaseSha256': digest(target), **metadata}
...
def fork(base, output):
    ...
    if digest(database) != manifest['databaseSha256'] or digest(base / 'checkpoint.sqlite') != manifest['databaseSha256']:
        raise ValueError('Database changed while copying the checkpoint')
    lineage = {'schema': 'moon_council_checkpoint_fork_v1', 'baseSha256': manifest['databaseSha256'],
               'sourceRevision': manifest['sourceRevision'], 'gameId': manifest['gameId'],
               'baselineUsage': manifest['baselineUsage'], 'baseEventSequence': manifest['eventSequence']}
...
def main():
    ...
    save = commands.add_parser('capture', help='Capture a paused v4 Day 1 database; output must not exist')
    ...
    for name in ['inspect', 'fork']:
        ...
        if name == 'fork': command.add_argument('--out', required=True)
    args = parser.parse_args()
    try:
        result = capture(args.db, args.game, args.out, args.source_revision) if args.command == 'capture' else fork(args.base, args.out) if args.command == 'fork' else inspect(args.base)
