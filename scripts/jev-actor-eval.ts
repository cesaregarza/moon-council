...
async function main() {
  ...
  const directory=join(parent,basename(output));await mkdir(directory,{mode:0o700});
  const results=[];
  for(const c of cases){
    await writeFile(join(directory,`${c.label}.input.json`),JSON.stringify(c,null,2)+"\n",{mode:0o600,flag:"wx"});
    const start=Date.now();
    let response;
    try{response=await evaluateProbeCase(c);}catch{response={raw:"",response:null,error:"Transport failed; see private ask-jev request log"};}
    const answers=response.response?.answers;
    const target=answers?.target;
    const score=(key:string)=>{const a=answers?.[key];return a?.type==="score"?a.score:undefined;};
    const passed=c.expected!==undefined?target?.type==="choice"&&target.choice===c.expected:score("listen_3")!==undefined&&score("listen_1")!==undefined&&score("listen_3")!>score("listen_1")!;
    const result={...describeProbeCase(c),promptVersion:c.promptVersion,latencyMs:Date.now()-start,expected:c.expected??"listen_3 > listen_1",rubric:c.rubric,passed,...response};
    results.push(result);await writeFile(join(directory,"results.json"),JSON.stringify(results,null,2)+"\n",{mode:0o600});
    console.log(JSON.stringify({label:c.label,passed,model:response.response?.model,latencyMs:result.latencyMs,usage:response.response?.usage,error:response.error}));
