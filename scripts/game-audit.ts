...
const maxIds = (values: Array<{ playerId: string; value: number }>): string[] => {
  ...
  return values.filter(item => Math.abs(item.value - highest) < 1e-9).map(item => item.playerId);
};
...
function aliveAt(players: EnginePlayer[], events: AuditEvent[], sequence: number): Set<string> {
  const eliminated = new Set(events.filter(event => event.sequence < sequence && event.type === "player.eliminated").map(event => String(event.payload.playerId)));
  return new Set(players.filter(player => !eliminated.has(player.id)).map(player => player.id));
}

function latestBelief(events: AuditEvent[], observerId: string, targetId: string, beforeSequence: number): number | null {
  const update = events.findLast(event => event.sequence < beforeSequence && event.type === "journal.v2_updated" && event.payload.playerId === observerId);
  const journal = update?.payload.journal as { beliefs?: Array<{ playerId?: string; probability?: number }> } | undefined;
...
export function summarizeGameAudit(game: AuditGame, events: AuditEvent[], attempts: AuditAttempt[]) {
  ...
  const ending = events.findLast(event => ["game.ended", "game.budget_exhausted", "game.aborted"].includes(event.type));
  const latest = events.at(-1);
  const eliminations = events.filter(event => event.type === "player.eliminated").map(event => ({
    sequence: event.sequence,
    day: event.day,
    playerId: String(event.payload.playerId),
    playerName: String(event.payload.playerName ?? playerById.get(String(event.payload.playerId))?.name ?? event.payload.playerId),
    roleName: String(event.payload.roleName ?? playerById.get(String(event.payload.playerId))?.role.name ?? "unknown"),
  ...
  const winnerIds = new Set(Array.isArray(ending?.payload.winnerPlayerIds) ? ending.payload.winnerPlayerIds.map(String) : []);
  ...
  const auctionDetails = auctions.map(event => {
    const scores = Array.isArray(event.payload.scores) ? event.payload.scores as AuctionScore[] : [];
    const selectedPlayerId = event.payload.selectedPlayerId == null ? null : String(event.payload.selectedPlayerId);
    const alive = aliveAt(players, events, event.sequence);
    const suspicion = scores.map(score => {
      const observations = [...alive].filter(observerId => observerId !== score.playerId).flatMap(observerId => {
        const value = latestBelief(events, observerId, score.playerId, event.sequence);
        return value == null ? [] : [value];
      });
      return { playerId: score.playerId, meanProbability: mean(observations), observers: observations.length };
    });
    const coveredSuspicion = suspicion.filter((item): item is { playerId: string; meanProbability: number; observers: number } => item.meanProbability != null);
    const maxSuspicionIds = maxIds(coveredSuspicion.map(item => ({playerId:item.playerId,value:item.meanProbability})));
    return {
      sequence: event.sequence,
      day: event.day,
      ...
      selectedPlayerId,
      selectedPlayerName: selectedPlayerId ? playerById.get(selectedPlayerId)?.name ?? selectedPlayerId : null,
      candidateCount: scores.length,
      selectedScore: scores.find(score => score.playerId === selectedPlayerId) ?? null,
      maximumUrgeIds: maxIds(scores.map(score => ({playerId:score.playerId,value:score.urge}))),
      maximumListenerInterestIds: maxIds(scores.map(score => ({playerId:score.playerId,value:score.listenerInterest}))),
  ...
  const byPlayer = roster.map(player => {
    const selected = auctionDetails.filter(auction => auction.selectedPlayerId === player.playerId);
    const playerSpeeches = speeches.filter(event => event.payload.playerId === player.playerId);
    const formalActs = speeches.flatMap(event => (Array.isArray(event.payload.acts) ? event.payload.acts : []).map(act => ({event, act:act as JsonRecord})));
    ...
    return {
      ...
      formalAccusationsReceived: formalActs.filter(({act}) => act.kind === "accusation" && act.targetId === player.playerId).length,
      formalChallengesReceived: formalActs.filter(({act}) => act.kind === "challenge" && act.targetId === player.playerId).length,
      selectedWithMaximumUrge: selected.filter(auction => auction.maximumUrgeIds.includes(player.playerId)).length,
      selectedWithMaximumListenerInterest: selected.filter(auction => auction.maximumListenerInterestIds.includes(player.playerId)).length,
      selectedWithMaximumPrivateSuspicion: selected.filter(auction => auction.maximumPrivateSuspicionIds.includes(player.playerId)).length,
      meanPrivateSuspicionWhenSelected: mean(selectedSuspicion),
      meanUrgeWhenSelected: mean(selected.flatMap(auction => auction.selectedScore ? [auction.selectedScore.urge] : [])),
      meanListenerInterestWhenSelected: mean(selected.flatMap(auction => auction.selectedScore ? [auction.selectedScore.listenerInterest] : [])),
    };
  });

  const statusCounts = Object.fromEntries([...new Set(attempts.map(attempt => attempt.status))].sort().map(status => [status, attempts.filter(attempt => attempt.status === status).length]));
  const attemptContext = new Map(events.filter(event => event.type === "model.attempt_started").map(event => [String(event.payload.attemptId), {day:event.day,phase:event.phase}]));
  const latencyValues = attempts.flatMap(attempt => attempt.latencyMs == null ? [] : [attempt.latencyMs]);
  const input = tokenField(attempts, "inputTokens");
  const cached = tokenField(attempts, "cachedInputTokens");
  const byTask = [...new Set(attempts.map(attempt => attempt.schemaVersion))].sort().map(schemaVersion => {
    ...
    return {
      ...
      cachePercent: taskInput.knownTotal ? 100 * taskCached.knownTotal / taskInput.knownTotal : null,
      medianLatencyMs: median(rows.flatMap(row => row.latencyMs == null ? [] : [row.latencyMs])),
    };
  });
  const dayPhaseKeys = [...new Set(attempts.map(attempt => {
  ...
  const byDayPhase = dayPhaseKeys.map(key => {
    ...
    const taskInput = tokenField(rows,"inputTokens"),taskOutput = tokenField(rows,"outputTokens"),taskCached = tokenField(rows,"cachedInputTokens");
    return {
      day:day === "unknown" ? null : Number(day),phase,attempts:rows.length,valid:rows.filter(row=>row.status==="valid").length,invalid:rows.filter(row=>row.status==="invalid").length,
      inputTokens:taskInput.knownTotal,outputTokens:taskOutput.knownTotal,cachedInputTokens:taskCached.knownTotal,cachePercent:taskInput.knownTotal?100*taskCached.knownTotal/taskInput.knownTotal:null,
      medianLatencyMs:median(rows.flatMap(row=>row.latencyMs==null?[]:[row.latencyMs])),
    };
  });
  const voteEvents = events.filter(event => event.type === "vote.resolved");
  const ballots = voteEvents.flatMap(event => Array.isArray(event.payload.ballots) ? (event.payload.ballots as Array<{voterId:string;targetId:string|null}>).map(ballot => ({day:event.day,...ballot})) : []);
  const villageBallots = ballots.filter(ballot => playerById.get(ballot.voterId)?.role.alignment === "village" && ballot.targetId);
  const villageVotesForWolves = villageBallots.filter(ballot => playerById.get(String(ballot.targetId))?.role.alignment === "werewolf").length;
  const packAgreements = events.filter(event => event.type === "team.agreement_frozen");
  const typeCounts = Object.fromEntries([...new Set(events.map(event => event.type))].sort().map(type => [type, events.filter(event => event.type === type).length]));
  const startedAt = events.find(event => event.type === "game.started")?.createdAt ?? game.createdAt;
  ...
  return {
    ...
    game: {
      ...
      ending: ending ? {type:ending.type,day:ending.day,createdAt:ending.createdAt,payload:ending.payload} : null,
      startedAt,
      endedAt,
    ...
    mechanics: {
      ...
      villageVotingAccuracy: villageBallots.length ? villageVotesForWolves / villageBallots.length : null,
      nightResolutions: events.filter(event => event.type === "night.resolved").length,
      packAgreements: packAgreements.length,
      unanimousPackTargets: packAgreements.filter(event => event.payload.reason === "unanimous" && event.payload.targetId != null).length,
    ...
    discussion: {
      ...
      selectedWithMaximumListenerInterest: auctionDetails.filter(auction => auction.selectedPlayerId && auction.maximumListenerInterestIds.includes(auction.selectedPlayerId)).length,
      selectedWithMaximumPrivateSuspicion: auctionDetails.filter(auction => auction.selectedPlayerId && auction.maximumPrivateSuspicionIds.includes(auction.selectedPlayerId)).length,
      privateSuspicionCoverageAuctions: auctionDetails.filter(auction => auction.suspicion.some(item => item.observers > 0)).length,
      byPlayer,
      auctionsByDay: [...new Set(auctionDetails.map(auction => auction.day))].sort((a,b)=>a-b).map(day => ({day,auctions:auctionDetails.filter(auction => auction.day === day).length,speeches:speeches.filter(event => event.day === day).length})),
      auctionDetails,
    },
    provider: {
      models: Object.fromEntries([...new Set(attempts.map(attempt=>`${attempt.provider}:${attempt.model}`))].sort().map(key=>[key,attempts.filter(attempt=>`${attempt.provider}:${attempt.model}`===key).length])),
      reasoningEfforts: Object.fromEntries([...new Set(attempts.map(attempt=>attempt.reasoningEffort))].sort().map(key=>[key,attempts.filter(attempt=>attempt.reasoningEffort===key).length])),
