...
spec=importlib.util.spec_from_file_location('snapshot',Path(__file__).with_name('public-snapshot.py'))
snapshot=importlib.util.module_from_spec(spec)
spec.loader.exec_module(snapshot)

class SnapshotTests(unittest.TestCase):
    def test_excludes_private_artifacts(self):
        for path in ['.env','apps/api/.env.production','data/pilots/one/research.json','docs/research/report.md','docs/HANDOFF_V3_2.md','compose.pi.yaml','scripts/deploy-pi.sh','apps/api/src/old.ts.orig','packages/x/private.key','scripts/__pycache__/x.pyc']:
            self.assertFalse(snapshot.allowed(path),path)
        for path in ['.env.example','packages/llm/src/jev.ts','docs/ACTOR_WORKFLOW.md','.github/workflows/checks.yml']:
    ...
    def test_scrub_removes_local_setup_and_ignores_runtime_data(self):
        ...
        with patch.dict(os.environ, {'PUBLIC_SNAPSHOT_REPLACEMENTS': '{"'+private_path+'":"moon-council"}'}):
    ...
    def test_bare_private_hostnames_are_detected(self):
        ...
        clean=snapshot.sanitize('apps/web/vite.config.ts',('allowedHosts: ["'+host+'"],').encode())
        self.assertNotIn(host,clean.decode())
    def test_refuses_mounted_paths(self):
        with self.assertRaises(ValueError):snapshot.native('/mnt/c')
