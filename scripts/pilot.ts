...
import { loadProviderEnvironment, AskJevProvider, createDecisionProvider, type DecisionProvider } from "@werewolf/llm";
...
const {values}=parseArgs({options:{"acknowledge-semantic":{type:"string"},"operator-note":{type:"string"},help:{type:"boolean"},journals:{type:"boolean"},"pause-at-first-night":{type:"boolean"},"event-type":{type:"string",multiple:true},task:{type:"string"},"jev-input":{type:"boolean"},attempt:{type:"string"},"jev-review":{type:"boolean"},"evidence":{type:"boolean"},player:{type:"string"},decision:{type:"string"},"decision-engine":{type:"string"},"jev-model":{type:"string"},"max-output-tokens":{type:"string",default:"8192"},"max-calls":{type:"string",default:"500"},"max-tokens":{type:"string",default:"unlimited"},"max-minutes":{type:"string",default:"30"},provider:{type:"string",default:"fake"},model:{type:"string"},effort:{type:"string",default:"xhigh"},seed:{type:"string",default:"standard-v3-pilot"},db:{type:"string"},game:{type:"string"},resume:{type:"boolean"},out:{type:"string"},audit:{type:"boolean"},snapshot:{type:"boolean"},mode:{type:"string",default:"gated"},parallel:{type:"string",default:"4"},"speaker-bias":{type:"string",default:"0.25"},"inspect-bundle":{type:"string"},match:{type:"string",multiple:true},day:{type:"string"},limit:{type:"string"},offset:{type:"string"}}});
if(values.help) console.log("Saved journals as Markdown: --inspect-bundle research.json --journals [--player p7]. Read-only; private game information.\nExact recorded CLI input: --inspect-bundle research.json --jev-input --decision ID [--attempt ID]. Writes the original JSON without a wrapper.\nAdd --jev-review [--player p1] [--decision ID] [--task vote_choice] [--evidence] to extract Jev gates, decisions and explicit LLM advice. Contains private game evidence.\nOffline inspection (no database writes or model calls): --inspect-bundle research.json [--match NAME_OR_ID ...] [--day DAY] [--limit 30] [--offset 0] [--event-type TYPE ...] [--player ID]. Event-type selection explicitly includes full matching event payloads. Prints latency/usage and only explicitly matched speech, journal patches, and reports as JSON.");
if(values["inspect-bundle"] && !values.help){
  if(values.resume||values.snapshot||values.db||values.game||values.out||values["pause-at-first-night"])throw new Error("Bundle inspection cannot be combined with game execution or database/output options");
  if(values.journals){
    if(values["jev-input"]||values["jev-review"]||values.evidence||values.decision||values.attempt||values.task||values.day||values.match||values.limit||values.offset||values["event-type"])throw new Error("Use --journals with optional --player only");
    const {journalSnapshotMarkdown}=await import("../packages/simulator/src/audit-bundle-v2");
    process.stdout.write(journalSnapshotMarkdown(JSON.parse(await readFile(await nativePath(values["inspect-bundle"]),"utf8")),values.player));
    process.exit(0);
  }
  if(values["jev-input"]){
    if(!values.decision || values["jev-review"] || values.evidence || values.task || values.player || values.day || values.match || values.limit || values.offset || values["event-type"])throw new Error("Use --jev-input with --decision and optional --attempt only");
    ...
    process.stdout.write(extractJevInput(bundle,{decision:values.decision,attempt:values.attempt}));
    process.exit(0);
  }
  if(values.attempt)throw new Error("--attempt requires --jev-input");
  if(!values["jev-review"] && (values.evidence||values.decision||values.task))throw new Error("Jev filters require --jev-review");
  if(values["jev-review"] && (values.match||values.limit||values.offset||values["event-type"]))throw new Error("Use --day, --player or --decision with --jev-review");
  ...
  const bundle=JSON.parse(await readFile(await nativePath(values["inspect-bundle"]),"utf8"));
  console.log(JSON.stringify(values["jev-review"] ? reviewJevDecisions(bundle,{day:values.day===undefined?undefined:Number(values.day),player:values.player,decision:values.decision,task:values.task,evidence:values.evidence}) : inspectResearchBundle(bundle,{eventTypes:values["event-type"],player:values.player,matches:values.match,day:values.day===undefined?undefined:Number(values.day),limit:values.limit===undefined?undefined:Number(values.limit),offset:values.offset===undefined?undefined:Number(values.offset)}),null,2));
  process.exit(0);
}
if(!values.help && (values.journals||values.match||values.day||values.limit||values.offset||values["jev-review"]||values.evidence||values.player||values.decision||values.task||values["event-type"]||values["jev-input"]||values.attempt))throw new Error("Inspection filters require --inspect-bundle");
if(values.help){console.log("Usage: npm run pilot -- --provider fake|codex|openai --model MODEL --effort xhigh --seed SEED [--decision-engine llm|jev] [--jev-model jev-latest] [--max-output-tokens 8192] [--max-calls 500] [--max-tokens unlimited|NUMBER] [--max-minutes 30] [--mode gated|single] [--parallel 4] [--speaker-bias 0.25] [--db NATIVE_PATH] [--out DIRECTORY] [--pause-at-first-night]\nNew games use protocol V3.1: stable public E refs and per-player private R refs, small task-specific model responses, reactive listener bids, selected-only speech generation, sparse memory, and a full Day 1 before Night 1. Add --pause-at-first-night to pause after Day 1 ballots, before any Night 1 work. Resume the SAME game: --db DB --game ID --resume (omit the pause flag). Audit only: --db DB --game ID --audit. Acknowledge a final semantic anomaly without model calls: --db DB --game ID --acknowledge-semantic DECISION --operator-note NOTE (commits the recorded ballot and stays paused). Add --snapshot to export an immutable SQLite backup in the output directory. Live providers require an explicit model; never substitute another model. Artifacts are moderator-spoiler JSON and Markdown; no credentials or hidden reasoning traces.");process.exit(0);}
if (values["acknowledge-semantic"]) {
  if (!values.db || !values.game || !values["operator-note"] || values.resume || values.audit || values["inspect-bundle"]) {
    throw new Error("Use --acknowledge-semantic DECISION --operator-note NOTE --db DB --game GAME; resume separately");
  }
  const connection = openDatabase(await nativePath(values.db));
  try {
    const result = acknowledgeSemanticAnomaly(new LabRepository(connection), values.game, values["acknowledge-semantic"], values["operator-note"]);
...
const decisionEngine = { workflow: "journal_v4", mode: values["decision-engine"] ?? "llm", model: values["jev-model"] ?? "jev-latest" };
if (!["llm", "jev"].includes(decisionEngine.mode)) throw new Error("--decision-engine must be llm or jev");
const maxModelCalls = Number(values["max-calls"]), maxTotalTokens = values["max-tokens"] === "unlimited" ? null : Number(values["max-tokens"]), maxWallClockMs = Number(values["max-minutes"]) * 60_000;
if (!Number.isInteger(maxModelCalls) || maxModelCalls < 20 || maxModelCalls > 10_000) throw new Error("--max-calls must be an integer from 20 to 10000");
if (maxTotalTokens !== null && (!Number.isInteger(maxTotalTokens) || maxTotalTokens < 1_000 || maxTotalTokens > 100_000_000)) throw new Error("--max-tokens must be unlimited or an integer from 1000 to 100000000");
if (!Number.isInteger(maxWallClockMs) || maxWallClockMs < 60_000 || maxWallClockMs > 86_400_000) throw new Error("--max-minutes must be from 1 to 1440");
...
if (values.resume && ((values["decision-engine"] && game.config.decisionEngine.mode !== values["decision-engine"]) || (values["jev-model"] && game.config.decisionEngine.model !== values["jev-model"]))) throw new Error("Resume must use the game's frozen decision engine and Jev model");
console.log(JSON.stringify({kind:"pilot",gameId:game.id,databasePath,directory,decisionEngine:game.config.decisionEngine,modelSettings:game.config.modelSettings,safety:game.config.safety,maxTotalTokens:game.config.maxTotalTokens}));
...
const logged = (provider: DecisionProvider): DecisionProvider => ({
  ...
  decide: async request => {
    ...
    const info = { actor:request.playerId,day:request.contextV2?.day,phase:request.contextV2?.phase,task:request.schemaName,model:request.model,effort:request.reasoningEffort };
    console.log(JSON.stringify({kind:"call",...info}));
    const result = await provider.decide(request);
    console.log(JSON.stringify({kind:"call_completed",...info,provider:result.provider,latencyMs:Date.now()-started,totalTokens:result.usage.totalTokens}));
    return result;
  },
});
const orchestrator=new V2GameOrchestrator(repository,logged(base),logged(new AskJevProvider()));
const startingState=orchestrator.initialize(game.id);
  if(!["completed","aborted","budget_exhausted","failed"].includes(game.status))new DecisionStore(repository).atomic(()=>{
    if(values.resume&&game.status==="paused")repository.appendEvent(game.id,{type:"game.resumed",phase:startingState.phase,day:startingState.day,visibility:"public",payload:{reason:"operator pilot resume"}});
  ...
  for(let step=0;step<128&&["running","stepping"].includes(repository.getGame(game.id)!.status);step++){
    ...
    if(values["pause-at-first-night"]&&before.day===1&&before.phase.startsWith("night_")){
      new DecisionStore(repository).atomic(()=>{
        repository.appendEvent(game.id,{type:"game.paused",phase:before.phase,day:before.day,visibility:"public",payload:{reason:"operator first-night checkpoint"}});
        repository.updateGame(game.id,{status:"paused",error:null});
      });
      checkpointReached=true;
      console.log(JSON.stringify({kind:"checkpoint",gameId:game.id,status:"paused",day:before.day,phase:before.phase,reason:"first night; no night actions started"}));
      break;
    }
    await orchestrator.runGameStep(game.id);
    const current=repository.getGame(game.id)!,state=reduceGame(game.id,repository.listEvents(game.id));
    console.log(JSON.stringify({kind:"progress",status:current.status,day:state.day,phase:state.phase,attempts:new DecisionStore(repository).attempts(game.id).length,error:current.error}));
  }
}
...
const details=decisionRecords(repository,game.id,{kind:"moderator"}).map(opportunity=>{
  ...
  const attempts=(payload.attempts??[]).filter(attempt=>attempt.decisionId===opportunity.id);
  return {opportunity,attempts,turns:events.filter(event=>event.type==="decision.reported").map(event=>event.payload),events};
});
...
connection.close();
if(audit.issues.length)process.exitCode=1;else if(!values.audit&&!checkpointReached&&audit.status!=="completed")process.exitCode=audit.status==="paused"?3:2;
