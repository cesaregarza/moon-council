...
type Row = {
  ...
  latencyMs: number;
  usage: UsageV2;
  original: { latencyMs: number; usage: UsageV2 };
  error: string | null;
  metadata?: { cacheDiagnostics?: { type?: string; reason?: string } | null };
};
...
export async function writeCacheComparison(paths: string[], output: string) {
  const summaries = await Promise.all(
    paths.map(async (path) => JSON.parse(await readFile(resolve(path), "utf8")) as Summary),
  );
  ...
  if (
    ...
    baseline.sourceGame !== revised.sourceGame ||
    baseline.rows.length !== revised.rows?.length
  )
    throw new Error("Need two completed live replays of the same game/sample");
  for (const [index, row] of baseline.rows.entries()) {
    const other = revised.rows[index]!;
    if (row.error || other.error || !row.usage || !other.usage)
      throw new Error("Cannot call a failed replay a successful comparison");
    if (
      ["attemptId", "sourcePromptHash", "model", "effort"].some(
        (key) => row[key as keyof Row] !== other[key as keyof Row],
      )
    )
      throw new Error(
        "Comparison requires identical source prompts, sample order, model and effort",
      );
  }
  const original = baseline.rows.map((row) => ({
    ...row,
    usage: row.original.usage,
    latencyMs: row.original.latencyMs,
  }));
  const series = [
    { name: "Recorded Codex", rows: original },
    { name: "Initial API layout", rows: baseline.rows },
    { name: "Four-layer API", rows: revised.rows },
  ];
  ...
  const table = series.map((s, index) => {
    ...
    const estimate =
      index > 0 && luna && totals.inputRateEquivalentTokens !== null
        ? (totals.inputRateEquivalentTokens * 0.1 + totals.outputTokens * 0.5) / 1_000_000
        : null;
  ...
  const text = [
    "# Moon Council: recorded-decision cache experiment",
    "",
    `Source game: ${baseline.sourceGame}. ${baseline.rows.length} frozen decisions replayed in each API layout; all schema and domain checks passed. Model and effort were preserved (${baseline.rows[0]!.model}, ${baseline.rows[0]!.effort}).`,
    "",
    "| Transport/layout | Input tokens | Cached reads | Cache writes | Output tokens, including reasoning | Median wall time | Estimated API token cost |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...table,
    "",
    ...
    "",
    "## Actual cache receipts",
    "",
    "| Replay | Initial API reads | Four-layer reads | Four-layer writes | Four-layer diagnostic |",
    "| --- | ---: | ---: | ---: | --- |",
    ...rows,
    "",
    ...
    "",
    `Exact receipts: ${paths.join(" and ")}. Requests/outputs in their sibling files contain private game context; credentials and hidden reasoning are not included.`,
    "",
  ].join("\n");
